...
/obj/item/device/traitor_announcer/attack_self(mob/living/user, modifiers)
	...
	var/origin = sanitize_text(reject_bad_text(tgui_input_text(user, "Who is announcing, or where is the announcement coming from?", "Announcement Origin", get_area_name(user), max_length = 56), ascii_only = FALSE)) // FF edit: now can use ru characters
	if(!origin)
	...
	var/title = sanitize_text(reject_bad_text(tgui_input_text(user, "Choose the title of the announcement.", "Announcement Title", max_length = 84), ascii_only = FALSE)) // FF edit: now can use ru characters
	if(!title)
		balloon_alert(user, "bad title!")
		return

	var/input = sanitize_text(reject_bad_text(tgui_input_text(user, "Choose the bodytext of the announcement.", "Announcement Text", multiline = TRUE), ascii_only = FALSE)) // FF edit: now can use ru characters
	if(!input)
	...
	priority_announce(
		...
		encode_title = FALSE, encode_text = FALSE,
	)
