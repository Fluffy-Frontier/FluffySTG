...
SUBSYSTEM_DEF(automapper)
	...
	var/ff_config_file = "_maps/fluffy_frontier/automapper/automapper_config.toml" // FLUFFY FRONTIER ADDITION - MODULAR AUTOMAPPER
	var/temporary_config_file = "_maps/skyrat/automapper/temporary_automapper_config.toml" // FLUFFY FRONTIER ADDITION - MODULAR AUTOMAPPER
...
/datum/controller/subsystem/automapper/Initialize()
	// FLUFFY FRONTIER REMOVAL - MODULAR AUTOMAPPER: loaded_config = rustg_read_toml_file(config_file)
	// FLUFFY FRONTIER ADDITION BEGIN - MODULAR AUTOMAPPER

	// По идеи - этот код навряд ли будут в скором времени менять, но если крысы внесут какие-то серьёзные правки, которые вызовут анал-карнавал - то:
	// Либо тыкайте наш dev, чтобы фиксили и подгоняли всё для работы с изменённой/обновлённой версией кода.
	// Либо же просто можете забить болт и взять просто версию крыс, дабы не задерживать обновление билда до момента фикса авто-маппера.
	...
	// Также плюсом ко всему этому - возможность выгружать и отключать определённые шаблоны Крыс, чтобы заменить их на свой вариант.
