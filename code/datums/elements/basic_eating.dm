...
/datum/element/basic_eating/proc/finish_eating(mob/living/eater, atom/target, mob/living/feeder)
	set waitfor = FALSE
	if(drinking)
		playsound_if_pref(eater.loc,'tff_modular/modules/sec-antag-balance-patch-super-cool/slurp.ogg', rand(10,50), TRUE, pref_to_check = /datum/preference/toggle/sound_eating) // NOVA EDIT CHANGE - Original: playsound(eater.loc,'tff_modular/modules/sec-antag-balance-patch-super-cool/slurp.ogg', rand(10,50), TRUE)
	else
