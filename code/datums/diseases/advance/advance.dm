...
/datum/disease/advance/proc/assign_properties()
	...
	if(properties?.len)
		if(properties["stealth"] >= 2) // FLUFFY FRONTIER EDIT - ORIGINAL: if(properties["stealth"] >= properties["severity"] && properties["severity"] > 0)
		...
		stage_prob = max(0.5 * properties["stage_rate"], 1) // FLUFFY FRONTIER EDIT - ORIGINAL: stage_prob = max(0.3 * properties["stage_rate"], 1)
