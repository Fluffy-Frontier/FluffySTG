...
ADMIN_VERB_ONLY_CONTEXT_MENU(show_player_panel, R_ADMIN, "Show Player Panel", mob/player in world)
	...
	if(player.client && !user.is_eventmaker()) // TFF EDIT - Eventmaker
		...
		if(SSplayer_ranks.is_nova_star(player.client, admin_bypass = FALSE))
			player_ranks += "Veteran" // FLUFFY FRONTIER EDIT - ORIGINAL: player_ranks += "Nova Star"
	...
	if(player.client)
		...
		if(!user.is_eventmaker()) // TFF ADDITION - Eventmaker
			...
			body += "<A href='byond://?_src_=holder;[HrefToken()];mute=[player.ckey];mute_type=[MUTE_SIGNAL_WHISPER]'><font color='[(muted & MUTE_SIGNAL_WHISPER)?"red":"blue"]'>SIGNALWHISPER</font></a>\]"
