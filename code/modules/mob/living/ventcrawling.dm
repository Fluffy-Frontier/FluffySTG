...
/mob/living/proc/can_enter_vent(obj/machinery/atmospherics/components/ventcrawl_target, provide_feedback = TRUE)
	...
	if(!Adjacent(ventcrawl_target))
		return
	// TFF ADDITION START - VENTCRAWLING SIGNAL
	...
	if(!HAS_TRAIT(src, TRAIT_VENTCRAWLER_NUDE) && !HAS_TRAIT(src, TRAIT_VENTCRAWLER_ALWAYS) && !signal_result)
		return
	// TFF ADDITION END - VENTCRAWLING SIGNAL
	if(stat)
		if(provide_feedback)
	...
	if(buckled)
		...
		return
	if(iscarbon(src) && required_nudity && !signal_result) // TFF CHANGE - VENTCRAWLING SIGNAL
...
/mob/living/proc/handle_ventcrawl(obj/machinery/atmospherics/components/ventcrawl_target)
	...
	if(HAS_TRAIT(src, TRAIT_MOVE_VENTCRAWLING) && istype(loc, /obj/machinery/atmospherics) && movement_type & VENTCRAWLING)
		...
		if(has_client && isnull(client))
			return
		if(!do_after(src, exit_time, target = ventcrawl_target)) // TFF CHANGE - VENTCRAWLING SIGNAL
			SEND_SIGNAL(src, COMSIG_VENTCRAWL_PRE_CANCEL, ventcrawl_target) // TFF ADDITION - VENTCRAWLING SIGNAL
			return
		...
		update_pipe_vision()
		SEND_SIGNAL(src, COMSIG_VENTCRAWL_EXIT, ventcrawl_target) // TFF ADDITION - VENTCRAWLING SIGNAL
	...
	//Entrance here
		...
		if(vent_parent && (vent_parent.members.len || vent_parent.other_atmos_machines))
			...
			if(!do_after(src, enter_time, target = ventcrawl_target, extra_checks = CALLBACK(src, PROC_REF(can_enter_vent), ventcrawl_target))) // TFF CHANGE - VENTCRAWLING SIGNAL
				SEND_SIGNAL(src, COMSIG_VENTCRAWL_PRE_CANCEL, ventcrawl_target) // TFF ADDITION - VENTCRAWLING SIGNAL
			if(has_client && isnull(client))
				return
			if(ventcrawl_target.welded) // in case it got welded during our sleep
			...
			SEND_SIGNAL(src, COMSIG_VENTCRAWL_ENTER, ventcrawl_target) // TFF ADDITION - VENTCRAWLING SIGNAL
		else
