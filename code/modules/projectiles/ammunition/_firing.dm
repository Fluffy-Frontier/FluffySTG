...
/obj/item/ammo_casing/proc/ready_proj(atom/target, mob/living/user, quiet, zone_override = "", atom/fired_from)
	...
	if(isgun(fired_from))
		...
		loaded_projectile.damage *= gun.projectile_damage_multiplier * integrity_mult  + (gun.battery_damage_multiplier - 1)
		loaded_projectile.stamina *= gun.projectile_damage_multiplier * integrity_mult + (gun.battery_damage_multiplier - 1)
