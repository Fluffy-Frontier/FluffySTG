...
/datum/artifact_effect/cellcharge/DoEffectTouch(mob/user)
	...
	if(!.)
		return
	for(var/obj/item/stock_parts/cell/D in user.contents)
		D.give(1e5)
		if(issilicon(user))
...
/datum/artifact_effect/cellcharge/proc/try_give_charge(atom/reciever_atmon, power)
	if(istype(reciever_atmon, /obj/item/stock_parts/cell))
		var/obj/item/stock_parts/cell/C = reciever_atmon
		C.give(power)
	if(istype(reciever_atmon, /obj/machinery/power/apc))
		for(var/obj/item/stock_parts/cell/C in reciever_atmon.contents)
			C.give(power)
	if(istype(reciever_atmon, /obj/machinery/power/smes))
		var/obj/machinery/power/smes/lucky = reciever_atmon
		lucky.charge += power
	...
	if(istype(reciever_atmon, /obj/item/mod/control))
		...
		for(var/obj/item/mod/core/luckycore in luckymod.contents)
			for(var/obj/item/stock_parts/cell/C in luckycore.contents)
				C.give(power)
