...
/obj/machinery/computer/cargo/express/ghost/interdyne/ui_act(action, params, datum/tgui/ui)
	if(action == "add") // if we're generating a supply order
		...
		if(!is_supply_pack || !istype(is_supply_pack)) //if we're ordering a company import pack, add a temp pack to the global supply packs list, and remove it
			...
			var/datum/supply_pack/temp_pack = new()
			...
			SSshuttle.supply_packs["[temp_pack.name]"] = temp_pack
			. = ..()
			SSshuttle.supply_packs -= "[temp_pack.name]"
			return
		else
