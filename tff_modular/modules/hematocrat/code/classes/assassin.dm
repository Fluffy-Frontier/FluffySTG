...
// Дэш - рывок, который больше напоминает телепорт. Можно увидеть у пешек средневековых пиратов. Изменяем код под невидимость, ибо дэш ее снимает багом.
...
/datum/action/cooldown/mob_cooldown/hematocrat_dash/Activate(atom/target_atom)
...
/datum/action/cooldown/mob_cooldown/hematocrat_dash/proc/dash_to(atom/dash_target)
