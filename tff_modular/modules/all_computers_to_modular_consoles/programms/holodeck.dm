...
/datum/computer_file/program/disk_binded/holodeck/on_install(datum/computer_file/source, obj/item/modular_computer/computer_installing)
	...
	if(!bottom_left)
		log_mapping("[src] at [AREACOORD(computer_installing)] has an invalid holodeck area.")
		if (fm)
			INVOKE_ASYNC(fm, PROC_REF(try_eject), null, TRUE)
		computer_installing.remove_file(source)
		return

	var/area/computer_area = get_area(computer_installing.physical)
	...
	linked.linked = computer_installing.physical
	var/area/my_area = get_area(computer_installing.physical)
	if(my_area)
		linked.energy_usage = my_area.energy_usage
	else
		linked.energy_usage = list(AREA_USAGE_LEN)

	RegisterSignal(computer_installing, COMSIG_ATOM_EMP_ACT, PROC_REF(emergency_shutdown))
	RegisterSignal(computer_installing, COMSIG_ATOM_EX_ACT, PROC_REF(emergency_shutdown))

	RegisterSignal(computer_installing, COMSIG_ATOM_EMAG_ACT, PROC_REF(emag_act))


	COOLDOWN_START(src, holodeck_cooldown, HOLODECK_CD)
	generate_program_list()
	INVOKE_ASYNC(src, PROC_REF(load_program), offline_program, TRUE)

/datum/computer_file/program/disk_binded/holodeck/Destroy()
	reset_to_default()
	if(linked)
		linked.linked = null
		linked.energy_usage = list(AREA_USAGE_LEN)
	if (computer && !QDELETED(computer))
		UnregisterSignal(computer, COMSIG_ATOM_EMP_ACT)
		UnregisterSignal(computer, COMSIG_ATOM_EX_ACT)
		UnregisterSignal(computer, COMSIG_ATOM_EMAG_ACT)
	return ..()

/datum/computer_file/program/disk_binded/holodeck/process_tick(seconds_per_tick)
	...
	if(!floorcheck()) //if any turfs in the floor of the holodeck are broken
		emergency_shutdown()
		damaged = TRUE
		computer.physical.visible_message("The holodeck overloads!")
		for(var/turf/holo_turf in linked)
			if(prob(30))
				do_sparks(2, 1, holo_turf)
	...
	for(var/obj/effect/holodeck_effect/holo_effect as anything in effects)
		holo_effect.tick()
	power_cell_use = (initial(power_cell_use) + spawned.len * 3 + effects.len * 5)

///finalizes objects in the spawned list
/datum/computer_file/program/disk_binded/holodeck/proc/finish_spawn()
...
/datum/computer_file/program/disk_binded/holodeck/proc/finalize_spawned(atom/holo_atom)
...
/datum/computer_file/program/disk_binded/holodeck/proc/load_program(map_id, force = FALSE, add_delay = TRUE)
	if (program == map_id)
		return

	if (!computer || !computer.enabled || computer.active_program != src)//load_program is called once with a timer (in toggle_power) we dont want this to load anything if its off
	...
	nerf(!(computer.obj_flags & EMAGGED))
	finish_spawn()

/datum/computer_file/program/disk_binded/holodeck/proc/clear_projection()
...
/datum/computer_file/program/disk_binded/holodeck/proc/derez(atom/movable/holo_atom, silent = TRUE, forced = FALSE)
	spawned -= holo_atom
	if(!holo_atom)
		return
	...
	if(!silent)
		holo_atom.visible_message(span_notice("[holo_atom] fades away!"))

	if(is_type_in_list(holo_atom.loc, special_locs))
		qdel(holo_atom.loc)
	qdel(holo_atom)

/datum/computer_file/program/disk_binded/holodeck/proc/emag_act(mob/user, obj/item/card/emag/emag_card, forced)
	SIGNAL_HANDLER
	...
	if(!LAZYLEN(emag_programs))
		if (user)
			to_chat(user, span_warning("[filedesc]: Program list update failed. Reason: already up to date!"))
		return FALSE
	playsound(computer, SFX_SPARKS, 75, TRUE)
	if (user)
	...
	nerf(!(computer.obj_flags & EMAGGED), FALSE)
	return TRUE
...
/datum/computer_file/program/disk_binded/holodeck/proc/reset_to_default()
...
/datum/computer_file/program/disk_binded/holodeck/proc/register_contents(obj/structure/closet/storage)
	SIGNAL_HANDLER
 ...
 */
/datum/computer_file/program/disk_binded/holodeck/proc/add_to_spawned(atom/holo_atom)
	spawned |= holo_atom
	if(!(computer.obj_flags & EMAGGED) && isitem(holo_atom))
		var/obj/item/to_be_nerfed = holo_atom
		to_be_nerfed.damtype = STAMINA
	finalize_spawned(holo_atom)

/datum/computer_file/program/disk_binded/holodeck/proc/remove_from_holo_lists(datum/to_remove, _forced)
	SIGNAL_HANDLER
	spawned -= to_remove
...
/datum/computer_file/program/disk_binded/holodeck/proc/emergency_shutdown()
...
/datum/computer_file/program/disk_binded/holodeck/on_start(mob/living/user)
	. = ..()
	if (!.)
		return
...
/datum/computer_file/program/disk_binded/holodeck/kill_program(mob/user)
	. = ..()
	if (!.)
		return
...
/datum/computer_file/program/disk_binded/holodeck/proc/floorcheck()
...
/obj/item/computer_console_disk/holodeck
	program = /datum/computer_file/program/disk_binded/holodeck
	light_color = LIGHT_COLOR_BLUE

/obj/machinery/modular_computer/preset/battery_less/console/holodeck
	name = "holodeck control console"
	desc = "A computer used to control a nearby holodeck."
	starting_programs = list(/datum/computer_file/program/disk_binded/holodeck)

/obj/machinery/modular_computer/preset/battery_less/console/holodeck/wrench_act_secondary(mob/user, obj/item/tool)
	// Please no disasembling me
	if (cpu && !HAS_TRAIT_FROM(src, TRAIT_MODPC_INTERACTING_WITH_FRAME, REF(user)))
		balloon_alert(user, "unable to disassemble this!")
		return ITEM_INTERACT_BLOCKING
	else
		return ..()

/obj/machinery/modular_computer/blob_act(obj/structure/blob/B)
	if (cpu)
		var/datum/computer_file/program/disk_binded/holodeck/hd = cpu.find_file_by_name("holodeck_admin")
		hd?.emergency_shutdown()
	return ..()
