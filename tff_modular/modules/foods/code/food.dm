...
/obj/item/food/christmas_ham/proc/make_processable()
    if(slice_type)
        AddElement(
            ...
            3 SECONDS,
            table_required = TRUE,
            screentip_verb = "Carve",
            sound_to_play = SFX_KNIFE_SLICE
        )
...
/obj/item/food/canned_red_caviar/proc/update_icon_state()
	if(servings_left <= 0)
