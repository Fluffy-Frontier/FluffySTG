...
/datum/status_effect/awed/tick(seconds_between_ticks)
	...
	if(HAS_TRAIT(owner, TRAIT_INCAPACITATED) || !COOLDOWN_FINISHED(src, awe_effect_cooldown))
		return
	...
	switch(rand(1, 5))
		...
		if(5)
			owner.face_atom(source_vampire)
			if(owner.body_position == STANDING_UP && owner.get_stamina_loss() <= (100 / 2))
				...
				owner.adjust_stamina_loss(rand(20, 40), 10)
