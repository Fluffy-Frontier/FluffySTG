...
/obj/machinery/camera/post_machine_initialize()
	...
	// Because... why not?
	if (!camera_enabled && is_station_level(z))
		triggerCameraAlarm()
...
/obj/machinery/camera/proc/start_processing()
...
/obj/machinery/camera/examine(mob/user)
	...
	if(panel_open)
		if (!cell && !has_mock_cell)
			...
			. += span_info("It looks stuck though. And probably can be removed only by disassembling the camera.")
