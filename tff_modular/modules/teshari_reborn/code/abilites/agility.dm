...
#define AGILITY_MODE_BELLOW "agility_mode_bellow"
...
/datum/action/cooldown/teshari/agility/Activate(atom/target)
	...
	var/list/modes = list(
		...
		AGILITY_MODE_BELLOW = image(icon = 'tff_modular/modules/teshari_reborn/icons/actions.dmi', icon_state = AGILITY_MODE_BELLOW),
	...
	switch(selected_mode)
		...
		if(AGILITY_MODE_ABOVE)
			...
			owner.layer = initial(owner.layer)
			owner.pass_flags += PASSTABLE
		if(AGILITY_MODE_BELLOW)
			update_button_state(AGILITY_MODE_BELLOW)
			owner.balloon_alert(owner, "Moving bellow!")
			if(!(owner.pass_flags & PASSTABLE))
				owner.pass_flags += PASSTABLE
