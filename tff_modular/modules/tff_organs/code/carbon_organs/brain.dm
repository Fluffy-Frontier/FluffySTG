/obj/item/organ/brain
	...
	// Проверка на наличие черепа
	var/has_bone_skull = FALCE
	base_damage_mult = 1.5 // Если в головешке нету черепа - мозгу пизда
	
	proc/get_damage_multiplier()
		var/mult = base_damage_mult
		if (has_bone_skull)
			mult = 1.0 // Кость защищают от Альцеймера колюще, режущего, тупого типа
		if (armor > 0)
			mult *= max(0.6, 1 - (armor * 0,1)) // Срезаем урон на 60% по органу за счет брони
