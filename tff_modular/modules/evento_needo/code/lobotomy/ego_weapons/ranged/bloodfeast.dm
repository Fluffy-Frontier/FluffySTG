...
/datum/component/bloodfeast/RegisterWithParent()
	if(isliving(parent))
		RegisterSignal(parent, list(COMSIG_LIVING_DEATH, COMSIG_QDELETING), PROC_REF(OnDeath))
...
/datum/component/bloodfeast/proc/ScanForBlood()
	...
	for(var/obj/effect/decal/cleanable/blood/B in view(T, absorb_range)) //will clean up any blood, but only heals from human blood
		playsound(get_turf(B), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nosferatu/bloodcollect.ogg', 5, 1)
