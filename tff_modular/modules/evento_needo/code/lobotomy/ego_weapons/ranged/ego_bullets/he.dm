...
/obj/projectile/ego_bullet/ego_harmony/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(ishuman(target))
...
/obj/projectile/ego_bullet/replica/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	if(!isliving(target))
		return
	var/mob/living/T = target
	var/mob/living/user = firer
	if(user.faction_check_atom(T))//player faction
