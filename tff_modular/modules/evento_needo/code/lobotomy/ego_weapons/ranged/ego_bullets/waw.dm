...
/obj/projectile/ego_bullet/ego_hatred/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(ishuman(target) && isliving(firer))
		var/mob/living/carbon/human/H = target
		var/mob/living/user = firer
		if(firer==target)
			return BULLET_ACT_BLOCK
		if(user.faction_check_atom(H)) // Our faction
			switch(damage_type)
				if(BRAIN)
					H.adjustSanityLoss(-10)
					H.apply_damage(-10, BRAIN)
				if(BRUTE)
					H.adjustBruteLoss(-5)
...
/obj/projectile/ego_bullet/ego_hatred/Initialize()
	...
	damage_type = pick(BRUTE, BURN, BRAIN, TOX)
...
/obj/projectile/ego_bullet/ego_magicbullet/abnormality/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(istype(target, /mob/living/simple_animal/hostile/der_freis_portal))
		...
		the_target.apply_damage(7, BURN)
	. = ..()
...
/obj/projectile/ego_bullet/ego_loyalty/iff/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(!ishuman(target))
		nodamage = FALSE
	else
		return
...
/obj/projectile/ego_bullet/ego_praetorian/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(!ishuman(target))
		nodamage = FALSE
	else
		return
...
/obj/projectile/beam/assonance/on_hit(atom/target, blocked, pierce_hit)
	...
	if(H.stat == DEAD || HAS_TRAIT(H, TRAIT_GODMODE))
		return
...
/obj/projectile/ego_bullet/ego_warring/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	var/obj/item/ego_weapon/ranged/warring/bow = fired_from
	var/mob/living/L = target
	if(!isliving(target))
		return
	if((L.stat == DEAD) || HAS_TRAIT(L, TRAIT_GODMODE))//if the target is dead or godmode
		return FALSE
...
/obj/projectile/ego_bullet/ego_warring2/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	var/mob/living/carbon/human/H = target
	var/mob/living/user = firer
	if(user.faction_check_atom(H))//player faction
...
/obj/projectile/ego_bullet/special_fellbullet/on_hit(atom/target, blocked = FALSE, pierce_hit, angle)
	. = ..()
	var/mob/living/carbon/human/H = target
	if(ishuman(H))
		if(H.stat == DEAD)
...
/obj/effect/fellcircle/proc/FireBullets(angle, damage)
	...
	for(var/i = 0, i < shotsleft, i++)
		sleep(0.5 SECONDS)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fluchschutze/fell_scatter2.ogg', 25, TRUE)
