...
/obj/projectile/despair_rapier/Initialize()
	. = ..()
	hitsound = "tff_modular/modules/evento_needo/sounds/Tegusounds/ego/rapier[pick(1,2)].ogg"
	animate(src, alpha = 255, time = 3)

/obj/projectile/despair_rapier/on_hit(atom/target, blocked, pierce_hit)
	if(!ishuman(target))
		return ..()
	...
	if(.) // Hit passed and damage applied
		if((old_stat < DEAD) && (H.stat >= DEAD))
			H.add_overlay(icon('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "despair_kill"))
...
/obj/projectile/despair_rapier/justice/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(!ishuman(target))
		nodamage = FALSE
	else
		return
...
/obj/projectile/melting_blob
	...
	icon_state = "slime"
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/meltinglove/ranged_hit.ogg'
	damage_type = BRUTE
...
/obj/projectile/melting_blob/prehit_pierce(atom/A)
	if(isliving(A) && isliving(firer))
		...
		var/mob/living/L = A
		if(mob_firer.faction_check_atom(L))
			return PROJECTILE_PIERCE_PHASE
	return ..()

/obj/projectile/melting_blob/on_hit(target, blocked, pierce_hit)
	if(isliving(target))
...
/obj/projectile/mountain_spit/reduce_range()
	for(var/mob/living/L in range(1, get_turf(src)))
		if(L.stat != DEAD && L != firer && !L.faction_check_atom(firer, FALSE))
			return Bump(L)
	..()
...
/obj/projectile/clown_throw/on_hit(target, blocked = FALSE, pierce_hit)
	if(!ishuman(target))
		return ..()
	var/mob/living/carbon/human/H = target
...
/obj/projectile/clown_throw_rcorp/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(ishuman(target))
		damage = 5
		nodamage = FALSE
		var/mob/living/carbon/human/H = target
		var/obj/item/bodypart/random_part = pick(H.bodyparts)
		random_part.adjustBleedStacks(5)
		H.add_movespeed_modifier(/datum/movespeed_modifier/clowned)
...
/obj/projectile/season_projectile/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	. = ..()
...
/obj/projectile/actor/on_hit(target, blocked = FALSE, pierce_hit)
	. = ..()
	if(!ishuman(target))
		return
	var/mob/living/carbon/human/H = target
...
/obj/projectile/beam/water_jet/on_hit(atom/target, blocked = FALSE, pierce_hit)
...
/obj/projectile/ego_bullet/abno_nihil
	...
	damage = 25 //Fires 4
	damage_type = BRAIN //deals both white and red
	projectile_piercing = PASSMOB
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nihil/filter.ogg'
...
/obj/projectile/lifestew_spit/Initialize()
	...
	switch(chansu)
		if(1)
			icon = 'tff_modular/modules/evento_needo/icons/Teguicons/projectiles.dmi'
...
/obj/projectile/bonebullet/on_hit(atom/target, blocked = FALSE, pierce_hit)
...
/obj/projectile/nosferatu_bat
	...
	icon_state = "bat"
	damage = 25
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nosferatu/bat_attack.ogg'
	var/mob/living/simple_animal/hostile/abnormality/nosferatu/owner = null

/obj/projectile/nosferatu_bat/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
