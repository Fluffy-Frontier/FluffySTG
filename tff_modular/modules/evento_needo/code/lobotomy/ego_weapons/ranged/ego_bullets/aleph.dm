...
/obj/projectile/ego_bullet/adoration/dot/on_hit(target, blocked = FALSE, pierce_hit)
	. = ..()
	...
	if(!isbot(H) && isliving(H) && !QDELETED(H))
		...
		for(var/i = 1 to 14)
			addtimer(CALLBACK(H, TYPE_PROC_REF(/mob/living, apply_damage), rand(4,8), BRUTE, null, H.run_armor_check(null, BRUTE)), 2 SECONDS * i)

/obj/projectile/ego_bullet/adoration/aoe
	color = "#6666BB"

/obj/projectile/ego_bullet/adoration/aoe/on_hit(target, blocked = FALSE, pierce_hit)
	. = ..()
		...
		if(user.faction_check_atom(H)) // Our faction
			switch(damage_type)
				if(BRAIN)
					...
					H.apply_damage(-damage*0.2, BRAIN)
				if(BRUTE)
