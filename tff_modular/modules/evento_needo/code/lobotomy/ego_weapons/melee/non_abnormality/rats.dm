...
/obj/item/ego_weapon/city/rats/brick/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	if(!..())
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/bricksmash.ogg', 50, TRUE)
		qdel(src)
...
/obj/item/ego_weapon/ranged/pistol/rats/afterattack(atom/target, mob/living/user, flag, params)
	if(flag && (user.combat_mode || forced_melee)) // Don't want to take damage when just using melee
		return ..()
	if(prob(50))
		to_chat(user,span_warning("You pinch your fingers in the weapon."))
		user.apply_damage(5, BRUTE, null, user.run_armor_check(null, BRUTE))
		return FALSE
	return ..()
