...
/obj/item/ego_weapon/black_silence_gloves
	...
	icon_state = "gloves"
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/black_silence_weapons.dmi'
	lefthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/black_silence_lefthand.dmi'
	righthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/black_silence_righthand.dmi'
	force = 1
	damtype = BRUTE
...
/obj/item/ego_weapon/black_silence_gloves/equipped(mob/user, slot)
	...
	if(!user)
		return
	RegisterSignal(user, COMSIG_SHIFT_CLICKED_ON, PROC_REF(DoChecks))

/obj/item/ego_weapon/black_silence_gloves/Destroy(mob/user)
	UnregisterSignal(user, COMSIG_SHIFT_CLICKED_ON)
	return ..()
...
/datum/action/item_action/toggle_iff
	...
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/black_silence_weapons.dmi'
	button_icon_state = "gloves"

/datum/action/item_action/toggle_iff/Trigger(trigger_flags, atom/target)
...
/obj/item/ego_weapon/black_silence_gloves/proc/exchange_armaments(mob/user)
	...
	if(selected_armament) // I'm sorry if this is confusing, it checks if you've already used this weapon and transfers variables
		...
		if(!(unlocked) && Y.unlocked_list.len > 8)
			playsound(playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/unlock.ogg', 100, 1))
			to_chat(user,span_userdanger("You are ready to unleash Furioso!"))
			Y.unlocked = TRUE
		if(unlocked)
			Y.unlocked = TRUE
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/snap.ogg', 50, 1)
		else
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/snap.ogg', 50, 1)
...
/obj/item/ego_weapon/black_silence_gloves/ranga/proc/dash_attack(mob/living/user, atom/target)
	...
	L.apply_damage(80, BRUTE, null, L.run_armor_check(null, BRUTE))
	exchange_cooldown -= 20
	switch(dash_count)
		if(0)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/mace.ogg', 80, 1)
			dash_count += 1
			addtimer(CALLBACK(src, PROC_REF(dash_attack), user, target), 3)
		if(1)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/axe.ogg', 80, 1)
			dash_count += 1
			addtimer(CALLBACK(src, PROC_REF(dash_attack), user, target), 3)
		if(2)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/shortsword.ogg', 90, 1)
			dash_count = 0
...
/obj/item/ego_weapon/black_silence_gloves/ranga/attack(mob/living/M, mob/living/user)
	...
	switch(sfx)
		...
		if(2)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/axe.ogg'
		if(3)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/shortsword.ogg'
	..()
	special_cooldown -= 10
...
/obj/item/ego_weapon/black_silence_gloves/old_boys/Special(mob/living/user, atom/target)
	if (block == 0)
		...
		block = TRUE
		block_success = FALSE
		shield_user.physiology.burn_mod *= max(0.001, (1 - ((reductions[1]) / 100)))
		shield_user.physiology.brain_mod *= max(0.001, (1 - ((reductions[2]) / 100)))
		shield_user.physiology.brute_mod *= max(0.001, (1 - ((reductions[3]) / 100)))
		shield_user.physiology.tox_mod *= max(0.001, (1 - ((reductions[4]) / 100)))
		RegisterSignal(user, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(AnnounceBlock))
...
/obj/item/ego_weapon/black_silence_gloves/old_boys/proc/DisableBlock(mob/living/carbon/human/user)
	user.physiology.burn_mod /= max(0.001, (1 - ((reductions[1]) / 100)))
	user.physiology.brain_mod /= max(0.001, (1 - ((reductions[2]) / 100)))
	user.physiology.brute_mod /= max(0.001, (1 - ((reductions[3]) / 100)))
	user.physiology.tox_mod /= max(0.001, (1 - ((reductions[4]) / 100)))
...
/obj/item/ego_weapon/black_silence_gloves/old_boys/proc/AnnounceBlock(mob/living/carbon/human/source, damage, damagetype, def_zone)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/guard.ogg', 50, 0, 7)
...
/obj/item/ego_weapon/black_silence_gloves/old_boys/attack(mob/living/M, mob/living/user)
	if(parry_buff)
		force = 130
		hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/greatsword.ogg'
		exchange_cooldown -= 50
	..()
	if(parry_buff)
...
/obj/item/ego_weapon/black_silence_gloves/allas/Special(mob/living/user, atom/target)
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/duelsword_strong.ogg'
...
/obj/item/ego_weapon/black_silence_gloves/mook/Special(mob/living/user, atom/target)
	...
	if(stacks > 0)
		stacks -= 1
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/longsword_start.ogg', 50, 1)
		sleep(3)
		var/turf/T = get_turf(target)
		playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/longsword_atk.ogg', 50, 1)
		for (var/i = 0; i < 3; i++)
			new /obj/effect/temp_visual/smash_effect(T)
			for(var/mob/living/L in user.HurtInTurf(T, list(), 50, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
				exchange_cooldown -= 10
			sleep(0.25 SECONDS)
...
/obj/item/ego_weapon/black_silence_gloves/logic/Special(mob/living/user, atom/target)
	...
	switch(dir_to_target)
		if(EAST)
			middle_line = get_line(get_step(source_turf, EAST), get_ranged_target_turf(source_turf, EAST, aoe_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, aoe_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, aoe_width)))
					if (U.density)
						break
					...
					area_of_effect += U
		if(WEST)
			middle_line = get_line(get_step(source_turf, WEST), get_ranged_target_turf(source_turf, WEST, aoe_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, aoe_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, aoe_width)))
					...
					area_of_effect += U
		if(SOUTH)
			middle_line = get_line(get_step(source_turf, SOUTH), get_ranged_target_turf(source_turf, SOUTH, aoe_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, aoe_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, aoe_width)))
					if (U.density)
						break
					if (U in area_of_effect)
						continue
					area_of_effect += U
		if(NORTH)
			middle_line = get_line(get_step(source_turf, NORTH), get_ranged_target_turf(source_turf, NORTH, aoe_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, aoe_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, aoe_width)))
					...
					if (U in area_of_effect)
						continue
					area_of_effect += U
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/shotgun.ogg', 75, 1)
	var/list/been_hit = list()
	var/atom/throw_target = get_edge_target_turf(target, user.dir)
	for(var/turf/T in area_of_effect)
		new /obj/effect/temp_visual/smash_effect(T)
		var/list/new_hits = user.HurtInTurf(T, been_hit, 100, BRUTE, check_faction = TRUE, hurt_mechs = TRUE) - been_hit
		been_hit += new_hits
...
/obj/item/ego_weapon/black_silence_gloves/logic/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag && gun_cooldown <= world.time)
		...
		G.fire()
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/revolver.ogg', 60, 1)
		gun_cooldown = world.time + gun_cooldown_time
		return

/obj/item/ego_weapon/black_silence_gloves/logic/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
...
/obj/projectile/ego_bullet/atelier_logic/iff/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(!ishuman(target))
		nodamage = FALSE
	else
		return
...
/obj/item/ego_weapon/black_silence_gloves/durandal/attack(mob/living/M, mob/living/user)
	...
	if(finisher)
		...
		hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/durandal_strong.ogg'
...
/obj/item/ego_weapon/black_silence_gloves/crystal/Special(mob/living/user, atom/target)
	...
	if(evade_time > world.time)
		...
		for(var/turf/T in get_line(user, get_ranged_target_turf_direct(user, target, evade_range)))
			if(T.density)
				break
			target_turf = T
			line_turfs += T
		...
		for(var/i = 1 to line_turfs.len)
			...
			D.alpha = min(150 + i*15, 255)
			animate(D, alpha = 0, time = 2 + i*2)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/evasion.ogg', 50, 1)
...
/obj/item/ego_weapon/black_silence_gloves/crystal/proc/dash_attack(mob/living/user, atom/target)
	...
	var/turf/F = get_turf(L)
	dash(user, target_turf)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/duelsword.ogg', 50, 1)
	if(dash_count < 1)
		...
		L.apply_damage(100, BRUTE, null, L.run_armor_check(null, BRUTE))
		new /obj/effect/temp_visual/smash_effect(F)
		exchange_cooldown -= 30
		dash_count = 0
...
/obj/item/ego_weapon/black_silence_gloves/wheels/Special(mob/living/user, atom/target)
	...
	if(do_after(user, 10, target))
		...
		switch(dir_to_target)
			if(EAST)
				middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, EAST, aoe_length))
				for(var/turf/T in middle_line)
					if(T.density)
						break
					for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, aoe_width)))
					...
					for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, aoe_width)))
						...
						area_of_effect += U
			if(WEST)
				middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, WEST, aoe_length))
				for(var/turf/T in middle_line)
					if(T.density)
						break
					for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, aoe_width)))
					...
					for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, aoe_width)))
						...
						area_of_effect += U
			if(SOUTH)
				middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, SOUTH, aoe_length))
				for(var/turf/T in middle_line)
					if(T.density)
						break
					for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, aoe_width)))
					...
					for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, aoe_width)))
						...
						area_of_effect += U
			if(NORTH)
				middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, NORTH, aoe_length))
				for(var/turf/T in middle_line)
					if(T.density)
						break
					for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, aoe_width)))
					...
					for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, aoe_width)))
						...
						if (U in area_of_effect)
							continue
						area_of_effect += U
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/greatsword.ogg', 75, 1)
		var/list/been_hit = list()
		for(var/turf/T in area_of_effect)
			new /obj/effect/temp_visual/smash_effect(T)
			var/list/new_hits = user.HurtInTurf(T, been_hit, 300, BRUTE, check_faction = TRUE, hurt_mechs = TRUE) - been_hit
			been_hit += new_hits
			for(var/mob/living/L in new_hits)
...
/obj/item/ego_weapon/black_silence_gloves/proc/furioso(mob/living/user, mob/living/target)
	...
	for(i = 0, i < 2, i++)
		...
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/revolver.ogg', 100, 1)
		sleep(3.5)
	//Spear
	icon_state = "allas"
	target_turf = get_step(get_turf(target), get_dir(user, target))
	...
	T = get_turf(target)
	new /obj/effect/temp_visual/smash_effect(T)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/duelsword_strong.ogg', 100, 1)
	sleep(2)

	//Hammer
	icon_state = "old_boys"
	user.dir = get_dir(user, target)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/mace.ogg', 100, 1)
	target_turf = get_step(get_turf(target), get_dir(user, target))
	new /obj/effect/temp_visual/smash_effect(T)
	if(!target.anchored)
		target.Move(target_turf)
	...
	//LongSword
	icon_state = "mook"
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/longsword_start.ogg', 100, 1)
	sleep(1.5)
	T = get_turf(target)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/longsword_atk.ogg', 100, 1)
	for (i = 0; i < 3; i++)
		new /obj/effect/temp_visual/smash_effect(T)
		sleep(1.25)
	...
	for(i = 0, i <= 2, i++)
		...
		if(i == 0)
			...
			sleep(1)
		if(i == 1)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/axe.ogg', 100, 1)
			sleep(1)
		if(i == 2)
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/shortsword.ogg', 100, 1)
			sleep(3)
	//Mace & Axe
	icon_state = "zelkova"
	user.dir = get_dir(user, target)
	...
	icon_state = "wheels"
	target_turf = get_step(get_turf(target), get_dir(user, target))
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/greatsword.ogg', 100, 1)
	new /obj/effect/temp_visual/smash_effect(T)
	if(!target.anchored)
		target.Move(target_turf)
	sleep(5)
	...
	user.dir = get_dir(user, target)
	new /obj/effect/temp_visual/smash_effect(T)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/shotgun.ogg', 100, 1)
	target_turf = get_step(get_turf(target), get_dir(user, target))
	...
	dash(user, target_turf)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/durandal_down.ogg', 100, 1)
	T = get_turf(target)
	new /obj/effect/temp_visual/smash_effect(T)
	sleep(3.5)
	target_turf = get_step(get_turf(target), get_dir(user, target))
	dash(user, target_turf)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/durandal_up.ogg', 100, 1)
	new /obj/effect/temp_visual/smash_effect(T)
	sleep(3.5)
	user.dir = get_dir(user, target)
	target_turf = get_step(get_turf(target), get_dir(user, target))
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/durandal_strong.ogg', 100, 1)
	T = get_turf(target)
	new /obj/effect/temp_visual/smash_effect(T)
	if(!target.anchored)
		target.Move(target_turf)
	L.apply_damage(1500, BRUTE, null, L.run_armor_check(null, BRUTE)) //this went on for 5 sec, so 300 DPS as the final attack
	sleep(10)
...
/obj/item/ego_weapon/black_silence_gloves/proc/furioso_start(mob/living/user, mob/living/target)
	...
	ADD_TRAIT(user, TRAIT_GODMODE, ADMIN_TRAIT)
...
/obj/item/ego_weapon/black_silence_gloves/proc/furioso_end(mob/living/user, mob/living/target)
	REMOVE_TRAIT(user, TRAIT_GODMODE, ADMIN_TRAIT)
