...
/obj/projectile/ego_bullet/lcorp/fire(angle, atom/direct_target)
	if(fired_from)
		if(istype(fired_from, /obj/item/ego_weapon/ranged/city/lcorp))
			...
			if(damage_type == BRUTE) //pale deals 25% less damage
...
/obj/item/ego_weapon/ranged/city/lcorp
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/lcorp_weapons.dmi'
	lefthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/lcorp_left.dmi'
	righthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/lcorp_right.dmi'
...
/obj/item/ego_weapon/ranged/clerk/handle_suicide(mob/living/carbon/human/user, mob/living/carbon/human/target, params, bypass_timer)
	...
	semicd = TRUE
	if(!bypass_timer && (!do_after(user, user_target ? 3 SECONDS : 12 SECONDS, target) || user.zone_selected != BODY_ZONE_PRECISE_MOUTH))
		if(user)
			if(user == target)
