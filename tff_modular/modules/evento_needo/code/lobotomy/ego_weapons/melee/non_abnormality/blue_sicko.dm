...
/obj/item/ego_weapon/city/reverberation/attack(mob/living/target, mob/living/user)
	...
	if(S)
		if(S.stacks == vibration)
			...
			force = 90
			hitsound = "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/reverb_strong[rand(1, 2)].ogg"
...
/obj/item/ego_weapon/city/reverberation/proc/TempestuousDanza(mob/living/user)
	...
	for(var/mob/living/L in range(8, user))
		...
		playsound(user, "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/reverb_far[rand(1,2)].ogg", 50)
		src.attack(L, user)
...
/obj/item/ego_weapon/city/reverberation/proc/GrandFinale(mob/living/user)
	...
	playsound(user, "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/reverb_grand_start.ogg", 70, extrarange = 8)
	sleep(3)
	...
	for(var/mob/living/L in range(12, user))
		...
		user.forceMove(tp_loc)
		to_hit += L
		playsound(user, "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/reverb_grand_dash.ogg", 50)
		prev_loc.Beam(tp_loc, "sm_arc_supercharged", time=25)
		sleep(2)
	if(!hit_target)
		var/datum/action/item_action/charging/tempestuous/T = locate() in actions
	...
	playsound(user, "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/reverb_grand_end.ogg", 70, extrarange = 8)
	for(var/mob/living/L in to_hit)
		...
		if(isanimal(L))
			damage *= 1.5
		L.apply_damage(damage, BRUTE, null, L.run_armor_check(null, BRUTE))
...
/obj/effect/temp_visual/reverb_slash
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/lc13_effects.dmi'
...
/datum/action/item_action/charging/Trigger(trigger_flags, atom/target)
	. = ..()
	if(.)
...
/datum/action/item_action/charging/IsAvailable(feedback)
