...
/obj/item/ego_weapon/grinder/attack(mob/living/target, mob/living/user)
	...
	if(!.)
		return FALSE
	//damage calculations
	var/justicemod = 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
	var/damage_dealt = force * justicemod
	var/list/been_hit = QDELETED(target) ? list() : list(target)
	user.HurtInTurf(T, been_hit, damage_dealt, BRUTE, hurt_mechs = TRUE, hurt_structure = TRUE)
...
/obj/item/ego_weapon/harvest
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/harvest.ogg'
	var/can_spin = TRUE
	var/spinning = FALSE
...
/obj/item/ego_weapon/harvest/attack_self(mob/living/carbon/human/user)
	if(!CanUseEgo(user))
		return
	if(!can_spin)
	...
	if(do_after(user, 12, src))
		can_spin = TRUE
		addtimer(CALLBACK(src, PROC_REF(spin_reset)), 12)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/harvest.ogg', 75, FALSE, 4)
...
/obj/item/ego_weapon/paw/melee_attack_chain(mob/user, atom/target, params)
	..()
	hitsound = "sound/items/weapons/punch[pick(1,2,3,4)].ogg"
...
/obj/item/ego_weapon/shield/daredevil
	...
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/crumbling_parry.ogg'
...
/obj/item/ego_weapon/logging
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_attack.ogg'
...
/obj/item/ego_weapon/logging/proc/Smash(mob/living/carbon/human/user, atom/target)
	...
	switch(dir_to_target)
		if(EAST)
			middle_line = get_line(get_step(source_turf, EAST), get_ranged_target_turf(source_turf, EAST, flurry_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, flurry_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, flurry_width)))
					if (U.density)
						break
					...
					area_of_effect += U
		if(WEST)
			middle_line = get_line(get_step(source_turf, WEST), get_ranged_target_turf(source_turf, WEST, flurry_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, flurry_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, flurry_width)))
					...
					area_of_effect += U
		if(SOUTH)
			middle_line = get_line(get_step(source_turf, SOUTH), get_ranged_target_turf(source_turf, SOUTH, flurry_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, flurry_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, flurry_width)))
					if (U.density)
						break
					if (U in area_of_effect)
						continue
					area_of_effect += U
		if(NORTH)
			middle_line = get_line(get_step(source_turf, NORTH), get_ranged_target_turf(source_turf, NORTH, flurry_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, flurry_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, flurry_width)))
					...
					area_of_effect += U
	smashing = TRUE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_prepare.ogg', 50, 0, 3)
	for(var/i = 0; i < 3; i++)
		var/list/been_hit = list()
		for(var/turf/T in area_of_effect)
			new /obj/effect/temp_visual/smash_effect(T)
			var/smash_damage = (i > 2 ? 40 : 10)*(1 + user.get_clothing_class_level(CLOTHING_ARMORED))
			been_hit = user.HurtInTurf(T, been_hit, smash_damage, BRUTE)
		if (i > 2)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_strong.ogg', 75, 0, 5) // BAM
		else
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_attack.ogg', 50, 0, 2)
		sleep(0.25 SECONDS)
	smashing = FALSE
	return
...
/obj/item/ego_weapon/shield/bravery
	...
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scaredycat/cateleport.ogg'
...
/obj/item/ego_weapon/shield/bravery/attack_self(mob/user)
	...
	if(!friend_count && icon_state == "bravery")
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scaredycat/catchange.ogg', 25, FALSE, 4)
	else if(friend_count && icon_state == "bravery_broken")
		to_chat(user, "<span class='nicegreen'>Your weapon puffs back up to impress your allies!")
		icon_state = "bravery"
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scaredycat/catgrunt.ogg', 50, FALSE, 4)
...
/obj/item/ego_weapon/pleasure/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(M == user && !happy && istype(user))
		...
		var/justice_mod = 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
		...
		playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_giggle.ogg', 50, FALSE, 4)
		playsound(H, 'sound/items/weapons/bladeslice.ogg', 50, FALSE, 4)
...
/obj/item/ego_weapon/pleasure/proc/Withdrawal(mob/living/M, mob/living/user)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_giggle.ogg', 50, FALSE, 4)
...
/obj/item/ego_weapon/revelation
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/da_capo2.ogg'
...
/obj/item/ego_weapon/inheritance
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/rapier1.ogg'
...
/obj/item/ego_weapon/shield/legerdemain
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Legerdemain.ogg'
	reductions = list(30, 20, 30, 0) // 80
	projectile_block_duration = 1 SECONDS
	block_duration = 1 SECONDS
	block_cooldown = 3 SECONDS
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Attack2.ogg'
...
/obj/item/ego_weapon/shield/legerdemain/attack_self(mob/user)//FIXME: Find a better way to use this override!
	if(block == 0) //Extra check because shields returns nothing on 1
		if(..())
			...
			RegisterSignal(user, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(NoParry), override = TRUE)//728 and 729 must be able to unregister the signal of 730
			return TRUE
		else
			return FALSE
...
/obj/item/ego_weapon/shield/legerdemain/proc/NoParry(mob/living/carbon/human/user, obj/item/L)//Disables AnnounceBlock when attacked by an item or a human
	SIGNAL_HANDLER
	UnregisterSignal(user, COMSIG_MOB_APPLY_DAMAGE)//y'all can't behave
...
/obj/item/ego_weapon/get_strong
	...
	var/list/mode_stats = list(
		"Spear" = list("_sp", 42, 1, 2, list("pokes", "jabs"), list("poke", "jab"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/spear1.ogg'),	//Now immobilizes you.
		"Sword" = list("_sw", 22, 1, 1, list("slashes", "slices"), list("slash", "slice"), 'sound/items/weapons/bladeslice.ogg'),
		"Gauntlet" = list("_f", 50, 3, 1, list("crushes", "smashes"), list("crush", "smash"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/strong_gauntlet.ogg')
		)
	var/windup = 0
...
/obj/item/ego_weapon/get_strong/attack(mob/living/target, mob/living/carbon/human/user)
	...
	switch(windup)
		if(50 to INFINITY)
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/strong_charged2.ogg', 60)
			to_chat(user, span_nicegreen("[src] beeps and whirls as it reaches full capacity!"))
		if(25 to 49)
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/strong_charged1.ogg', 40)
		else
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/strong_uncharged.ogg', 20)
	return
...
/obj/item/ego_weapon/impending_day/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(target.stat == DEAD && living)
		if(!sacrifice)
			...
			playsound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/doomsdaycalendar/Doomsday_Attack.ogg', 3, TRUE)
			sacrifice = FALSE
		for(var/mob/living/L in range(1, target))
			if(L == user || ishuman(L))
				continue
			L.apply_damage(50, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/obj/item/ego_weapon/fluid_sac
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hammer1.ogg'
...
//		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/jump.ogg', 50, FALSE, -1)
...
/obj/item/ego_weapon/fluid_sac/proc/JumpAttack(atom/A, mob/living/user, proximity_flag, params)
	...
	for(var/mob/living/L in range(1, A))
		...
		if(L == user || ishuman(L))
			continue
		L.apply_damage(25, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/obj/item/ego_weapon/sanguine
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Attack.ogg'
...
/obj/item/ego_weapon/replica
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kqe/hitsound2.ogg'
...
/obj/item/ego_weapon/replica/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag && gun_cooldown <= world.time)
		...
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kqe/load3.ogg', 100, TRUE)
		G.firer = user
		G.set_angle(get_angle(user, target))
		G.fire()
		gun_cooldown = world.time + gun_cooldown_time

/obj/item/ego_weapon/replica/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
	if(!isliving(target))
		return TRUE
...
/obj/item/ego_weapon/warp
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/attack1.ogg'
	reach = 2
	stuntime = 5	//Longer reach, gives you a short stun.
	damtype = BRUTE
	attack_verb_continuous = list("stabs", "slashes", "attacks")
	attack_verb_simple = list("stab", "slash", "attack")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/attack2.ogg'
...
/obj/item/ego_weapon/warp/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag)
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/teleport2.ogg', 50, TRUE)
		P1.link_portal(P2)
		P2.link_portal(P1)
		P1.teleport(user)
...
//	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/teleport2.ogg', 50, TRUE)//doesn't work
//	..()
...
/obj/item/ego_weapon/marionette
	...
	attack_verb_continuous = list("slices", "cleaves", "chops")
	attack_verb_simple = list("slice", "cleave", "chop")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pinocchio/attack.ogg'
...
/obj/item/ego_weapon/divinity
	...
	attack_verb_continuous = list("stabs", "slashes", "attacks")
	attack_verb_simple = list("stab", "slash", "attack")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade2.ogg'
...
/obj/item/ego_weapon/divinity/proc/DoLineAttack(list/line, atom/target, mob/living/carbon/human/user)
	...
	for(var/turf/T in line)
		if(locate(/obj/effect/temp_visual/smash_effect) in T)
			continue
		playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade3.ogg', 30, TRUE, 3)
		new /obj/effect/temp_visual/smash_effect(T)
		been_hit = user.HurtInTurf(T, been_hit, punishment_damage, BRUTE, check_faction = TRUE)
...
/obj/item/ego_weapon/destiny
	...
	attack_verb_continuous = list("stabs", "slashes", "attacks")
	attack_verb_simple = list("stab", "slash", "attack")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fateloom/garrote_bloody.ogg'//it's a bit loud
...
/obj/item/ego_weapon/rhythm
	...
	attack_verb_continuous = list("slices", "saws", "rips")
	attack_verb_simple = list("slice", "saw", "rip")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/singingmachine/crunch.ogg'
...
/obj/item/ego_weapon/rhythm/attack_self(mob/living/carbon/human/user)
	if(do_after(user, 10, src))	//Just a second to heal people around you, but it also harms you
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/singingmachine/music.ogg', 100, FALSE, 9)
...
/obj/item/ego_weapon/roseate/attack(mob/living/M, mob/living/user)//negative temperance multiplier
	force = 35
	var/temperancemod = 1 + user.get_clothing_class_level(CLOTHING_SERVICE)
	force /= temperancemod
	..()
	force = initial(force)
...
/obj/item/ego_weapon/aedd/proc/power_attack(mob/living/target, mob/living/user)
	var/justicemod = 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
	target.apply_damage((force * justicemod), BRUTE, null, target.run_armor_check(null, BRUTE), spread_damage = TRUE)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_charge.ogg', 50, TRUE)
	var/turf/T = get_turf(target)
...
/obj/item/ego_weapon/lance/lifestew_lance
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/fire1.ogg'
...
/obj/item/ego_weapon/lance/lifestew_lance/proc/SoupBlast(mob/living/carbon/human/user, mob/target)
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bloodbath/Bloodbath_EyeOn.ogg', 80, TRUE, -3) //yes im reusing a sound bite me
	for(var/turf/T in view(1, target))
		var/obj/effect/temp_visual/small_smoke/halfsecond/FX =  new(T)
		FX.color = "#622F22"
		user.HurtInTurf(T, list(), 40, BRUTE, check_faction = TRUE)
	return
...
/obj/item/ego_weapon/lifestew
	...
	attack_verb_continuous = list("burns", "boils")
	attack_verb_simple = list("burn", "boil")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/fire2.ogg'
...
/obj/item/ego_weapon/lifestew/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
	return TRUE
...
/obj/projectile/ego_bullet/lifestew
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/jump.ogg'
	damage = 35
	damage_type = BRUTE
...
/obj/item/ego_weapon/faelantern
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/sword1.ogg'
...
/obj/item/ego_weapon/faelantern/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag)
		...
		G.fired_from = src //for signal check
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/orangetree/ding.ogg', 100, TRUE) //yes im reusing a sound bite me
		G.firer = user
		G.set_angle(get_angle(user, target))
		G.fire()
		firing_cooldown = firing_cooldown_time + world.time
...
/obj/item/ego_weapon/faelantern/proc/on_projectile_hit(atom/fired_from, mob/living/carbon/human/firer, atom/target, Angle)
	SIGNAL_HANDLER
	if(isliving(target))
...
/obj/projectile/ego_bullet/faelantern
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/orangetree/ding.ogg'
	damage = 25
	damage_type = BRUTE


/obj/projectile/ego_bullet/faelantern/on_hit(target, blocked, pierce_hit)
	. = ..()
...
/datum/status_effect/fairybite/tick()
	owner.apply_damage(damage_amount, BRUTE, null, owner.run_armor_check(null, BRUTE), spread_damage = TRUE)
	playsound(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/bite.ogg', 70, TRUE) //yes im reusing a sound bite me
...
/obj/item/ego_weapon/lance/brick_road/UserMoved(mob/user)
	..()
	if(!raised)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_MakeRoad.ogg', 100, FALSE, 8)
...
/datum/status_effect/brown_bricks/on_apply()
	...
	M.set_varspeed(M.speed + 0.25)
	addtimer(CALLBACK(M, TYPE_PROC_REF(/mob/living/simple_animal/hostile, set_varspeed), M.speed - 0.25), 3 SECONDS)
...
/obj/item/ego_weapon/mini/voodoo/attack(mob/living/target, mob/living/user)
	...
	if(combo)
		...
		if(target in view(reach,user))
			...
			log_combat(user, target, pick(attack_verb_continuous), Y.name, "(COMBAT_MODE: [uppertext(user.combat_mode)]) (DAMTYPE: [uppertext(Y.damtype)])")
...
/obj/item/ego_weapon/nixie
	...
	attack_verb_continuous = list("bashes", "clubs")
	attack_verb_simple = list("bashes", "clubs")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/baton2.ogg'
...
/obj/item/ego_weapon/sunshower/attack(mob/living/target, mob/living/carbon/human/user)
	...
	switch(combo) //Weird combo where the later attacks deal more damage
		if(1)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/sunshower1.ogg'
			force *= 1.2
			user.changeNext_move(CLICK_CD_MELEE * 1.8)
		if(2)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/sunshower2.ogg'
			force *= 1.8
			user.changeNext_move(CLICK_CD_MELEE * 1.2)
		if(3)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/sunshower3.ogg'
			combo = 0
			force *= 2
...
/obj/item/ego_weapon/uturn/attack(mob/living/target, mob/living/user)
	...
	for(var/turf/T in get_line(user, end_turf))
		if(user in T)
			continue
		for(var/turf/T2 in view(T,1))
			...
			for(var/mob/living/L in T2)
				...
				if(L == user || ishuman(L))
					continue
				been_hit = user.HurtInTurf(T2, been_hit, aoe, BRUTE, hurt_mechs = TRUE, hurt_structure = TRUE)
...
/obj/item/ego_weapon/telepole
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alleywaywatchdog/telepole_1.ogg'
...
/obj/item/ego_weapon/telepole/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alleywaywatchdog/telepole_2.ogg', 100, 1)
	currently_charging = FALSE
...
/obj/item/ego_weapon/telepole/proc/Leap(mob/living/user, dir = SOUTH, leap_range)//doesn't work
	...
	for(var/turf/T in orange(1, user))
		hit_turfs |= T
	if(end_leap)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alleywaywatchdog/telepole_3.ogg', 100, 1)
...
/obj/item/ego_weapon/hexnail/melee_attack_chain(mob/user, atom/target, params)
	..()
	hitsound = "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/knife[pick(1,2,3,4)].ogg"
...
/obj/item/ego_weapon/desert/click_alt(mob/user)
	...
	switch(chain)
		if(1)
			if(activated) //H - Drop Kick attack
				...
				stuntime = 20
				force *= 3
				hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/oldboys.ogg'
				user.Knockdown(10)
...
/obj/item/ego_weapon/desert/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if((get_dist(user, A) < 2))
		A.attackby(src,user)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge.ogg', 50, FALSE, 9)
