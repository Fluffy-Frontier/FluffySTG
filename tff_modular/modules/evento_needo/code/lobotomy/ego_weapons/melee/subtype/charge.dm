...
/obj/item/ego_weapon/attack(mob/living/target_mob, mob/living/user, list/modifiers, list/attack_modifiers)
	. = ..()
	if(!.)
		return FALSE

	if(currently_charging && ability_type == ABILITY_ON_ATTACK)
		ChargeAttack(target_mob, user)
...
/obj/item/ego_weapon/proc/HandleCharge(added_charge, mob/target)
	if(target)
		if((target.stat == DEAD) || HAS_TRAIT(target, TRAIT_GODMODE)) // lets not give them charge for beating up contained abnormalities
			return FALSE
