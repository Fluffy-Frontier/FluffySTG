...
/obj/item/ego_weapon/the_claw
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/attack.ogg'
...
/obj/item/ego_weapon/the_claw/equipped(mob/living/user)
	...
	justicemod = 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
...
/obj/item/ego_weapon/the_claw/dropped()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/death.ogg', 50, TRUE)
	qdel(src)
	return ..()
...
/datum/action/item_action/switch_serum/Trigger(trigger_flags, atom/target)
...
/obj/item/ego_weapon/the_claw/proc/SwitchSerum(mob/living/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/error.ogg', 50, TRUE)
...
/obj/item/ego_weapon/the_claw/attack_self(mob/living/user)
	...
	switch(serum)
		if("K")
			...
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/prepare.ogg', 50, TRUE)
		...
		if("R")
			...
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/r_prep.ogg', 50, TRUE)
			special_attack = TRUE
...
/obj/item/ego_weapon/the_claw/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	switch(serum)
		if("R") // Thanks helper
			...
			for(var/turf/T in get_line(user, get_ranged_target_turf_direct(user, A , dash_range)))
			...
			for(var/i = 1 to line_turfs.len)
				...
				playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/move.ogg', 50, 1)
			...
			for(var/mob/living/L in mobs_to_hit)
				...
				playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/stab.ogg', 25, 1)
				new /obj/effect/temp_visual/cleave(get_turf(L))
				L.apply_damage(justicemod*60, BRUTE, null, L.run_armor_check(null, BRUTE))
			dash_charges--
			if(dash_charges == 0)
				special_attack = FALSE
		...
		if("W")
			...
			uhoh.orbit(A, 0)
			playsound(A, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate1.ogg', 100, 1)
...
/obj/item/ego_weapon/the_claw/proc/SerumW(mob/living/user, mob/living/target) // It wasn't nice meeting you, farewell.
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/w_portal.ogg', 50, 1)
	...
	for(var/T in get_turf(shuffle(GLOB.start_landmarks_list)))
	...
	for(var/i = 1 to 5) // Thanks egor
		...
		for(var/mob/living/AA in range(1, target_turf))
			...
			AA.apply_damage(justicemod*50, BRUTE, null, AA.run_armor_check(null, BRUTE))
		...
		user.face_atom(target)
		sleep(4)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/w_slashes.ogg', 75, 1)
	user.face_atom(target)
	target.visible_message(
	...
	for(var/mob/living/AA in range(2, target))
		...
		AA.apply_damage(justicemod*60, BRUTE, null, AA.run_armor_check(null, BRUTE)) // Shouldn't have gotten close.
		new /obj/effect/temp_visual/cleave(get_turf(AA))
	user.AdjustStun(-60 SECONDS, ignore_canstun = TRUE)
	target.AdjustStun(-60 SECONDS, ignore_canstun = TRUE)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/w_fin.ogg', 75, 1)
	target.visible_message(
		span_warning("[target] suddenly gushes blood!"),
		span_userdanger("As [user] lets go, you start gushing blood!")
	)
	target.apply_damage(justicemod*150, BRUTE, null, target.run_armor_check(null, BRUTE)) // 150 so that it can scale form justice to about 300
...
/obj/item/ego_weapon/the_claw/proc/TriSerum(mob/living/user) // from PT, which was from Blue reverb
	...
	for(var/mob/living/L in range(12, user))
		...
		oh_dear.orbit(L, 0)
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate1.ogg', 25, 1)
	...
	for(var/mob/living/L in targets)
		...
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate2.ogg', 100, 1)
		L.apply_damage(justicemod*60, BRUTE, null, L.run_armor_check(null, BRUTE))
