...
/obj/item/ego_weapon/shield/lutemia
	...
	block_sound = 'sound/effects/parry.ogg'
...
/obj/item/ego_weapon/regret
	...
	usesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/club3.ogg'
...
/obj/item/ego_weapon/sorrow/attack_self(mob/living/user)
	if(do_after(user, 50, src))	//Five seconds of not doing anything, then teleport.
		...
		playsound(src, 'sound/items/weapons/contractor_baton/contractorbatonhit.ogg', 100, FALSE, 9)
...
/obj/item/ego_weapon/sorority
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/baton4.ogg'
...
/obj/item/ego_weapon/sorority/attack_self(mob/user)
	if(do_after(user, 10, src))	//Just a second to heal people around you, but it also harms them
		playsound(src, 'sound/items/weapons/taser.ogg', 200, FALSE, 9)
...
/obj/item/ego_weapon/hearth //From my sweet home.
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/sword3.ogg'
...
/obj/item/ego_weapon/hearth/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	playsound(target_turf, 'sound/items/weapons/pulse.ogg', 50, TRUE)
	var/damage_dealt = 0
...
/obj/item/ego_weapon/lantern/proc/CreateTrap(target, mob/living/carbon/human/user, proximity_flag)
...
/obj/effect/temp_visual/lanterntrap/proc/burst_check()
	for(var/mob/living/L in get_turf(src))
		if(creator.faction_check_atom(L))
			if(ishuman(L))
...
/obj/effect/temp_visual/lanterntrap/proc/burst()
	...
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_out.ogg', 40,TRUE)
	for(var/turf/open/T2 in RANGE_TURFS(range, src))
		new /obj/effect/temp_visual/yellowsmoke(T2)
		for(var/mob/living/L in creator.HurtInTurf(T2, list(), resonance_damage * damage_multiplier, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
...
/obj/item/ego_weapon/red_sheet
	...
	force = 22
	damtype = BRUTE
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nocry/ego_redsheet.ogg'
	var/hit_count = 0
...
/obj/item/ego_weapon/red_sheet/attack(mob/living/target, mob/living/user)
	...
	if(isliving(target))
		...
		if(hit_count >= 4)
			...
			if(!ishuman(M))
				playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/so_that_no_cry/curse_talisman.ogg', 100, 1)
...
/obj/item/ego_weapon/shield/capote
	...
	block_duration = 1 SECONDS
	block_cooldown = 3 SECONDS
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge.ogg'
...
/obj/item/ego_weapon/zauberhorn/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag && gun_cooldown <= world.time)
		...
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pagoda/throw.ogg', 100, TRUE) //yes im reusing a sound bite me
		G.firer = user
...
/obj/item/ego_weapon/zauberhorn/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
	return TRUE
...
/obj/item/ego_weapon/luminosity/attack(mob/living/M, mob/living/user)
	...
	if(combo==4)
		...
		if(!(HAS_TRAIT(M, TRAIT_GODMODE)) && M.stat != DEAD)
