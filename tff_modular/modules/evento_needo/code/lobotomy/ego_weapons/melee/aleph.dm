/obj/item/ego_weapon/paradise
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/paradise.ogg'
	var/ranged_cooldown
...
/obj/item/ego_weapon/paradise/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/paradise_ranged.ogg', 50, TRUE)
	var/damage_dealt = 0
	for(var/turf/open/T in range(target_turf, 1))
		new /obj/effect/temp_visual/paradise_attack(T)
		for(var/mob/living/L in user.HurtInTurf(T, list(), ranged_damage, BRUTE, hurt_mechs = TRUE))
...
/obj/item/ego_weapon/justitia
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/justitia1.ogg'
...
/obj/item/ego_weapon/justitia/attack(mob/living/M, mob/living/user)
	...
	switch(combo)
		...
		if(1,4)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/justitia3.ogg'
			user.changeNext_move(CLICK_CD_MELEE * 0.3)
		if(6)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/justitia4.ogg'
			combo = -1
			user.changeNext_move(CLICK_CD_MELEE * 1.2)
			var/turf/T = get_turf(M)
			new /obj/effect/temp_visual/justitia_effect(T)
			user.HurtInTurf(T, list(), 50, BRUTE)
		else
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/justitia1.ogg'
...
/obj/item/ego_weapon/da_capo
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/da_capo1.ogg'
...
/obj/item/ego_weapon/da_capo/attack(mob/living/M, mob/living/user)
	...
	switch(combo)
		...
		if(2)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/da_capo3.ogg'
			force *= 1.5
			combo = -1
		else
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/da_capo1.ogg'
...
/obj/item/ego_weapon/mimicry
	...
	attack_verb_continuous = list("slashes", "slices", "rips", "cuts")
	attack_verb_simple = list("slash", "slice", "rip", "cut")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/attack.ogg'
...
/obj/item/ego_weapon/twilight
	...
	force = 35
	swingstyle = WEAPONSWING_LARGESWEEP
	damtype = BRUTE // It's all damage types, actually
	attack_verb_continuous = list("slashes", "slices", "rips", "cuts")
	attack_verb_simple = list("slash", "slice", "rip", "cut")
...
/obj/item/ego_weapon/twilight/attack(mob/living/M, mob/living/user)
	if(!CanUseEgo(user))
		return
	..()
	for(var/damage_type in list(BRUTE, BRUTE, BRUTE))
		damtype = damage_type
...
/obj/item/ego_weapon/goldrush/attack(mob/living/target, mob/living/user)
	...
	if(do_after(user, 4, target))
		...
		target.apply_damage(goldrush_damage, BRUTE, null, target.run_armor_check(null, BRUTE), spread_damage = TRUE)		//MASSIVE fuckoff punch

		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/gen2.ogg', 50, TRUE)
...
/obj/item/ego_weapon/smile
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/hammer.ogg'
	usesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/hammer.ogg'
	toolspeed = 0.12
...
/obj/item/ego_weapon/smile/attack(mob/living/target, mob/living/carbon/human/user)
	if(!CanUseEgo(user))
		return
	. = ..()
	if((target.health <= target.maxHealth * 0.1 || target.stat == DEAD) && !(HAS_TRAIT(target, TRAIT_GODMODE)))	//Makes up for the lack of damage by automatically killing things under 10% HP
		target.gib()
...
/obj/item/ego_weapon/smile/suicide_act(mob/living/carbon/user)
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/hammer.ogg', 50, TRUE, -1)
	user.gib()
...
/obj/item/ego_weapon/blooming
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/rapier2.ogg'
...
/obj/item/ego_weapon/censored
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/censored1.ogg'
...
/obj/item/ego_weapon/censored/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	var/list/turfs_to_hit = get_line(user, target_turf)
	for(var/turf/T in turfs_to_hit)
		if(T.density)
			break
		new /obj/effect/temp_visual/cult/sparks(T)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/censored2.ogg', 75)
	special_cooldown = world.time + special_cooldown_time
	if(!do_after(user, 7, src))
		return
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/censored3.ogg', 75)
		...
		for(var/mob/living/L in T)
			if(special_checks_faction && user.faction_check_atom(L))
				if(ishuman(L))
					...
					continue
			L.apply_damage(special_damage, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/obj/item/ego_weapon/soulmate/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag && gun_cooldown <= world.time)
		...
		if(gunbuff)
			...
			playsound(user, 'sound/items/weapons/ionrifle.ogg', 100, TRUE)
		else
...
/obj/item/ego_weapon/soulmate/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
...
/obj/item/ego_weapon/space/attack(mob/living/target, mob/living/user)
	...
	if(!CanUseEgo(user))
		return
	target.apply_damage(force, BRUTE, null, target.run_armor_check(null, BRUTE), spread_damage = TRUE)

	if(!canaoe)
		return
	if(do_after(user, 5, src, IGNORE_USER_LOC_CHANGE))
		playsound(src, 'sound/items/weapons/rapierhit.ogg', 100, FALSE, 4)
...
/obj/item/ego_weapon/seasons
	...
	var/list/season_list = list(
		"spring" = list(80, 1, 1, list("bashes", "bludgeons"), list("bash", "bludgeon"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/gen1.ogg', "vernal equinox", BRUTE, BRUTE,
		"A gigantic, thorny bouquet of roses."),
		"summer" = list(120, 1.6, 1, list("tears", "slices", "mutilates"), list("tear", "slice","mutilate"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/seasons/summer_attack.ogg', "summer solstice", BRUTE, BRUTE,
		"Looks some sort of axe or bladed mace. An unbearable amount of heat comes off of it."),
		"fall" = list(100, 1.2, 1, list("crushes", "burns"), list("crush", "burn"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/seasons/fall_attack.ogg', "autumnal equinox",BRUTE ,BRUTE,
		"In nature, a light is often used as a simple but effective lure. This weapon follows the same premise."),
		"winter" = list(80, 1, 2, list("skewers", "jabs"), list("skewer", "jab"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/seasons/winter_attack.ogg', "winter solstice",BRUTE ,BRUTE,
...
/obj/item/ego_weapon/shield/distortion
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/distortedform/slam.ogg'
	reductions = list(60, 60, 60, 60)
	projectile_block_duration = 3 SECONDS
	block_duration = 4.5 SECONDS
	block_cooldown = 2.5 SECONDS
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/heavy_guard.ogg'
...
/obj/item/ego_weapon/farmwatch
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/farmwatch.ogg'
...
/obj/item/ego_weapon/farmwatch/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(istype(target, /mob/living/simple_animal/hostile/farmwatch_plant))
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/farmwatch_tree.ogg', 200, 1)
...
/obj/item/ego_weapon/farmwatch/attack_self(mob/user)
	...
	if(ability_cooldown > world.time)
		...
		return
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/white/white_reflect.ogg', 50, TRUE)
...
/obj/item/ego_weapon/spicebush//TODO: actually code this
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/spicebush.ogg'
...
/obj/item/ego_weapon/spicebush/attack_self(mob/user)
	...
	if(do_after(user, 20, src))
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/spicebush_special.ogg', 50, FALSE)
...
/obj/item/ego_weapon/spicebush/fan/proc/ResetIcons()
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/spicebush_openfan.ogg', 50, TRUE)
	icon_state = "spicebush_2"
...
/obj/item/ego_weapon/spicebush/fan/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	..()
	ranged_cooldown = world.time + ranged_cooldown_time
	playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/spicebush_fan.ogg', 50, TRUE)
	var/damage_dealt = 0
	var/modified_damage = (ranged_damage)
	if(do_after(user, 5))
		for(var/turf/open/T in range(target_turf, 1))
			new /obj/effect/temp_visual/spicebloom(T)
			for(var/mob/living/L in T.contents)
				L.apply_damage(modified_damage, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
				if((L.stat < DEAD) && !(HAS_TRAIT(L, TRAIT_GODMODE)))
...
/obj/item/ego_weapon/mockery
	...
	var/list/weapon_list = list(
		"whip" = list(35, 0.5, 3, list("lacerates", "disciplines"), list("lacerate", "discipline"), 'sound/items/weapons/whip.ogg'),
		"sword" = list(80, 1, 1, list("tears", "slices", "mutilates"), list("tear", "slice","mutilate"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade4.ogg'),
		"hammer" = list(40, 1.4, 1, list("crushes"), list("crush"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/baton2.ogg'),
		"bat" = list(120, 1.6, 1, list("bludgeons", "bashes"), list("bludgeon", "bash"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/gen1.ogg')
		)
...
/obj/item/ego_weapon/mockery/proc/Transform()
	...
	if(current_holder)
		to_chat(current_holder,span_notice("[src] suddenly transforms!"))
		current_holder.playsound_local(current_holder, 'sound/effects/blob/blobattack.ogg', 75, FALSE)
...
/obj/item/ego_weapon/oberon
	...
	var/form = "whip"
	var/list/weapon_list = list(
		"whip" = list(30, 0.8, 3, list("lacerates", "disciplines"), list("lacerate", "discipline"), 'sound/items/weapons/whip.ogg', BRUTE, "This form has higher reach, hits 3 times, and builds up attack speed before unleasheing a powerful burst of damage."),
		"sword" = list(55, 0.8, 1, list("tears", "slices", "mutilates"), list("tear", "slice","mutilate"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade4.ogg', BRUTE, "This form can fire a projectile and does both RED DAMAGE and BLACK DAMAGE."),
		"hammer" = list(55, 1.4, 1, list("crushes"), list("crush"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/baton2.ogg', BRUTE, "This form deals damage in an area and incease the RED and BLACK vulnerability by 0.2 to everything in that area."),
		"bat" = list(160, 1.6, 1, list("bludgeons", "bashes"), list("bludgeon", "bash"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/gen1.ogg', BRUTE, "This form does RED DAMAGE and knocks back enemies."),
		"scythe" = list(100, 1.2, 1, list("slashes", "slices", "rips", "cuts"), list("slash", "slice", "rip", "cut"), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/attack.ogg', BRUTE, "This form does RED DAMAGE and does 50% more damage when hitting targets below 50% health.")
		)
	var/gun_cooldown
	var/gun_cooldown_time = 1 SECONDS
	var/build_up = 0.8
...
/obj/item/ego_weapon/oberon/attack(mob/living/target, mob/living/carbon/human/user)
	...
	switch(form)
		if("sword")
			var/red = force
			red*=justicemod
			target.apply_damage(red, BRUTE, null, target.run_armor_check(null, BRUTE), spread_damage = TRUE)
		...
		if("whip")
			...
			for(var/i = 1 to 2)
				...
				if(target in view(reach,user))
					...
					target.attacked_by(src, user)
					log_combat(user, target, pick(attack_verb_continuous), src.name, "(COMBAT_MODE: [uppertext(user.combat_mode)]) (DAMTYPE: [uppertext(damtype)])")
...
/obj/item/ego_weapon/oberon/proc/Smash(mob/living/carbon/user, atom/target)
	smashing = TRUE
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_prepare.ogg', 50, 0, 3)
	var/justicemod = 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
	...
	for(var/i = 0; i < 3; i++)
		for(var/turf/T in view(3, user))
			...
			for(var/mob/living/L in T)
				if(user.faction_check_atom(L))
					continue
				L.apply_damage(smash_damage, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
		playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fairy_longlegs/attack.ogg', 75, 0, 3)
		sleep(0.5 SECONDS)
	smashing = FALSE
	return
...
/obj/item/ego_weapon/shield/gasharpoon
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_hit.ogg'
	reductions = list(60, 40, 70, 40)
	projectile_block_duration = 2 SECONDS
	block_duration = 3 SECONDS
	block_cooldown = 2 SECONDS
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_queeblock.ogg'
	...
	var/list/weapon_list = list(
		"pip" = list(40, 1.6, 1, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_piphit.ogg', "You burn the E.G.O of the innocent deckhand."),
		"starbuck" = list(70, 1, 1, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_starbuckhit.ogg', "Your burn the E.G.O of your first mate."),
		"queeqeg" = list(70, 1.6, 2, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_queehit.ogg', "You burn the E.G.O of the indominable harpooneer.")
		)
...
/obj/item/ego_weapon/shield/gasharpoon/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	if(!proximity_flag && gun_cooldown <= world.time)
		...
		for(var/i = 0 , i <= burst_size, i++)
			...
			G.fired_from = src //for signal check
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_fire.ogg', 100, TRUE)
			G.firer = user
			G.set_angle(get_angle(user, target))
			G.fire()
			//sleep(0.1 SECONDS)
...
/obj/item/ego_weapon/shield/gasharpoon/AnnounceBlock(mob/living/carbon/user)//block
	..()
	var/aoe = 5
	var/fortmod = 1 + user.get_clothing_class_level(CLOTHING_ENGINEERING)
	aoe*=fortmod
...
/obj/item/ego_weapon/shield/gasharpoon/proc/Transform()
	...
	update_icon_state()
	if(current_holder)
		current_holder.playsound_local(current_holder, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_transform.ogg', 75, FALSE)
	force = weapon_list[form][1]
	attack_speed = weapon_list[form][2]
	reach = weapon_list[form][3]
...
/obj/item/ego_weapon/shield/gasharpoon/click_alt(mob/user)
	..()
	return Revert()
...
/obj/item/ego_weapon/shield/gasharpoon/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
	return TRUE
...
/obj/projectile/ego_bullet/gasharpoon
	...
	damage = 25
	damage_type = BRUTE
	hitsound = "tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/gasharpoon_bullet_impact.ogg"
...
/obj/item/ego_weapon/wield/darkcarnival/OnWield(obj/item/source, mob/user)
	damtype = BRUTE
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clownsmiling/egostab.ogg'
...
/obj/item/ego_weapon/wield/darkcarnival/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if((get_dist(user, A) < 2))
		A.attackby(src,user)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clownsmiling/jumpscare.ogg', 50, FALSE, 9)
