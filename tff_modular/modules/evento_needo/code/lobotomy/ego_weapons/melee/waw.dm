...
/obj/item/ego_weapon/oppression
	...
	attack_verb_continuous = list("cleaves", "cuts")
	attack_verb_simple = list("cleaves", "cuts")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade4.ogg'
	var/charged = FALSE
	var/meter = 0
	var/meter_counter = 1
...
/obj/item/ego_weapon/remorse
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/remorse.ogg'
...
/obj/item/ego_weapon/remorse/attack(mob/living/M, mob/living/user)
	...
	if(mode)
		if(M in targets)
			playsound(M, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/nail1.ogg', 100, FALSE, 4)
			M.apply_damage(ranged_damage, BRUTE, null, M.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/obj/item/ego_weapon/mini/crimson/attack(mob/living/M, mob/living/user)
	...
	switch(combo)
		if(2)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/attack_2.ogg'
		if(3)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/attack_3.ogg'
		else
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/attack_1.ogg'
	force *= (1 + (combo * 0.15))
...
/obj/item/ego_weapon/mini/crimson/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if(!LAZYLEN(turfs_to_hit))
		return
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/throw.ogg', 75, TRUE, 3)
	...
	for(var/i = 1 to turfs_to_hit.len) // Basically, I copied my code from helper's realized ability. Yep.
		...
		for(var/mob/living/L in T)
			...
			L.apply_damage(dealing_damage, BRUTE, null, L.run_armor_check(null, BRUTE))
...
/obj/item/ego_weapon/thirteen/attack(mob/living/M, mob/living/user)
	...
	if(combo >= 13)
		combo = 0
		force = user.get_clothing_class_level(CLOTHING_ARMORED) * 10
		new /obj/effect/temp_visual/thirteen(get_turf(M))
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/price_of_silence.ogg', 25, FALSE, 9)
	..()
	combo += 1
	force = initial(force)
...
/obj/item/ego_weapon/stem
	...
	equip_sound = 'sound/mobs/non-humanoids/venus_trap/venus_trap_hit.ogg'
	pickup_sound = 'sound/mobs/non-humanoids/venus_trap/venus_trap_hurt.ogg'
	...
	if(vine_cooldown <= world.time)
		...
		for(var/i = 1 to channeling_cycle_max)
			...
			for(var/mob/living/C in oview(5, get_turf(src)))
				...
				if(user.faction_check_atom(C) && !vine_damage_bonus)
					continue
				//new /obj/effect/temp_visual/vinespike(get_turf(C))
				C.apply_damage(vine_damage + vine_damage_bonus, BRUTE, null, C.run_armor_check(null, BRUTE), spread_damage = TRUE)
				affected_mobs += 1
			playsound(loc, 'sound/mobs/non-humanoids/venus_trap/venus_trap_hurt.ogg', min(75, affected_mobs * 15), TRUE, round( affected_mobs * 0.5))
		AlterMoveResist(user, 0.4)
...
/obj/item/ego_weapon/ebony_stem/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if(do_after(user, 5))
		var/damage_dealt = (ranged_damage)
		playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ebonyqueen/attack.ogg', 50, TRUE)
		for(var/turf/open/T in RANGE_TURFS(1, target_turf))
			new /obj/effect/temp_visual/thornspike(T)
			user.HurtInTurf(T, list(), damage_dealt, BRUTE, hurt_mechs = TRUE)
...
/obj/item/ego_weapon/wings // Is this overcomplicated? Yes. But I'm finally happy with what I want to make of this weapon.
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge3.ogg'
	var/hit_count = 0
	var/max_count = 16
	var/special_cost = 4
...
/obj/item/ego_weapon/wings/proc/SecondSwing(mob/living/M, mob/living/user)
	...
	log_combat(user, M, "attacked", src.name, "(COMBAT_MODE: [uppertext(user.combat_mode)]) (DAMTYPE: [uppertext(damtype)])")
...
/obj/item/ego_weapon/shield/swan
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/spear1.ogg'
...
/obj/item/ego_weapon/shield/swan/proc/Reflect(mob/living/carbon/human/user, damage, damagetype, def_zone)
	...
	for(var/mob/living/simple_animal/hostile/H in view(8, user))
		mob_list += H
	if(!mob_list.len)
		return
	...
	playsound(user, 'sound/items/weapons/resonator_blast.ogg', 30, TRUE)
	S.firer = user
	S.set_angle(get_angle(user, target))
	S.fire()
	return
...
/obj/item/ego_weapon/shield/swan/proc/on_projectile_hit(atom/fired_from, atom/movable/firer, atom/target, Angle)
	SIGNAL_HANDLER
	return TRUE
...
/obj/projectile/ego_bullet/swan
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/small_smash1.ogg'
	hitsound_wall = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/small_smash1.ogg'
...
/obj/item/ego_weapon/moonlight/attack_self(mob/user)
	...
	if(do_after(user, 30))	//3 seconds for a big heal.
		playsound(src, 'sound/effects/magic/staff_healing.ogg', 200, FALSE, 9)
...
/obj/item/ego_weapon/dipsia/attack_self(mob/living/user)
	...
	if(siphoning)
		...
		siphoning = FALSE
		filters = null
		user.playsound_local(user, 'sound/effects/wounds/crackandbleed.ogg', 25, TRUE)
		return
	var/datum/component/bloodfeast/bloodfeast = GetComponent(/datum/component/bloodfeast)
	siphoning = TRUE
	user.playsound_local(user, 'sound/effects/wounds/crackandbleed.ogg', 25, TRUE)
...
/obj/item/ego_weapon/blind_rage
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash1.ogg'


	var/aoe_damage = 15
	var/aoe_damage_type = BRUTE
	var/aoe_range = 2
	var/attacks = 0
...
/obj/item/ego_weapon/blind_rage/attack(mob/living/M, mob/living/carbon/human/user)
	...
	switch(attacks)
		if(0)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash1.ogg'
		if(1)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash2.ogg'
		if(2)
			hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash3.ogg'
	var/damage = aoe_damage * (1 + user.get_clothing_class_level(CLOTHING_ARMORED))
	if(attacks == 0)
		damage *= 3
	if(user.sanity_lost)
		damage *= 1.2
...
/obj/item/ego_weapon/diffraction/attack(mob/living/target, mob/living/user)
	if((target.health <= target.maxHealth * 0.4) && !(HAS_TRAIT(target, TRAIT_GODMODE)))
		force *= 2
	..()
	force = initial(force)
...
/obj/item/ego_weapon/mini/infinity
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/book/scribble.ogg'
...
/obj/item/ego_weapon/mini/infinity/attack(mob/living/target, mob/living/user)
	...
	if(do_after(user, 4, src))
		...
		mark_type = pick(BRUTE, BRUTE, BRUTE)
		damtype = mark_type
...
/obj/item/ego_weapon/mini/infinity/proc/cast(mob/living/target, mob/living/user, damage_color)
	...
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energyfinisher3.ogg', 15, TRUE, extrarange = stealthy_audio ? SILENCED_SOUND_EXTRARANGE : -1, falloff_distance = 0)
...
/obj/item/ego_weapon/amrita
	...
	attack_verb_continuous = list("slams", "attacks")
	attack_verb_simple = list("slam", "attack")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_attack.ogg'
...
/obj/item/ego_weapon/amrita/attack_self(mob/living/carbon/human/user) //spin attack with knockback
	if(!CanUseEgo(user))
		return
	if(!can_spin)
	...
	if(do_after(user, 13, src))
		...
		can_spin = TRUE
		addtimer(CALLBACK(src, PROC_REF(spin_reset)), 13)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_bite.ogg', 75, FALSE, 4)
		aoe*=justicemod
...
/obj/item/ego_weapon/wield/discord/attack(mob/living/target, mob/living/carbon/human/user)
	...
	if(!ishostile(target))
		return
	if(target.stat == DEAD || HAS_TRAIT(target, TRAIT_GODMODE))
		return
	Harmony(user)
	if(!wielded)
		return
...
/obj/item/ego_weapon/shield/innocence
	...
	block_cooldown = 3 SECONDS
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/orangetree/ding.ogg'
...
/obj/item/ego_weapon/rimeshank
	...
	attack_verb_continuous = list("slams", "attacks")
	attack_verb_simple = list("slam", "attack")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/babayaga/attack.ogg'
...
/obj/item/ego_weapon/rimeshank/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if(do_after(user, 5, src))
		dash_cooldown = world.time + dash_cooldown_time
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/babayaga/charge.ogg', 50, FALSE, -1)
...
/obj/item/ego_weapon/psychic
	...
	attack_verb_continuous = list("stabs", "attacks", "slashes")
	attack_verb_simple = list("stab", "attack", "slash")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/knife4.ogg'
	var/dodgelanding
...
/obj/item/ego_weapon/cobalt
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/big_wolf/Wolf_Hori.ogg'
...
/obj/item/ego_weapon/cobalt/proc/FrenzySwipe(mob/living/wolf)
	...
	if(ishuman(wolf))
		...
		log_combat(wolf, those_we_rend, pick(attack_verb_continuous), src.name, "(COMBAT_MODE: [uppertext(wolf.combat_mode)]) (DAMTYPE: [uppertext(damtype)])")
...
/obj/item/ego_weapon/warring/attack_self(mob/living/user)
	if(!currently_charging && charge_amount >= 5)
		...
		hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_peck.ogg'
	else
		return
	. = ..()

/obj/item/ego_weapon/warring/ChargeAttack(mob/living/user)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_bolt.ogg', 50, TRUE)
...
/obj/item/ego_weapon/hyde/proc/Transform(mob/living/carbon/human/user)
	user.emote("scream")
	playsound(get_turf(src),'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg', 75, 1)//YEOWCH!
	icon_state = ("hyde_" + chosen_style)
	update_icon_state()
	force = 42
	switch(chosen_style)
		if("red")
			user.apply_damage(50, BRUTE, null, user.run_armor_check(null, BRUTE), spread_damage = TRUE)
			damtype = BRUTE
...
/obj/item/ego_weapon/hyde/proc/SwitchForm(mob/living/carbon/human/user)
	var/list/armament_icons = list(
		"red" = image(icon = 'tff_modular/modules/evento_needo/icons/Teguicons/ego_weapons.dmi', icon_state = "hyde_red_indicator"),
		"white"  = image(icon = 'tff_modular/modules/evento_needo/icons/Teguicons/ego_weapons.dmi', icon_state = "hyde_white_indicator"),
		"black"  = image(icon = 'tff_modular/modules/evento_needo/icons/Teguicons/ego_weapons.dmi', icon_state = "hyde_black_indicator"),
	)
...
/obj/item/ego_weapon/hyde/proc/ResetTimer(mob/living/carbon/human/user)
	...
	if(user)
		...
		playsound(get_turf(src),'sound/effects/blob/attackblob.ogg', 75, 1)
...
/obj/item/ego_weapon/rosa/attack(mob/living/M, mob/living/carbon/human/user)
	..()
	if(M==user)
		var/justicemod = 1 + (user.get_clothing_class_level(CLOTHING_ARMORED))
		var/heal_amount = (force * justicemod * 0.75)
		var/armormod = (user.run_armor_check(null, BRUTE))
...
/obj/item/ego_weapon/blind_obsession/throw_impact(atom/hit_atom, datum/thrownthing/throwingdatum)
	. = ..()
	deltimer(power_timer)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/hammer.ogg', 300, FALSE, 9)
	if(charged)
		var/damage = 75
		if(ishuman(hit_atom))
			var/mob/living/carbon/human/user = hit_atom
			damage += user.get_clothing_class_level(CLOTHING_ARMORED)
			damage *= 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
...
/obj/item/ego_weapon/abyssal_route/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	if(combo == 4)
		...
		if(QDELETED(user))
			return
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/piscinemermaid/waterjump.ogg', 20, 0, 3)
		animate(user, alpha = 1,pixel_x = 0, pixel_z = -16, time = 0.1 SECONDS)
		user.pixel_z = -16
		//sleep(0.5 SECONDS)
		can_attack = TRUE
		...
		if((get_dist(user, A) < 2))
			DiveAttack(A,user)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bloodbath/Bloodbath_EyeOn.ogg', 20, 0, 3)
...
/obj/item/ego_weapon/windup/attack(mob/living/M, mob/living/user)
	...
	if(charges > 0)
		if(charges == 4)
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clock/finish.ogg', 60)
		else
...
/obj/item/ego_weapon/windup/proc/PlayChargeSound()
	...
	if(!charges) //We don't play the sound if the player has already emptied out by now
		return
	if(charges >= 4)
		playsound(src.loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energy3.ogg', 75, TRUE)
		return
...
/obj/item/ego_weapon/sunyata
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/myformempties/attack.ogg'
...
/obj/item/ego_weapon/effervescent/SweepMiss(atom/target, mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ambling pearl/goo effect.ogg', 30, TRUE)
...
/datum/status_effect/effervescent_sticky/tick()
	...
	if(prob(40))
		playsound(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ambling pearl/goo effect.ogg', 40)

/datum/status_effect/effervescent_sticky/proc/CleanOff(datum/source, mob/living/carbon/wiper)
	. = FALSE
	if(!istype(wiper))
		return
	if(!wiper.combat_mode)
		return
	if(wiper != owner)
...
/obj/item/ego_weapon/contempt
	...
	attack_verb_continuous = list("pokes", "jabs", "tears", "lacerates", "gores")
	attack_verb_simple = list("poke", "jab", "tear", "lacerate", "gore")
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spiral_contempt/spiral_hit.ogg'
...
/obj/item/ego_weapon/contempt/attack(mob/living/M, mob/living/user)
	...
	if(mode)
		if(M in targets)
			playsound(M, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spiral_contempt/spiral_bleed.ogg', 100, FALSE, 4)
			M.apply_damage(ranged_damage, BRUTE, null, M.run_armor_check(null, BRUTE), spread_damage = TRUE)
			targets -= M
	..()
	hitsound = initial(hitsound)
...
/obj/item/ego_weapon/contempt/attack_self(mob/user)
	...
	if(mode)
		...
		targets = list()
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spiral_contempt/spiral_grow.ogg', 20, FALSE)
		return

	if(!mode)
		mode = TRUE
		to_chat(user,span_warning("Your [src] now menances with spikes of gold."))
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spiral_contempt/spiral_whine.ogg', 20, FALSE)
		return
