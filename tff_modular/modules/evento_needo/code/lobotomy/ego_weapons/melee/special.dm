...
/obj/item/ego_weapon/eyeball/attack(mob/living/target, mob/living/carbon/human/user)
	...
	var/userfort = user.get_clothing_class_level(CLOTHING_ENGINEERING)
	var/fortitude_mod = clamp((userfort - 2) / 2 + 2, 0, 50) // 2 at 40 fortitude, 12 at 60 fortitude, 22 at 80 fortitude, 32 at 100 fortitude
...
/obj/item/ego_weapon/mail_satchel/attack(atom/A, mob/living/user, proximity_flag, params)
	force = initial(force)
	var/usertemp = user.get_clothing_class_level(CLOTHING_SCIENCE)
	var/temperance_mod = clamp((usertemp - 2) / 3 + 2, 0, 20)
...
/obj/item/ego_weapon/lance/famiglia
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/mace1.ogg'
...
/obj/item/ego_weapon/iron_maiden/attack(mob/living/target, mob/living/user)
	...
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/we_can_change_anything/change_generate.ogg', get_clamped_volume(), FALSE, extrarange = stealthy_audio ? SILENCED_SOUND_EXTRARANGE : -1, falloff_distance = 0)
...
/obj/item/ego_weapon/goldrush/nihil/attack(mob/living/target, mob/living/user)
	...
	if(combo==4)
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/finisher2.ogg', 50, FALSE, 9)
...
/obj/item/ego_weapon/shield/despair_nihil/attack(mob/living/M, mob/living/user)
	...
	if(combo==4)
		...
		user.changeNext_move(CLICK_CD_MELEE * 2)
		force *= 5	// Should actually keep up with normal damage.
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/sword5.ogg', 50, FALSE, 9)
		to_chat(user,span_warning("You are offbalance, you take a moment to reset your stance."))
	else
		user.changeNext_move(CLICK_CD_MELEE * 0.4)
	..()
