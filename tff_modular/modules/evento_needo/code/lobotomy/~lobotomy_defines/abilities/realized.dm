/* E.G.O assimilation */
/datum/action/spell_action/ego_assimilation
...
/datum/action/spell_action/ego_assimilation/Perform(atom/target, user)
	..()
...
/datum/action/spell_action/ego_assimilation/proc/FindItems(user)
...
/datum/action/spell_action/ego_assimilation/proc/DeleteAbility(mob/living/carbon/human/user)
...
/datum/action/spell_action/universe_song
...
/datum/action/spell_action/universe_song/Perform(target, mob/user)
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fragment/sing.ogg', 50, 0, 4)
	Pulse(user)
	for(var/i = 1 to damage_count - 1)
		addtimer(CALLBACK(src, PROC_REF(Pulse), user), i*3)
	return ..()

/datum/action/spell_action/universe_song/proc/Pulse(mob/user)
...
/datum/action/spell_action/shrimp
	name = "Backup Shrimp"
...
/datum/action/spell_action/lamp/Perform(target, mob/user)
	cooldown = world.time + (2 SECONDS)
	if(!do_after(user, 1.5 SECONDS))
		to_chat(user, span_warning("You must stand still to see!"))
		return
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bigbird/hypnosis.ogg', 75, 0, 2)
...
/* Nothing There - Shell */
/datum/action/spell_action/goodbye
	name = "Goodbye"
...
/datum/action/spell_action/goodbye/Perform(target, mob/user)
	var/mob/living/carbon/human/H = user
	cooldown = world.time + (1.5 SECONDS)
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/goodbye_cast.ogg', 75, 0, 5)
	...
	for(var/turf/T in view(2, user))
		...
		for(var/mob/living/L in T)
			...
			if(L.health < 0)
				L.gib()
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/goodbye_attack.ogg', 75, 0, 7)
	return ..()
/* Mosb - Laughter */
/datum/action/spell_action/screach
	name = "Screach"
...
/datum/action/spell_action/screach/Perform(target, mob/user)
	cooldown = world.time + (2 SECONDS)
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/bite.ogg', 50, 0)
	if(!do_after(user, 1.5 SECONDS))
		to_chat(user, span_warning("You must stand still to screach!"))
		return
	var/mob/living/carbon/human/H = user
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/scream.ogg', 75, 0, 2)
...
/datum/action/spell_action/judgement
	name = "Soul Judgement"
...
/datum/action/spell_action/judgement/Perform(target, mob/user)
	cooldown = world.time + (2 SECONDS)
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/judgementbird/pre_ability.ogg', 50, 0)
	...
	if(!do_after(user, 1.5 SECONDS))
		...
		return
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/judgementbird/ability.ogg', 75, 0, 2)
	for(var/mob/living/L in view(damage_range, user))
		if(user.faction_check_atom(L, FALSE))
			continue
		if(L.stat == DEAD)
...
/datum/action/spell_action/fire_explosion
	name = "Match flame"
...
/datum/action/spell_action/fire_explosion/Perform(target, mob/user)
	cooldown = world.time + (5 SECONDS)
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scorchedgirl/pre_ability.ogg', 50, 0, 2)
	if(!do_after(user, 1.5 SECONDS))
		to_chat(user, span_warning("You must stand still to ignite the explosion!"))
		return
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scorchedgirl/ability.ogg', 60, 0, 4)
...
/datum/action/spell_action/fire_explosion/proc/FireExplosion(turf/T)
...
/datum/action/spell_action/road_of_gold
...
/datum/action/spell_action/road_of_gold/Perform(mob/living/simple_animal/hostile/target, mob/user)
	if(!istype(user))
		return ..()
	cooldown = world.time + (2 SECONDS)
	target = null
...
/datum/action/spell_action/road_of_gold/proc/CleanUp()
...
/datum/action/spell_action/road_of_gold/proc/Circle(turf/first_target, turf/second_target, mob/user = null)
...
/datum/action/spell_action/justice_and_balance
...
/datum/action/spell_action/justice_and_balance/Perform(target, user)
...
/datum/action/spell_action/justice_and_balance/proc/Smash(mob/user, on_use_charges)
...
/datum/action/spell_action/punishment
	name = "Punishment"
...
/datum/action/spell_action/punishment/Perform(target, mob/user)
	var/mob/living/carbon/human/H = user
...
/datum/action/spell_action/petal_blizzard
	name = "Petal Blizzard"
...
/datum/action/spell_action/petal_blizzard/Perform(target, mob/user)
	var/mob/living/carbon/human/H = user
...
/mob/living/simple_animal/hostile/spicebush_plant/death()
	density = FALSE
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ego/farmwatch_tree.ogg', 100, 1)
...
/datum/action/spell_action/overheat
	name = "Overheat"
...
/datum/action/spell_action/overheat/Perform(target, mob/user)
	var/mob/living/carbon/human/H = user
...
/* Yang - Duality */
/datum/action/spell_action/tranquility
	name = "Tranquility"
...
/datum/action/spell_action/tranquility/Perform(target, mob/living/carbon/human/user)
	cooldown = world.time + (1.5 SECONDS)
	if(!do_after(user, 1 SECONDS))
	...
	for(var/i = 0 to explosion_range)
		for(var/turf/T in all_turfs)
			...
			user.HurtInTurf(damage_amount, list(), BRAIN)
			for(var/mob/living/carbon/human/L in T)
				if(!user.faction_check_atom(L, FALSE))
					continue
				if(L.stat == DEAD)
...
/datum/action/spell_action/galaxy_gift
...
/datum/action/spell_action/galaxy_gift/Perform(target, mob/living/carbon/human/user)
	if(!istype(user))
		return
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(H.has_status_effect(/datum/status_effect/galaxy_gift))
			existing_gifted += H
	playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/despairknight/gift.ogg', 50, 0, 2) //placeholder, uses KoD blessing noise at the moment
...
/datum/action/spell_action/comatose
	name = "Comatose"
...
/datum/action/spell_action/comatose/Perform(target, mob/living/carbon/human/user)
	if(istype(user.get_item_by_slot(ITEM_SLOT_OCLOTHING), /obj/item/clothing/suit/armor/ego_gear/realization/comatose))
		...
		user.Knockdown(1)
		user.playsound_local(get_turf(user), "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/happyteddy/teddy_lullaby.ogg", 25, 0)
...
/datum/action/spell_action/brokencrown
	name = "Broken Crown"
...
/datum/action/spell_action/brokencrown/proc/Absorb(obj/item/I, mob/living/user)
	if(!ego_list || ready)
...
/datum/action/spell_action/brokencrown/proc/Reload(obj/item/I, mob/living/user)
	to_chat(user, span_nicegreen("The ability has been recharged."))
	ready = TRUE
	qdel(I)

/datum/action/spell_action/brokencrown/Perform(target, mob/living/carbon/human/user)
	if(!ready)
	...
	if(istype(user.get_item_by_slot(ITEM_SLOT_OCLOTHING), /obj/item/clothing/suit/armor/ego_gear/realization/brokencrown))
		user.playsound_local(get_turf(user), "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bloodbath/Bloodbath_EyeOn.ogg", 25, 0)
		if(!linked_structure)
...
/datum/action/spell_action/brokencrown/proc/Reabsorb()
	if(linkeditem && !ready)
...
/datum/action/spell_action/wellcheers
...
/datum/action/spell_action/wellcheers/Perform(target, mob/user)
	for(var/i = 1 to 3)
...
/mob/living/simple_animal/hostile/shrimp/friendly/Initialize()
	.=..()
	AddElement(/datum/component/knockback, 1, FALSE, TRUE)
	QDEL_IN(src, (90 SECONDS))
...
/datum/action/spell_action/prayer
	name = "Prayer"
...
/datum/action/spell_action/prayer/Perform(target, mob/living/carbon/human/user)
...
/datum/action/spell_action/nest
	name = "Worm spawn"
...
/datum/action/spell_action/nest/Perform(target, mob/user)
	for(var/i = 1 to 7)
...
/datum/action/spell_action/rip_space
	name = "Rip Space"
...
/datum/action/spell_action/rip_space/Perform(target, mob/living/user)
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/ripspace_begin.ogg', 100, 0)
	sleep(1 SECONDS)
	qdel(P)
	user.alpha = 0
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/ripspace_end.ogg', 100, 0)
	return ..()

/datum/action/spell_action/rip_space/proc/dash_attack(mob/living/target, mob/living/user)
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/ripspace_hit.ogg', 75, 0)
	sleep(1)
	qdel(DE)
