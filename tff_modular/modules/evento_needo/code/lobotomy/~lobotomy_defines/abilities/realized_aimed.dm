...
/datum/action/cooldown/spell_action/aimed/helper_dash
	name = "Blade dash"
...
/datum/action/cooldown/spell_action/aimed/helper_dash/Activate(atom/target)
	var/turf/target_turf = get_turf(target)
	var/list/line_turfs = list(target_turf)
	var/list/mobs_to_hit = list()
	var/stop_charge = FALSE
	for(var/turf/T in get_line(owner, get_ranged_target_turf_direct(owner, target, dash_range)))
	...
	for(var/i = 1 to line_turfs.len)
		...
		if(!istype(T))
			continue
		var/obj/effect/temp_visual/decoy/D = new /obj/effect/temp_visual/decoy(T, owner)
		var/matrix/M = matrix(D.transform)
		M.Turn(45 * i)
		D.transform = M
		D.alpha = min(150 + i*15, 255)
		animate(D, alpha = 0, time = 2 + i*2)
		playsound(D, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/move0[pick(1,2,3)].ogg", rand(10, 30), 1, 3)
	// Damage
	for(var/mob/living/L in mobs_to_hit)
		if(owner.faction_check_atom(L))
			continue
		if(HAS_TRAIT(L, TRAIT_GODMODE))
			continue
		L.visible_message("<span class='boldwarning'>[owner] runs through [L]!</span>")
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/attack.ogg', 25, 1)
		new /obj/effect/temp_visual/kinetic_blast(get_turf(L))
		L.apply_damage(dash_damage, BURN, null, L.run_armor_check(null, BURN))
		if(L.health <= 0)
			L.gib()
	return .. ()

/* One Sin and Hundreds of Good Deeds - Confessional */
/datum/action/cooldown/spell_action/aimed/cross_spawn
	name = "Cross summon"
	desc = "An ability that allows its user to summon a holy cross to damage the enemies and heal the mind of their allies."
	button_icon_state = "cross_spawn0"
...
/datum/action/cooldown/spell_action/aimed/cross_spawn/Activate(atom/target)
	if(get_dist(owner, target) > 10)
		return
	var/turf/target_turf = get_turf(target)
	new /obj/effect/temp_visual/cross/fall(target_turf)
	addtimer(CALLBACK(src, PROC_REF(SplashEffect), target_turf, target), 5.5)
	return ..()

/datum/action/cooldown/spell_action/aimed/cross_spawn/proc/SplashEffect(turf/T, mob/user)
	owner.visible_message("<span class='warning'>A giant cross falls down on the ground!</span>")
	...
	for(var/mob/living/L in view(damage_range, T))
		...
		L.apply_damage((damage_amount - distance_decrease), BRAIN, null, L.run_armor_check(null, BRAIN))
		new /obj/effect/temp_visual/revenant(get_turf(L))

/* Knight of Despair - Quenched with Blood */
/datum/action/cooldown/spell_action/aimed/despair_swords
	...
	button_icon_state = "despair0"
	base_icon_state = "despair"
	cooldown_time = 20 SECONDS

	var/swords = 2

/datum/action/cooldown/spell_action/aimed/despair_swords/Activate(atom/target)
	var/turf/target_turf = get_turf(target)
	var/list/OT = get_adjacent_open_turfs(owner)
	for(var/i = 1 to swords)
		if(OT.len <= 0)
			OT = get_adjacent_open_turfs(owner)
		var/turf/T = pick(OT)
		OT -= T
		...
		RP.set_angle(get_angle(T, target_turf))
		addtimer(CALLBACK (RP, TYPE_PROC_REF(/obj/projectile, fire)), 3)
	sleep(3)
	playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/despairknight/attack.ogg', 50, 0, 4)
	return ..()
...
/obj/projectile/despair_rapier/ego/on_hit(atom/target, blocked = FALSE, pierce_hit)
	if(ishuman(target))
		return
	nodamage = FALSE
	if(ishostile(target))
		...
		H.apply_damage(max(0.05 * H.maxHealth, 120), TOX)
	..()
	qdel(src)

/* Queen of Hatred - Love and Justice */
/datum/action/cooldown/spell_action/aimed/arcana_slave
	name = "Arcana Slave"
	...
	button_icon_state = "arcana0"
	base_icon_state = "arcana"
	cooldown_time = 3 MINUTES
	//base_action = /datum/action/cooldown/spell_action/ability/item/ego_arcana_slave
...
/datum/action/cooldown/spell_action/aimed/arcana_slave/New(Target, original)
	. = ..()
	beamloop = new
	beamloop.volume = 40
	custom_speech = list()
...
/datum/action/cooldown/spell_action/aimed/arcana_slave/Destroy()
	. = ..()
	QDEL_NULL(beamloop)

/datum/action/cooldown/spell_action/aimed/arcana_slave/Activate(atom/target)
	var/turf/t_turf = get_turf(target)
	INVOKE_ASYNC(src, PROC_REF(Cast), t_turf, owner)
	return ..()

/datum/action/cooldown/spell_action/aimed/arcana_slave/proc/Cast(turf/target, mob/user)
	if(!ishuman(user))
		return
	...
	for(var/i = 1 to 3)
		...
		spawned_effects += S
		playsound(H, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/beam[clamp(i, 1, 2)].ogg", 50, FALSE, 4*i)
	...
	var/list/hit_line = get_line(my_turf, TT)
	beamloop.start(user)
...
/datum/action/cooldown/spell_action/aimed/arcana_slave/proc/Channel(mob/user, duration = 0)
	if(!duration)
		return FALSE
	if(!user)
		return FALSE
...
/datum/action/cooldown/spell_action/aimed/arcana_slave/proc/CleanUp(mob/user)
...
/datum/action/cooldown/spell_action/ability/item/ego_arcana_slave

/datum/action/cooldown/spell_action/ability/item/ego_arcana_slave/New(Target)
	. = ..()
	//button.Destroy()
...
/atom/movable/screen/movable/action_button/ego_arcana_slave/Click(location, control, params)
	if(!istype(linked_action, /datum/action/cooldown/spell_action/ability/item/ego_arcana_slave))
		return ..()
	var/datum/action/cooldown/spell_action/ability/item/ego_arcana_slave/act = linked_action
	if(!istype(act.target, /datum/action/cooldown/spell_action/aimed/arcana_slave))
		return ..()
	var/datum/action/cooldown/spell_action/aimed/arcana_slave/AS = act.target
...
/datum/action/cooldown/spell_action/aimed/gleaming_eyes/Activate(atom/target)
	if(get_dist(owner, target) > 10 || !(target in view(9, owner)))
		return
	playsound(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spider_bud/jump.ogg', 50, FALSE, -1)
	var/turf/target_turf = get_turf(target)
	//new /mob/living/simple_animal/cocoonability(target_turf) FUCK YOU NO COCOON!!!!!!!!!!!
	animate(owner, alpha = 1,pixel_x = 0, pixel_z = 16, time = 0.1 SECONDS)
	owner.pixel_z = 16
	sleep(0.5 SECONDS)
	for(var/i in 2 to get_dist(owner, target_turf))
		step_towards(owner,target_turf)
	animate(owner, alpha = 255,pixel_x = 0, pixel_z = -16, time = 0.1 SECONDS)
	owner.pixel_z = 0
	sleep(0.1 SECONDS)
	JumpAttack(target_turf,owner)
	return ..()

/datum/action/cooldown/spell_action/aimed/gleaming_eyes/proc/JumpAttack(target, mob/user)
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/spider_bud/land.ogg', 50, FALSE, -1)
	...
	for(var/mob/living/L in view(damage_range, user))
		...
		if(L.stat == DEAD)
			continue
		L.apply_damage(ishuman(L) ? damage_amount*0.5 : damage_amount, BURN, null, L.run_armor_check(null, BURN), spread_damage = TRUE)
		//if(ishostile(L))
...
/mob/living/simple_animal/cocoonability/proc/SplashEffect()
	...
	for(var/mob/living/L in view(damage_range, src))
		if(src.faction_check_atom(L, FALSE))
			continue
		if(L.stat == DEAD)
			continue
		L.apply_damage(ishuman(L) ? damage_amount*0.5 : damage_amount, BURN, null, L.run_armor_check(null, BURN), spread_damage = TRUE)
...
/datum/action/cooldown/spell_action/aimed/blackhole
	name = "blackhole"
	desc = "An ability that allows its user to summon a black hole to drag everyone who is hostile near it."
	button_icon_state = "blackhole0"
	base_icon_state = "blackhole"
	cooldown_time = 30 SECONDS

/datum/action/cooldown/spell_action/aimed/blackhole/Activate(atom/target)
	if(get_dist(owner, target) > 10 || !(target in view(9, owner)))
		return
	var/turf/target_turf = get_turf(target)
...
/obj/projectile/black_hole_realized/proc/SplashEffect()
	...
	for(var/mob/living/L in view(damage_range, src))
		if(ishostile(L))
			...
			L.apply_damage(ishuman(L) ? (damage_amount - distance_decrease)*0.5 : (damage_amount - distance_decrease), BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/datum/action/cooldown/spell_action/aimed/yin_laser
	name = "Anarchy"
	...
	button_icon_state = "yinform0"
	base_icon_state = "yinform"
	cooldown_time = 60 SECONDS
	var/laser_range = 20

/datum/action/cooldown/spell_action/aimed/yin_laser/Activate(atom/target)
	var/turf/t_turf = get_turf(target)
	INVOKE_ASYNC(src, PROC_REF(Cast), t_turf, owner)
	return ..()

/datum/action/cooldown/spell_action/aimed/yin_laser/proc/Cast(turf/target, mob/living/carbon/human/user)
	user.face_atom(target)
	var/turf/my_turf = get_turf(user)
	var/turf/target_turf = get_ranged_target_turf_direct(user, target, laser_range)
	var/list/to_hit = get_line(user, target_turf)
	...
	for(var/turf/T in range(1, src))
		...
		for(var/mob/living/L in T)
			if(faction_check(L.faction, src.faction))
				continue
			L.apply_damage(damage, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/datum/action/cooldown/spell_action/aimed/house_spawn
	...
	button_icon_state = "home_spawn0"
	base_icon_state = "home_spawn"
	cooldown_time = 20 SECONDS
	var/damage_amount = 300
	var/damage_range = 10

/datum/action/cooldown/spell_action/aimed/house_spawn/Activate(atom/target)
	if(!istype(owner))
		return
	if(get_dist(owner, target) > 10)
		return
	...
	playsound(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_MakeRoad.ogg', 50, FALSE, 8)
	addtimer(CALLBACK(src, PROC_REF(HouseSlam), target_turf, owner), 1 SECONDS)
	return ..()

/datum/action/cooldown/spell_action/aimed/house_spawn/proc/HouseSlam(turf/T, mob/user)
	//visible_message("<span class='warning'>A giant House falls down on the ground!</span>")
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_HouseBoom.ogg', 75, FALSE, 10, falloff_exponent = 0.75) //LOUD
	for(var/turf/open/TF in view(damage_range, T))
		new /obj/effect/temp_visual/small_smoke/halfsecond(TF)
	for(var/mob/living/L in view(damage_range, T))
		if(user.faction_check_atom(L))
			...
			continue
		var/distance_decrease = get_dist(T, L) * 10
		L.apply_damage((damage_amount - distance_decrease), BRUTE, null, L.run_armor_check(null, BRUTE))
		new /obj/effect/temp_visual/revenant(get_turf(L))
