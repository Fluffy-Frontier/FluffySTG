...
/datum/ordeal/brown_dawn
	...
	level = 1
	reward_percent = 0.1
	announce_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown_start.ogg'
	end_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown_end.ogg'
	color = "#69350b"
	var/spawn_places = 6
	var/spawn_amount = 2
...
/datum/ordeal/brown_dawn/Run() // So basically we want to spawn a crap ton of enemies - Identical to simplespawn, but one type per group
	..()
	var/place_player_mod = round(living_player_count() * place_player_multiplicator) // Ten players add a new spot
	var/spawn_player_mod = round(living_player_count() * spawn_player_multiplicator)
