...
/datum/controller/subsystem/lobotomy_events/proc/OnAbnoBreach(datum/source, mob/living/simple_animal/hostile/abnormality/abno)
	...
	if(abno.type in YY_types)
		...
		for(var/mob/living/simple_animal/hostile/abnormality/A in GLOB.abnormality_mob_list)
			...
			if(!A.IsContained())
				continue
			INVOKE_ASYNC(A, TYPE_PROC_REF(/mob/living/simple_animal/hostile/abnormality, qliphoth_change), -999)
			break
	return
...
/datum/controller/subsystem/lobotomy_events/proc/SpawnEvent(event_type = 0)
	...
	switch(event_type)
		if(APOCALYPSE)
			...
			for(var/mob/living/simple_animal/hostile/abnormality/A in AB_breached)
				if(A.IsContained())
					A.qliphoth_change(-999) // Breach it!
				...
				A.Goto(get_turf(portal), TRUE)
				A.density = FALSE // They ignore you and walk past you.
				A.AIStatus = AI_OFF
				A.ChangeResistances(list(BRUTE = 0, BURN = 0, BRAIN = 0, BRUTE = 0, TOX = 0)) // You can kill the portal but not them.
...
/datum/controller/subsystem/lobotomy_events/proc/SpawnEventAbnos(event_type = 0)
	...
	for(var/datum/abnormality/abno_ref in SSlobotomy_corp.all_abnormality_datums) //Now that the abnormalities are spawned, breach them
		if(abno_ref.abno_path in type_list)
			abno_ref.current.qliphoth_change(-999)
	return
