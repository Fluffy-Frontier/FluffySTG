...
/datum/ai_controller/insane/get_able_to_run()
...
/datum/ai_controller/insane/SelectBehaviors(seconds_per_tick)
	current_behaviors = list()
	if(ResistCheck() && prob(resist_chance))
...
/datum/ai_controller/insane/murder/PossessPawn(atom/new_pawn)
	...
	total_locations |= GLOB.generic_event_spawns // Avoids Department centers, unlike Wander Panic
	var/mob/living/L = pawn
	L.active_hand_index = 1

/datum/ai_controller/insane/murder/set_movement_target(source, atom/target, datum/ai_movement/new_movement)
	timerid = null
	var/mob/living/living_pawn = pawn
	...
	if(the_mecha)
		...
		if(initial(boolet.damage_type) == BRAIN && ishuman(selected_enemy))
	...
	timerid = addtimer(CALLBACK(src, PROC_REF(set_movement_target)), move_mod, TIMER_STOPPABLE)
...
/datum/ai_controller/insane/murder/SelectBehaviors(delta_time)
	...
	if(selected_enemy && is_mecha)
		...
		if(HAS_TRAIT(living_enemy, TRAIT_GODMODE))
...
/datum/ai_controller/insane/murder/proc/TryFindWeapon(is_white_allowed = TRUE)
	...
	for(var/obj/item/I in living_pawn.held_items)
		...
		if(istype(EG))
			...
			if(!is_white_allowed)
				...
				if(initial(boolet.damage_type) == BRAIN)
					continue
		if(!is_white_allowed && I.damtype == BRAIN)
			continue
	...
	var/list/items_to_check = living_pawn.get_equipped_items()
...
/proc/IsBetterWeapon(mob/living/carbon/human/H, obj/item/I, current_highest_force, considerRangedAttack = TRUE, applyJustice = FALSE)
	var/justice = applyJustice ? H.get_clothing_class_level(CLOTHING_ARMORED) : 0
...
/datum/ai_controller/insane/murder/proc/PossibleEnemies(range = aggro_range)
	...
	if(!isturf(living_pawn.loc))
		sees_from = pawn.loc
	var/list/potential_enemies = view(range, sees_from)
	. = list()
...
/datum/ai_controller/insane/murder/proc/CanTarget(atom/movable/thing)
	...
	if(isliving(thing))
		...
		if(living_thing == living_pawn)
			return FALSE
		if(HAS_TRAIT(living_thing, TRAIT_GODMODE))
			return FALSE
...
/datum/ai_controller/insane/murder/proc/FindEnemies(range = aggro_range)
	...
	if(weighted_list.len > 0)
		GiveTarget(pick_weight(weighted_list))
		return TRUE
	return FALSE
...
/datum/ai_controller/insane/murder/on_attackby(datum/source, obj/item/I, mob/living/carbon/human/user)
	..()
	retaliate(user)
	var/aggro = I.force
	if(ishuman(user))
		aggro *= 1 + user.get_clothing_class_level(CLOTHING_ARMORED)
...
/datum/ai_controller/insane/murder/on_hitby(datum/source, atom/movable/AM, skipcatch = FALSE, hitpush = TRUE, blocked = FALSE, datum/thrownthing/throwingdatum)
	...
	//	var/obj/item/I = AM
		//if(I.throwforce > 0)
			...
			//retaliate(H)
			//var/aggro = I.throwforce * (1 + H.get_clothing_class_level(CLOTHING_ARMORED))
...
/datum/ai_controller/insane/murder/on_Crossed(datum/source, atom/movable/AM)
	...
	if(istype(living_thing) && !(HAS_TRAIT(living_thing, TRAIT_GODMODE)) && living_thing.stat != DEAD)
...
//	var/mob/living/carbon/human/human_pawn = pawn
//	var/suicide_target = 6 + round(human_pawn.get_clothing_class_level(CLOTHING_SCIENCE) / 0.4)
//	if(prob(10))
//		current_behaviors += GET_AI_BEHAVIOR(lines_type)
//		suicide_timer += 1
...
//		var/sanity_damage = human_pawn.get_clothing_class_level(human_pawn.get_major_clothing_class()) * 70
...
/datum/ai_controller/insane/wander/PossessPawn(atom/new_pawn)
	...
	total_locations |= GLOB.generic_event_spawns
