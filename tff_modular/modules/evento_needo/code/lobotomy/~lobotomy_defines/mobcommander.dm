...
/datum/controller/subsystem/mobcommander/proc/CheckLedger()
	//Remove duplicates.
	var/fixed_leader_list = unique_list(leaders)
	var/fixed_follower_list = unique_list(follower_list)
	//Replace list with a sorted fixed version.
	leaders = sort_key(fixed_leader_list)
	follower_list = sort_key(fixed_follower_list)
	//Remove nulls.
	list_clear_nulls(leaders)
	list_clear_nulls(follower_list)
