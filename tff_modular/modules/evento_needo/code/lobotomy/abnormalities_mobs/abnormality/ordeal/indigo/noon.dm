...
/mob/living/simple_animal/hostile/ordeal/indigo_noon/Initialize()
	. = ..()
	attack_sound = "tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/indigo/stab_[pick(1,2)].ogg"
	...
	if(health <= maxHealth * 0.8)
		for(var/turf/adjacent_turf in orange(1, src))
			for(var/mob/maybe_sweeper_corpse in adjacent_turf)
				if(faction_check_atom(maybe_sweeper_corpse) && maybe_sweeper_corpse.stat == DEAD)
...
/mob/living/simple_animal/hostile/ordeal/indigo_noon/lanky/Initialize()
	...
	icon_living = "sweeper_limbus"
	icon_state = icon_living
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/indigo/stab_2.ogg'
...
/mob/living/simple_animal/hostile/ordeal/indigo_noon/lanky/proc/SweepTheBackstreets(atom/prospective_fuel = target)
	...
	SLEEP_CHECK_DEATH(dash_windup, src)
	/// We're now dashing.
	BeginDash()
	walk_towards(src, dash_target_turf, dash_speed)
	SLEEP_CHECK_DEATH(get_dist(src, dash_target_turf) * dash_speed, src)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/knife3.ogg', 100, FALSE, 4)
...
/mob/living/simple_animal/hostile/ordeal/indigo_noon/chunky/proc/PrepareExtractFuel()
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/finisher1.ogg'
...
/datum/status_effect/stacking/sweeper_persistence/on_apply()
	...
	overlay = mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "sweeper_persistence", -MUTATIONS_LAYER)
	overlay.pixel_x = 4
	overlay.pixel_y = icon_height - 28
	if(icon_height == 32)
		overlay.transform *= 0.80
	owner.add_overlay(overlay)
	RegisterSignal(owner, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(CheckDeath))
...
/datum/status_effect/stacking/sweeper_persistence/proc/CheckDeath(datum_source, amount, damagetype, def_zone)
	...
	var/overkill_damage = amount - neighbor.health
	...
	if(amount >= neighbor.health)
		/// But it refused. Persistence goes off, we heal a tiny bit and lose a stack
		if(prob(final_chance))
			playsound(neighbor, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/indigo_start.ogg', 33)
