...
/mob/living/simple_animal/hostile/ordeal/NT_corrosion
	...
	damage_coeff = list(BURN = 0, BRAIN = 0.8, BRUTE = 0.8, TOX = 1.3)
...
/mob/living/simple_animal/hostile/ordeal/NT_corrosion/proc/ReflectDamage(mob/living/attacker, attack_type = BURN, damage)
	if(damage < 1)
		return
	...
	forceMove(jump_turf)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/sword1.ogg', min(15 + damage, 100), TRUE, 4)
...
/mob/living/simple_animal/hostile/ordeal/NT_corrosion/attack_hand(mob/living/carbon/human/M)
	...
	if(damage_reflection && M.combat_mode)
		ReflectDamage(M, BRUTE, M?.melee_damage_upper)
...
/mob/living/simple_animal/hostile/ordeal/NT_corrosion/proc/StartReflecting()
	...
	damage_taken = 0
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/breach.ogg', 25, 0, 5)
	...
	ChangeResistances(list(BURN = 0, BRAIN = 0.8, BRUTE = 0.8, TOX = 1.3))
	damage_reflection = FALSE
	can_act = TRUE
...
/mob/living/simple_animal/hostile/ordeal/snake_corrosion
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 1, BRAIN = 1, BRUTE = 0, TOX = 1.5)
	...
	for(var/turf/T in view(poison_range, target_turf))
		...
		for(var/mob/living/H in T)
			if(faction_check_atom(H))
				continue
			H.apply_damage(poison_damage, BRUTE, null, H.run_armor_check(null, BRUTE), spread_damage = TRUE)
			H.apply_venom(2)

/mob/living/simple_animal/hostile/ordeal/snake_corrosion/apply_damage(damage, damagetype, def_zone, blocked, forced, spread_damage, wound_bonus, bare_wound_bonus, sharpness, attack_direction, attacking_item, exposed_wound_bonus)
	. = ..()
	if(poison_releasing)
		return
...
/datum/status_effect/stacking/venom/can_have_status()
	return (owner.stat != DEAD || !(HAS_TRAIT(owner, TRAIT_GODMODE)))

/datum/status_effect/stacking/venom/on_apply()
	. = ..()
	owner.playsound_local(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/venom_apply.ogg', 50, TRUE)
...
/datum/status_effect/stacking/venom/tick()
	if(!can_have_status())
		qdel(src)
	owner.playsound_local(owner, 'tff_modular/modules/evento_needo/sounds/Tegusounds/venom.ogg', 50, TRUE)
	if(ishuman(owner))
...
/mob/living/simple_animal/hostile/ordeal/dog_corrosion
	...
	attack_verb_continuous = "slices"
	attack_verb_simple = "slice"
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/big_wolf/Wolf_Scratch.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 1.2, BRAIN = 0.7, BRUTE = 0.7, TOX = 1.5)
	...
	var/dash_damage = 80
	var/charge_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/gold/growl1.ogg'
	var/gibbing = TRUE
...
/mob/living/simple_animal/hostile/ordeal/dog_corrosion/proc/Charge(move_dir, times_ran)
	...
	for(var/mob/living/L in range(1, T))//damage applied to targets in range
		...
		L.apply_damage(dash_damage,BURN, null, L.run_armor_check(null, BURN), spread_damage = TRUE)
		been_hit += L
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/cromer_stab.ogg', 75, 1)
		if(!ishuman(L))
			continue
...
/mob/living/simple_animal/hostile/ordeal/dog_corrosion/strong
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.5, BRUTE = 0.5, TOX = 1.3)
	butcher_results = list(/obj/item/food/meat/slab/human = 3)
	guaranteed_butcher_results = list(/obj/item/food/meat/slab/human = 1)
	dash_num = 30
	...
	dash_damage = 100
	dash_count = 3
	charge_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/gold/growl2.ogg'
...
/obj/projectile/poisonglob/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	if(isliving(target))
