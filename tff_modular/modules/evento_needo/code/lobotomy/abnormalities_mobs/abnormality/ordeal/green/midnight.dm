...
/mob/living/simple_animal/hostile/ordeal/green_midnight/apply_damage(damage, damagetype, def_zone, blocked, forced, spread_damage, wound_bonus, bare_wound_bonus, sharpness, attack_direction, attacking_item, exposed_wound_bonus)
	. = ..()
	if(stat == DEAD)
		return
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/OpenShell()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_gears.ogg', 50, FALSE, 7)
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/CloseShell()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_gears_fast.ogg', 50, FALSE, 7)
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/SetupLaser()
	...
	for(var/i = 1 to max_lasers)
		...
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_laser_new.ogg', 50 + 5 * (i - max_lasers), FALSE)
		...
		for(var/attempt = 1 to 3) // Just so that we don't get ourselves absolutely the same angle twice in a row
			...
			if((new_angle > old_angle + 15) || (new_angle < old_angle - 15))
				break
		SLEEP_CHECK_DEATH(laser_spawn_delay, src)
	SLEEP_CHECK_DEATH(laser_rotation_time, src)
	FireLaser()
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/PrepareLaser(obj/effect/greenmidnight_laser/L, l_angle)
	if(stat == DEAD || QDELETED(L))
		return
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_gears_fast.ogg', 15, TRUE)
	animate(L, transform = turn(matrix(), l_angle), time = laser_rotation_time)
	SLEEP_CHECK_DEATH(laser_rotation_time * 0.75, src)
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/FireLaser()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_laser_prepare.ogg', 75, FALSE, 14)
	SLEEP_CHECK_DEATH(2.5 SECONDS, src)
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_laser_warning.ogg'))
	SLEEP_CHECK_DEATH(1.5 SECONDS, src)
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_laser_fire.ogg'))
	var/turf/T = get_turf(src)
	for(var/datum/beam/B in beams)
		QDEL_NULL(B)
	beams = list()
		...
		for(var/turf/T in hit_line)
			for(var/mob/living/L in range(1, T))
				...
				if(faction_check_atom(L))
					continue
				already_hit += L
				L.apply_damage(laser_damage, BRUTE, null, L.run_armor_check(null, BRUTE))
		SLEEP_CHECK_DEATH(0.25 SECONDS, src)
	StopLaser()

/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/StopLaser()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green/midnight_laser_end.ogg', 75, FALSE, 24)
	laserloop.stop()
	for(var/datum/beam/B in beams)
		QDEL_NULL(B)
	beams = list()
...
/obj/effect/temp_visual/helix_minilaser/proc/Blowup()
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/rcorp4.ogg', 15, FALSE, 4)
	for(var/mob/living/H in src.loc)
		if(!faction_check(H.faction, list("green_ordeal")))
			H.apply_damage(55, BRUTE, null, H.run_armor_check(null, BRUTE))
...
/obj/effect/temp_visual/helix_macrolaser/proc/Blowup()
	...
	transform *= 2.5
	pixel_y += 80
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/sorrow_shot.ogg', 65, TRUE, 4)
	for(var/mob/living/H in range(3, src))
		var/distance = get_dist(src, H)
		if(distance < 2)
			if(!faction_check(H.faction, list("green_ordeal")))
				H.apply_damage(50, BRUTE, null, H.run_armor_check(null, BRUTE))
				to_chat(H, span_userdanger("You're hit by [src.name]!"))
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/DeployBotSquad()
	...
	playsound(src, 'sound/items/fulton/fultext_launch.ogg', 90, TRUE, 10)
	...
	for(var/turf/place in deployment_points)
		...
		if(!is_in_sight(src, place) || place.is_blocked_turf(TRUE) || istype(place, /turf/open/lava))
...
/mob/living/simple_animal/hostile/ordeal/green_midnight/proc/HandleScaling()
	...
	if(LAZYLEN(GLOB.alive_player_list))
		for(var/mob/gremlin in GLOB.alive_player_list)
			base_squad_size += 2
