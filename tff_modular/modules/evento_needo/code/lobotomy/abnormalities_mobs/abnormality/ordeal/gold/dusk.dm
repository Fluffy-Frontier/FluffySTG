...
/mob/living/simple_animal/hostile/ordeal/centipede_corrosion
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_peck.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 0.8, BRAIN = 1, BRUTE = 0.5, TOX = 0.7)
	move_to_delay = 3
	var/pulse_cooldown
...
/mob/living/simple_animal/hostile/ordeal/centipede_corrosion/proc/Pulse()//Periodic weak AOE attack, gain charge constantly
	...
	if(charge_level < 5 || broken)
		return
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energy2.ogg', 10, FALSE, 3)
...
/mob/living/simple_animal/hostile/ordeal/centipede_corrosion/proc/PulseHit(turf/T)
	...
	HurtInTurf(T, list(), 5, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
	for(var/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion/TB in T)
		if(TB.charge_level >= TB.charge_level_cap)
			continue
		TB.AdjustCharge(4)
		playsound(get_turf(TB), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energy3.ogg', 75, FALSE, 3)
...
/mob/living/simple_animal/hostile/ordeal/centipede_corrosion/apply_damage(damage = 0,damagetype = BRUTE, def_zone = null, blocked = FALSE, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, sharpness = NONE, attack_direction, attacking_item, exposed_wound_bonus)
...
/mob/living/simple_animal/hostile/ordeal/centipede_corrosion/proc/Recharge()
	...
	for(var/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion/battery in view(8, src))
		...
		AdjustCharge(5)
		foundbattery = TRUE
		playsound(get_turf(battery), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energy2.ogg', 10, FALSE, 3)
		break
	if(foundbattery)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/energy3.ogg', 100, FALSE, 3)
	...
	//SLEEP_CHECK_DEATH(4 SECONDS, src)
	can_act = TRUE
	update_icon()
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_zombieattack.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 0.8, BRAIN = 0.5, BRUTE = 1, TOX = 0.7)
	move_to_delay = 3
	ranged = TRUE
	projectiletype = /obj/projectile/thunder_tomahawk
	projectilesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_peck.ogg'
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	. = ..()
	charge_progress += 1
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion/proc/Dash(atom/A, dist) // We move up to 10 tiles, using our charge as a battery.
	var/list/target_line = get_line(A, src) // can we reach them in the first place?
	var/available_turf
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion/proc/Convert(mob/living/carbon/human/H)
	if(!istype(H))
		return
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_zombify.ogg', 45, FALSE, 5)
...
/mob/living/simple_animal/hostile/ordeal/KHz_corrosion
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_peck.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 0.8, BRAIN = 0.5, BRUTE = 1, TOX = 1)
	move_to_delay = 4
	...
	var/list/radio_sounds = list(
		'sound/items/radio/radio_important.ogg',
		'sound/items/radio/radio_receive.ogg',
		'sound/items/radio/radio_talk.ogg'
		)
	var/list/damage_sounds = list(
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip1.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip2.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip3.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip4.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip5.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip6.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip7.ogg'
	)
...
/mob/living/simple_animal/hostile/ordeal/KHz_corrosion/proc/Screech()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/gold/radiostatic.ogg', 100, TRUE, 8)
	...
	for(var/turf/T in view(7, src))
		HurtInTurf(T, been_hit, 25, BRUTE, null, TRUE, FALSE, TRUE, hurt_hidden = TRUE)
	sleep(3)
	if(QDELETED(src) || stat == DEAD)
		if(!QDELETED(src))
			icon = initial(icon)
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion_boss/proc/ThunderWave(range_override = null)
	...
	been_hit = list()
	playsound(src, "tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/gold/weather_thunder_[pick(0,3)].ogg", 100, FALSE, range_override)
...
/mob/living/simple_animal/hostile/ordeal/thunderbird_corrosion_boss/proc/ThunderStrike(mob/living/L, attack_range = 1)
	...
	playsound(get_turf(L), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/thunderbird/tbird_charge.ogg', 15, 1, 4)
	L.add_overlay(icon('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "electricity"))
	addtimer(CALLBACK(L, TYPE_PROC_REF(/atom, cut_overlay), \
							icon('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "electricity")), 55)
