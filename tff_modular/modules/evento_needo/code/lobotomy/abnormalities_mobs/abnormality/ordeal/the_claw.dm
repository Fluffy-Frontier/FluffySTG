/mob/living/simple_animal/hostile/megafauna/claw
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/attack.ogg'
	icon_state = "claw"
	icon_living = "claw"
	icon_dead = "claw_dead"
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/death.ogg'
...
/datum/action/innate/megafauna_attack/serum_w
	name = "Serum 'W'"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi'
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/eviscerate(mob/living/carbon/human/target)
	...
	uhoh.orbit(target, 0)
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate1.ogg', 100, 1)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate1.ogg', 1, 1)
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/eviscerate2(mob/living/carbon/human/target, obj/effect/eff)
	...
	if(prob(2) || target.z != z || !target.loc.AllowClick()) // Be happy, mortal. Did you just hide in a locker?
		...
		playsound(src.loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/error.ogg', 50, 1)
		qdel(eff)
		return
	...
	forceMove(tp_loc)
	qdel(eff)
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate2.ogg', 100, 1)
	GiveTarget(target)
	for(var/mob/living/L in range(1, get_turf(src))) // Attacks everyone around.
		...
		L.apply_damage(75, BRUTE, null, L.run_armor_check(null, BRUTE))
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/TargetSerumW(mob/living/L)
	...
	uhoh.orbit(L, 0)
	playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate1.ogg', 100, 1)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/prepare.ogg', 1, 1)
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/TargetEviscerate(mob/living/L, obj/effect/eff)
	...
	forceMove(tp_loc)
	face_atom(L)
	playsound(tp_loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/move.ogg', 100, 1)
	L.Stun(12, TRUE, TRUE)
	...
	for(var/turf/T in shuffle(GLOB.generic_event_spawns))
	...
	for(var/i = 1 to 5)
		...
		playsound(tp_loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate2.ogg', 100, 1)
		tp_loc.Beam(target_turf, "nzcrentrs_power", time=15)
		playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate2.ogg', 100, 1)
		var/obj/effect/temp_visual/decoy/D = new /obj/effect/temp_visual/decoy(loc, src)
		D.color = COLOR_BRIGHT_BLUE
		animate(D, alpha = 0, time = 5)
		forceMove(target_turf)
		for(var/mob/living/LL in range(1, target_turf)) // Attacks everyone around.
			if(faction_check_atom(LL))
				continue
			if(LL == L)
				continue
			to_chat(LL, span_userdanger("\The [src] slashes you!"))
			LL.apply_damage(15, BRUTE, null, LL.run_armor_check(null, BRUTE))
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/triserum_eviscerate2(mob/living/carbon/human/target, obj/effect/eff)
	...
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/eviscerate2.ogg', 100, 1, 7)
	GiveTarget(target)
	for(var/mob/living/L in range(1, get_turf(src))) // Attacks everyone around.
		if(L in been_hit)
			continue
		...
		been_hit |= L
		to_chat(target, span_userdanger("\The [src] eviscerates you!"))
		L.apply_damage(70, TOX, null, L.run_armor_check(null, TOX))
		new /obj/effect/temp_visual/cleave(get_turf(L))
	for(var/turf/B in get_line(prev_loc, tp_loc))
		for(var/mob/living/L in range(1, B)) // Attacks everyone in line
			...
			L.apply_damage(50, TOX, null, L.run_armor_check(null, TOX))
			playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/attack.ogg', 35, 1)
			new /obj/effect/temp_visual/cleave(get_turf(L))
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/SwiftDash(atom/target, distance, wait_time)
	...
	var/list/turf_list = get_line(src, end_turf)
	for(var/turf/T in turf_list)
		new /obj/effect/temp_visual/cult/sparks(T)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/prepare.ogg', 100, 1)
	icon_state = "claw_prepare"
	face_atom(target)
	...
	for(var/turf/T in turf_list)
		...
		forceMove(T)
		playsound(src,'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/move.ogg', 50, 1)
		for(var/mob/living/L in HurtInTurf(T, list(), dash_damage, BURN, check_faction = TRUE, hurt_mechs = TRUE))
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/blink(mob/living/LT)
	...
	for(var/turf/T in get_line(start_turf, target_turf))
	...
	if(!istype(LT) || QDELETED(LT))
		return
	forceMove(target_turf)
	playsound(src,'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/move.ogg', 100, 1)
	for(var/turf/B in get_line(start_turf, target_turf))
		for(var/turf/TT in range(B, 1))
			...
			for(var/mob/living/victim in TT)
				...
				victim.apply_damage(serumA_damage, BURN, null, victim.run_armor_check(null, BURN))
				new /obj/effect/temp_visual/cleave(victim.loc)
				playsound(victim, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/attack.ogg', 35, 1)
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/WideSlash(atom/target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/prepare.ogg', 100, 1)
	icon_state = "claw_dash"
	SLEEP_CHECK_DEATH(0.7 SECONDS, src)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/claw/move.ogg', 100, 1)
	icon_state = "claw_prepare"
...
/mob/living/simple_animal/hostile/megafauna/claw/proc/DoLineAttack(list/line)
	for(var/turf/T in line)
		...
		for(var/mob/living/L in T)
			...
			if(faction_check_atom(L))
				continue
			L.apply_damage(wide_slash_damage, BRAIN, null, L.run_armor_check(null, BRAIN))
