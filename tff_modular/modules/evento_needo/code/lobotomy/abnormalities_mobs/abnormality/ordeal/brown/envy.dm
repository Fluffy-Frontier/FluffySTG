...
/mob/living/simple_animal/hostile/ordeal/sin_envy
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/club3.ogg'
...
/mob/living/simple_animal/hostile/ordeal/sin_envy/say(message, bubble_type, list/spans, sanitize, datum/language/language, ignore_spam, forced, message_mods, message_range, saymode)
	if(stat == DEAD)
		return
	if(!client)
		message = stars(message, 100)
		playsound(src, "tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/envy[pick(1,2,3)].ogg", 75, FALSE, 4)
	. = ..()
...
/mob/living/simple_animal/hostile/ordeal/sin_envy/proc/CopyPlayer()
	...
	if(target_mob)
		...
		name = generate_random_name(gender, TRUE)
