...
/mob/living/simple_animal/hostile/ordeal/crimson_clown
	...
	density = FALSE
	search_objects = 3
	damage_coeff = list(BURN = 0.8, BRAIN = 1.2, BRUTE = 1.3, TOX = 2)
...
/mob/living/simple_animal/hostile/ordeal/crimson_clown/proc/DeathExplosion()
	...
	for(var/mob/living/L in view(5, src))
		if(!faction_check_atom(L))
			L.apply_damage(35, BRUTE, null, L.run_armor_check(null, BRUTE))
	gib()
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/noon_dead.ogg'
	damage_coeff = list(BURN = 0.6, BRAIN = 1.2, BRUTE = 1.2, TOX = 1.5)
	blood_volume = BLOOD_VOLUME_NORMAL
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/dusk_attack.ogg'
	damage_coeff = list(BURN = 0.4, BRAIN = 1.2, BRUTE = 1.2, TOX = 1.5)
	mob_spawn_amount = 2
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk/DeathExplosion(gibbed = FALSE)
	if(QDELETED(src))
		return
	visible_message(span_danger("[src] suddenly explodes!"))
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/dusk_dead.ogg', 50, 1)
	var/valid_directions = list(0) // 0 is used by get_turf to find the turf a target, so it'll at the very least be able to spawn on itself.
	for(var/d in list(NORTH, SOUTH, EAST, WEST, NORTHEAST, NORTHWEST, SOUTHEAST, SOUTHWEST))
		var/turf/TF = get_step(src, d)
		if(!istype(TF))
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_dusk/proc/do_roll(move_dir, times_ran)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/dusk_move.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent
	...
	attack_verb_continuous = "bites"
	attack_verb_simple = "bite"
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dusk_attack.ogg'
	damage_coeff = list(BURN = 0.2, BRAIN = 1, BRUTE = 1, TOX = 1.5)
	var/spawn_time
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/Initialize()
	. = ..()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/midnight_appear.ogg', 50, FALSE)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/Life()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/midnight_spawn.ogg', 75, FALSE)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/death(gibbed)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/midnight_dead.ogg', 30, 0)
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/DeathExplosion()
	...
	for(var/mob/living/L in view(5, src))
		if(!faction_check_atom(L))
			L.apply_damage(700, BRUTE, null, L.run_armor_check(null, BRUTE))
	gib()
...
/mob/living/simple_animal/hostile/ordeal/crimson_tent/proc/Bite(target)
	...
	switch(dir_to_target)
		if(EAST)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, EAST, bite_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, bite_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, bite_width)))
					if (U.density)
						break
					...
					area_of_effect += U
		if(WEST)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, WEST, bite_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, bite_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, bite_width)))
					...
					area_of_effect += U
		if(SOUTH)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, SOUTH, bite_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, bite_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, bite_width)))
					if (U.density)
						break
					if (U in area_of_effect)
						continue
					area_of_effect += U
		if(NORTH)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, NORTH, bite_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, bite_width)))
	...
	can_act = FALSE
	dir = dir_to_target
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/ball.ogg', 75, 0, 5)
	for(var/turf/T in area_of_effect)
		new /obj/effect/temp_visual/cult/sparks(T)
	SLEEP_CHECK_DEATH(0.8 SECONDS, src)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/noon_bite.ogg', 100, 0, 5)
	for(var/turf/T in area_of_effect)
		...
		for(var/mob/living/L in T)
			...
			if (L == src)
				continue
			L.apply_damage(bite_damage, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_midnight //oh god why
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/midnight_slam.ogg'
	damage_coeff = list(BURN = 0.2, BRAIN = 1, BRUTE = 1, TOX = 1.5)
	mob_spawn_amount = 2
...
/mob/living/simple_animal/hostile/ordeal/crimson_noon/crimson_midnight/Initialize()
	...
	AddElement(/datum/element/knockback, 3, FALSE, TRUE) //1 is distance thrown, False is if it can throw anchored objects, True if doesnt apply damage or stun when hits a wall.
