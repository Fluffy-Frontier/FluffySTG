...
/mob/living/simple_animal/hostile/ordeal/amber_bug
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dawn_attack.ogg'
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dawn_dead.ogg'
	damage_coeff = list(BURN = 2, BRAIN = 1, BRUTE = 1, TOX = 2)
	...
	if(.)
		...
		for(var/i = 1 to 2)
			...
			if(ishuman(attacked_target) && can_infect)
				...
				var/parasite_slot = H.get_organ_slot(ORGAN_SLOT_PARASITE_EGG)
				if(H.stat != CONSCIOUS && !parasite_slot) //Only infect people in crit/dead and that don't have a parasite of some kind already.
					...
					playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dawn_dig_in.ogg', 25, 1)
...
/mob/living/simple_animal/hostile/ordeal/amber_bug/proc/BurrowOut(turf/T)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dawn_dig_out.ogg', 25, 1)
...
/obj/item/organ/amber_bug/proc/growProcess()
	...
	feeding_duration = world.time + (feeding_interval)
	feeding_stage++
	H.apply_damage(feeding_stage * 10, BRUTE, null, H.run_armor_check(null, BRUTE), spread_damage = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dusk_dead.ogg'
	damage_coeff = list(BURN = 1.2, BRAIN = 0.8, BRUTE = 0.5, TOX = 2)
	blood_volume = BLOOD_VOLUME_NORMAL
	/// This cooldown responds for both the burrowing and spawning in the dawns
	var/burrow_cooldown
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/proc/OnDirChange(atom/thing, dir, newdir)
	if(stat == DEAD)
		return
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/CanAttack(atom/the_target)
	var/relative_angle = abs(dir2angle(dir) - dir2angle(get_dir(src, the_target)))
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/revive(full_heal, excess_healing, force_grab_ghost)
	. = ..()
	density = TRUE
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/proc/AttemptBirth()
	...
	burrowing = TRUE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dusk_create.ogg', 50, FALSE)
	////SLEEP_CHECK_DEATH(5, src)
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/proc/BurrowIn()
	...
	if(!T)
		T = get_turf(src)
	visible_message(span_danger("[src] burrows into the ground!"))
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dusk_dig_in.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/ordeal/amber_dusk/proc/BurrowOut(turf/T)
	...
	animate(src, alpha = 255, time = 5)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/dusk_dig_out.ogg', 50, 1)
	visible_message(span_bolddanger("[src] burrows out from the ground!"))
	//(5, src)
	density = TRUE
...
/mob/living/simple_animal/hostile/ordeal/amber_midnight
	...
	health = 15000
	maxHealth = 15000
	damage_coeff = list(BURN = 1, BRAIN = 0.6, BRUTE = 0.4, TOX = 0.8)
	...
	blood_volume = BLOOD_VOLUME_NORMAL
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_dead.ogg'
...
/mob/living/simple_animal/hostile/ordeal/amber_midnight/proc/AttemptBirth()
	...
	burrowing = TRUE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_create.ogg', 50, FALSE)
...
/mob/living/simple_animal/hostile/ordeal/amber_midnight/proc/BurrowIn()
	burrowing = TRUE
	visible_message(span_danger("[src] burrows into the ground!"))
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_in.ogg', 50, FALSE, 7)
	icon_state = "ambermidnight_leave"
	new /obj/effect/temp_visual/ambermidnight_hole(get_turf(src))
	var/list/centers = GLOB.generic_event_spawns.Copy()
...
/mob/living/simple_animal/hostile/ordeal/amber_midnight/proc/BurrowOut()
	...
	alpha = 0
	density = FALSE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_out_far.ogg', 50, TRUE, 4)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_out.ogg', 75, FALSE, 7)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/amber/midnight_out_far.ogg', 25, FALSE, 24, 2, falloff_distance = 9)
	////SLEEP_CHECK_DEATH(2, src)
	density = TRUE
