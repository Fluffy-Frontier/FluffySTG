...
/mob/living/simple_animal/hostile/ordeal/steel_dawn
	...
	//similar to a human
	damage_coeff = list(BURN = 0.8, BRAIN = 1.2, BRUTE = 1, TOX = 1)

/mob/living/simple_animal/hostile/ordeal/steel_dawn/Initialize()
	. = ..()
	attack_sound = "tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/steel/gcorp_attack[pick(1,2,3)].ogg"
...
/mob/living/simple_animal/hostile/ordeal/steel_dawn/steel_noon
	...
	rapid_melee = 2
	damage_coeff = list(BURN = 0.8, BRAIN = 1, BRUTE = 0.8, TOX = 0.8)
...
/mob/living/simple_animal/hostile/ordeal/steel_dawn/steel_noon/proc/DeathExplosion()
	...
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/steel/gcorp_boom.ogg', 60, TRUE)
...
/mob/living/simple_animal/hostile/ordeal/steel_dawn/steel_noon/flying/Shoot(atom/A)
	...
	if(do_after(src, 2 SECONDS, target = src))
		...
		apply_damage(100, BRUTE, null, run_armor_check(null, BRUTE))
...
/mob/living/simple_animal/hostile/ordeal/steel_dawn/steel_noon/flying/proc/SweepAttack(mob/living/sweeptarget)
	...
	sweeptarget.apply_damage(30, BRUTE, null, run_armor_check(null, BRUTE))
...
/mob/living/simple_animal/hostile/ordeal/steel_dusk/handle_automated_movement()
	if(prob(25))
...
/mob/living/simple_animal/hostile/ordeal/steel_dusk/proc/Command(manager_order)
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/steel/gcorp_chitter.ogg', 60, TRUE)
	switch(manager_order)
		if(1)
			...
			for(var/mob/living/simple_animal/hostile/ordeal/G in oview(9, src))
				if(istype(G, /mob/living/simple_animal/hostile/ordeal/steel_dawn) && G.stat != DEAD)
					G.GiveTarget(target)
					G.set_varspeed(-1)
					addtimer(CALLBACK(G, PROC_REF(set_varspeed), 1), 1 SECONDS)
			last_command = 1
		if(2)
			say("Hold fast!")
			last_command = 2
...
/mob/living/simple_animal/hostile/ordeal/steel_dusk/proc/ManagerScreech()
	var/visual_overlay = mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "blip")
	...
	for(var/mob/living/L in oview(10, src))
		if(!faction_check_atom(L))
			L.apply_damage(120, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
