...
/mob/living/simple_animal/hostile/ordeal/white_lake_corrosion
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade3.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 0.8, BRAIN = 0.5, BRUTE = 1.5, TOX = 1)
	speed = 3
	move_to_delay = 3
...
/mob/living/simple_animal/hostile/ordeal/white_lake_corrosion/proc/Slash(target)
	...
	switch(dir_to_target)
		if(EAST)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, EAST, slash_length))
			for(var/turf/T in middle_line)
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, slash_width)))
					...
					area_of_effect += U
		if(WEST)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, WEST, slash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, slash_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, slash_width)))
					...
					area_of_effect += U
		if(SOUTH)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, SOUTH, slash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, slash_width)))
					...
					area_of_effect += U
		if(NORTH)
			middle_line = get_line(get_step_towards(source_turf, target), get_ranged_target_turf(source_turf, NORTH, slash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, slash_width)))
	...
	for(var/turf/T in area_of_effect)
		...
		for(var/mob/living/L in T)
			...
			if (L == src)
				continue
			HurtInTurf(T, list(), slash_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/white_lake_corrosion/proc/AreaAttack()
	...
	can_act = FALSE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge2.ogg', 100, 0, 5)
	for(var/turf/L in view(2, src))
		new /obj/effect/temp_visual/cult/sparks(L)
	SLEEP_CHECK_DEATH(12, src)
	for(var/turf/T in view(2, src))
		new /obj/effect/temp_visual/smash_effect(T)
		for(var/mob/living/L in HurtInTurf(T, list(), sweep_damage, BRUTE, null, TRUE, FALSE, TRUE, TRUE, TRUE, TRUE))
			if(L.health < 0)
				if(ishuman(L))
...
/mob/living/simple_animal/hostile/ordeal/white_lake_corrosion/proc/SpawnAdds()
	...
	playsound(get_turf(src), 'sound/mobs/humanoids/human/scream/femalescream_3.ogg', 75, 0, 4)
...
/mob/living/simple_animal/hostile/ordeal/silentgirl_corrosion
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.6, BRUTE = 1.5, TOX = 2)
	var/vengeful = FALSE
...
/mob/living/simple_animal/hostile/ordeal/silentgirl_corrosion/proc/TryTransform(mob/living/carbon/human/hooman)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/silentgirl/Guilt_Apply.ogg', 15, FALSE)
...
/datum/status_effect/gold_guilty/on_apply()
	...
	RegisterSignal(owner, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(DealWhite))
...
/datum/status_effect/gold_guilty/proc/DealWhite(mob/living/carbon/human/owner, damage, damagetype, def_zone)
	...
	H.apply_damage(damage_amt, BRUTE)
