/mob/living/simple_animal/hostile/ordeal/sin_sloth/noon
	...
	damage_coeff = list(BURN = 0.5, BRAIN = 1.2, BRUTE = 2, TOX = 1.5)
	move_to_delay = 6
	jump_aoe = 3
	jump_damage = 35
	jump_pixels = 8
...
/mob/living/simple_animal/hostile/ordeal/sin_gluttony/noon/proc/StrangleHit(count)
	...
	if(grab_victim.health < 0)
		...
		return
	do_attack_animation(get_step(src, dir), no_effect = TRUE)
	grab_victim.apply_damage(melee_damage_upper, BRUTE)
	new /obj/effect/temp_visual/damage_effect/rupture(get_turf(src))
	grab_victim.apply_damage(rupture_damage, BRUTE)
	grab_victim.Immobilize(10)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/flower_attack.ogg', 50, 0, 7)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/flower_kill.ogg', 50, 0, 7)
	switch(count)
		if(0 to 3)
		...
		if(4)	//Apply double damage
			...
			playsound(get_turf(src), 'sound/effects/wounds/crackandbleed.ogg', 200, 0, 7)
			grab_victim.apply_damage((rupture_damage * (3 - count)), BRUTE)
	count += 1
...
/mob/living/simple_animal/hostile/ordeal/sin_gluttony/noon/apply_damage(damage, damagetype, def_zone, blocked, forced, spread_damage, wound_bonus, bare_wound_bonus, sharpness, attack_direction, attacking_item, exposed_wound_bonus)
	. = ..()
	if(grab_victim)
...
/mob/living/simple_animal/hostile/ordeal/sin_gloom/noon
	...
	damage_coeff = list(BURN = 1.2, BRAIN = 0.5, BRUTE = 1.2, TOX = 1.5)
...
/mob/living/simple_animal/hostile/ordeal/sin_gloom/noon/OpenFire()
	...
	if(dist < 6)
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge2.ogg', 75, FALSE, 4)
...
/mob/living/simple_animal/hostile/ordeal/sin_gloom/noon/AreaAttack()
	...
	if(!can_act)
		return
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge.ogg', 60, TRUE)
	do_attack_animation(get_step(src, dir), no_effect = TRUE)
	can_act = FALSE
	SLEEP_CHECK_DEATH(18, src)
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hammer1.ogg', 60, TRUE)
	do_attack_animation(get_step(src, dir), no_effect = TRUE)
...
/mob/living/simple_animal/hostile/ordeal/sin_pride/noon
	...
	damage_coeff = list(BURN = 1.2, BRAIN = 1.2, BRUTE = 0.5, TOX = 1.5)
...
/mob/living/simple_animal/hostile/ordeal/sin_wrath/noon/EndCharge(bump = FALSE)
	...
	for(var/mob/living/L in range(1, T))//damage applied to targets in range
		...
		if(ishuman(L))
			var/mob/living/carbon/human/H = L
			H.apply_damage(floor(burn_stacks * 0.5), FIRE)
		L.apply_damage(charge_damage * 0.40,BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/cromer_slam.ogg', 75, 1)
