...
/mob/living/simple_animal/hostile/ordeal/sin_sloth
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/rock_attack.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/rock_dead.ogg'
	damage_coeff = list(BURN = 0.7, BRAIN = 1.5, BRUTE = 1.5, TOX = 2)
	ranged = TRUE
	move_to_delay = 8
...
/mob/living/simple_animal/hostile/ordeal/sin_sloth/proc/Dash(mob/living/target)
	...
	if(dist > 2 && jump_cooldown < world.time && dist < jump_range)
		can_act = FALSE
		var/list/dash_line = get_line(src, target)
		animate(src, pixel_y = (base_pixel_y + jump_pixels), time = 2)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/rock_kill.ogg', 50, FALSE, 4)
...
/mob/living/simple_animal/hostile/ordeal/sin_gluttony
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/flower_attack.ogg'
...
/mob/living/simple_animal/hostile/ordeal/sin_gluttony/AttackingTarget(atom/attacked_target)
	...
	if(. && isliving(attacked_target))
		...
		if(L.stat != DEAD)
			...
			L.apply_damage(rupture_damage, BRUTE)
		else
			devour(L)
...
/mob/living/simple_animal/hostile/ordeal/sin_gluttony/proc/devour(mob/living/L)
	...
	for(var/obj/item/bodypart/part in C.bodyparts)
		if(prob(dismember_probability) && part.body_part != CHEST && C.stat == DEAD)
			part.dismember()
			QDEL_NULL(part)
...
/mob/living/simple_animal/hostile/ordeal/sin_gloom
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/flea_dead.ogg'
	damage_coeff = list(BURN = 1.5, BRAIN = 0.7, BRUTE = 1.5, TOX = 2)
	ranged = TRUE
	...
	projectilesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/flea_attack.ogg'
	var/can_act = TRUE
...
/mob/living/simple_animal/hostile/ordeal/sin_gloom/proc/AreaAttack()
	...
	can_act = FALSE
	SLEEP_CHECK_DEATH(16, src)
	var/obj/effect/temp_visual/decoy/D = new /obj/effect/temp_visual/decoy(get_turf(src), src)
	animate(D, alpha = 0, transform = matrix()*1.6, time = 5)
	visible_message(span_danger("[src] suddenly explodes!"))
	playsound(loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hardslap.ogg', 60, TRUE)
...
/mob/living/simple_animal/hostile/ordeal/sin_pride
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/sword1.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/dead_generic.ogg'
	damage_coeff = list(BURN = 1.5, BRAIN = 1.5, BRUTE = 0.7, TOX = 2)
	ranged = TRUE
	var/charging = FALSE
	var/dash_num = 25
...
/mob/living/simple_animal/hostile/ordeal/sin_pride/proc/PrepCharge(target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/pridespin.ogg', 125, FALSE)
	if(charging_icon)
...
/mob/living/simple_animal/hostile/ordeal/sin_pride/proc/Charge(move_dir, times_ran)
	...
	for(var/obj/machinery/door/D in T.contents)
		if(D.density)
			D.open(2)
	forceMove(T)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade1.ogg', 100, 1)
	...
	for(var/mob/living/L in range(dash_range, T))//damage applied to targets in range
		...
		been_hit += L
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/sword4.ogg', 75, 1)
		if(!ishuman(L))
			continue
		var/mob/living/carbon/human/H = L
		if(H.health < 0)
...
/mob/living/simple_animal/hostile/ordeal/sin_lust //Tank that is resistant to bullets
	...
	attack_verb_continuous = "bashes"
	attack_verb_simple = "bashes"
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/cromer_slam.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg'
	damage_coeff = list(BURN = 0.7, BRAIN = 1.5, BRUTE = 1.5, TOX = 2)
	ranged = TRUE
	var/block_chance = 10
...
/mob/living/simple_animal/hostile/ordeal/sin_lust/proc/RangedAbility(atom/target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/censored2.ogg', 50, FALSE, 5)
	...
	SLEEP_CHECK_DEATH(ability_delay, src)
	Beam(T, "tentacle", time = 10)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/censored1.ogg', 75, FALSE, 5)
	for(var/turf/TT in turf_list)
		for(var/mob/living/L in HurtInTurf(TT, list(), ability_damage, BRUTE, null, TRUE, FALSE, TRUE, hurt_structure = TRUE))
			...
			L.blood_volume -= L.blood_volume * 0.15
	can_act = TRUE
...
/mob/living/simple_animal/hostile/ordeal/sin_wrath
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/tentacle_attack.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/dead_generic.ogg'
	damage_coeff = list(BURN = 1.5, BRAIN = 1.5, BRUTE = 0.7, TOX = 2)
	var/burn_stacks = 5
...
/mob/living/simple_animal/hostile/ordeal/sin_wrath/proc/Charge(atom/chargeat = target, delay = 1 SECONDS, chargepast = 2)
	...
	charge_ready = FALSE
	walk(src, 0)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/brown/tentacle_before_explode.ogg', 150, 1)
	...
	SLEEP_CHECK_DEATH(get_dist(src, T) * charging_speed, src)
	EndCharge()
...
/mob/living/simple_animal/hostile/ordeal/sin_wrath/Bump(atom/A)
	if(charging)
		if(isliving(A))
			...
			var/obj/vehicle/V = A
			V.take_damage(charge_damage*1.5, BRUTE)
