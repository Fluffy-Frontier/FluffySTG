...
/mob/living/simple_animal/hostile/ordeal/violet_fruit
	...
	speed = 4
	move_to_delay = 5
	damage_coeff = list(BURN = 1, BRAIN = 1.5, BRUTE = 1, TOX = 1)
...
/mob/living/simple_animal/hostile/ordeal/violet_fruit/proc/ReleaseDeathGas()
	...
	playsound(target_c, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/fruit_suicide.ogg', 50, 1, 16)
	setBruteLoss(maxHealth) // Die
	for(var/turf/open/T in turf_list)
		if(prob(25))
			new /obj/effect/temp_visual/revenant(T)
	for(var/mob/living/L in view(15, target_c))
		if(faction_check_atom(L))
			continue
		L.apply_damage(33, BRUTE, null, L.run_armor_check(null, BRUTE))
	for(var/mob/living/simple_animal/hostile/abnormality/A in urange(15, target_c))
		if(A.datum_reference && A.datum_reference.qliphoth_meter)
			A.qliphoth_change(pick(-999))
...
/mob/living/simple_animal/hostile/ordeal/violet_monolith
	...
	maxHealth = 1400
	health = 1400
	damage_coeff = list(BURN = 0.8, BRAIN = 2, BRUTE = 0.8, TOX = 1)
...
/mob/living/simple_animal/hostile/ordeal/violet_monolith/Life()
	...
	for(var/mob/living/L in view(2, src))
		if(!faction_check_atom(L))
			new /obj/effect/temp_visual/revenant(get_turf(L))
			L.apply_damage(9, BRUTE, null, L.run_armor_check(null, BRUTE))
...
/mob/living/simple_animal/hostile/ordeal/violet_monolith/proc/PulseAttack()
	...
	var/mob/living/simple_animal/hostile/abnormality/SA
	var/list/potential_abnormalities = list()
	for(var/mob/living/simple_animal/hostile/abnormality/A in urange(24, src))
		if(A.datum_reference && A.datum_reference.current && A.datum_reference.qliphoth_meter)
			potential_abnormalities += A
	if(LAZYLEN(potential_abnormalities))
		SA = pick(potential_abnormalities)
		SA.qliphoth_change(-1)
	icon_state = "violet_noon_attack"
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_dead.ogg'
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/apply_damage(damage, damagetype, def_zone, blocked, forced, spread_damage, wound_bonus, bare_wound_bonus, sharpness, attack_direction, attacking_item, exposed_wound_bonus)
	. = ..()
...
/obj/effect/violet_portal
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi' // TEMPORARY
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/red
	damage_coeff = list(BURN = -1, BRAIN = 0.7, BRUTE = 1.2, TOX = 1)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/red/proc/PerformAbility(atom/attack_target)
	...
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_portal_on.ogg', 75, TRUE, 16)
	//SLEEP_CHECK_DEATH(3.5 SECONDS, src)
	animate(B.visuals, alpha = 0, time = (0.5 SECONDS))
	var/list/first_line = get_line(T1, get_ranged_target_turf_direct(T1, T2, 18))
	var/list/second_line = get_line(T2, get_ranged_target_turf_direct(T2, T1, 18))
	INVOKE_ASYNC(src, PROC_REF(DoLineAttack), first_line)
	INVOKE_ASYNC(src, PROC_REF(DoLineAttack), second_line)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_red_attack.ogg', 50, FALSE, 24)
	//SLEEP_CHECK_DEATH(3 SECONDS, src)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_portal_off.ogg', 50, TRUE, 8)
	created_objects -= P1
	created_objects -= P2
		...
		for(var/turf/TT in RANGE_TURFS(3, T))
			...
			for(var/mob/living/L in TT)
				...
				if(!CanAttack(L))
					continue
				been_hit += L
				L.apply_damage(attack_damage, BRUTE, null, L.run_armor_check(null, BRUTE))
		//SLEEP_CHECK_DEATH(0.1, src)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/white
	...
	damage_coeff = list(BURN = 1, BRAIN = -1, BRUTE = 0.7, TOX = 1.2)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/white/proc/PerformAbility(atom/attack_target)
	...
	animate(B.visuals, alpha = 0, time = (0.5 SECONDS))
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_white_attack.ogg', 50, FALSE, 32)
	var/list/line = get_line(T1, T2)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/black
	...
	damage_coeff = list(BURN = 1.2, BRAIN = 1, BRUTE = -1, TOX = 0.7)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/black/proc/PerformAbility(atom/attack_target)
	...
	//SLEEP_CHECK_DEATH(2 SECONDS, src)
	animate(B.visuals, alpha = 0, time = (0.5 SECONDS))
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_black_attack1.ogg', 35, FALSE, 20)
	//SLEEP_CHECK_DEATH(0.4 SECONDS, src)
	var/list/first_line = get_line(T1, get_ranged_target_turf_direct(T1, T2, 18))
	var/list/second_line = get_line(T2, get_ranged_target_turf_direct(T2, T1, 18))
	INVOKE_ASYNC(src, PROC_REF(DoLineAttack), first_line)
	INVOKE_ASYNC(src, PROC_REF(DoLineAttack), second_line)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_black_attack2.ogg', 50, FALSE, 24)
	//SLEEP_CHECK_DEATH(2 SECONDS, src)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_portal_off.ogg', 50, TRUE, 8)
	animate(P1, alpha = 0, time = (1 SECONDS))
	animate(P2, alpha = 0, time = (1 SECONDS))
	...
	for(var/turf/T in line)
		for(var/turf/TT in RANGE_TURFS(2, T))
			...
			for(var/mob/living/L in TT)
				...
				been_hit += L
				L.apply_damage(attack_damage, BRUTE, null, L.run_armor_check(null, BRUTE))
		//SLEEP_CHECK_DEATH(0.05, src)

/obj/effect/black_portal
	name = "black portal"
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi' // TEMPORARY
	icon_state = "black_shield" // TEMPORARY
	layer = LIGHTING_MASK_LAYER
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/pale
	...
	damage_coeff = list(BURN = 0.7, BRAIN = 1.2, BRUTE = 1, TOX = -1)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/pale/proc/MoveEye(atom/eye_target)
	...
	playsound(get_turf(eye), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_pale_move.ogg', 50, TRUE, 8)
	//SLEEP_CHECK_DEATH(2 SECONDS, src)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_pale_move.ogg', 75, TRUE, 8)
	eye.forceMove(T)
...
/mob/living/simple_animal/hostile/ordeal/violet_midnight/pale/proc/Pulsate()
	...
	for(var/turf/T in RANGE_TURFS(pulse_range, eye))
		...
		for(var/mob/living/L in T)
			if(!CanAttack(L))
				continue
			L.apply_damage(pulse_damage, BRUTE, null, L.run_armor_check(null, BRUTE))
			has_targets = TRUE
	...
	if(has_targets)
		playsound(get_turf(eye), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_pale_attack.ogg', 75, TRUE, 8)
...
/datum/reusable_visual_pool/proc/NewPaleEyeAttack(turf/location, duration = 5)
	...
	RV.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi'
