...
/mob/living/simple_animal/hostile/megafauna/arbiter
	...
	health = 10000
	maxHealth = 10000
	damage_coeff = list(BURN = 0.1, BRAIN = 0.1, BRUTE = 0.1, TOX = 0.1)
	icon_state = "arbiter"
	icon_living = "arbiter"
	icon_dead = "arbiter"
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/StageChange()
	...
	ChangeResistances(list(BURN = 0.1, BRAIN = 0.1, BRUTE = 0.1, TOX = 0.1))
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/repulse.ogg', 50, TRUE, 24)
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/FairyFire(atom/target)
	...
	for(var/turf/T in get_line(start_loc, get_ranged_target_turf_direct(start_loc, target_loc, 14)))
		if(T.density)
			break
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/fairy.ogg', 100, FALSE, 12)
	for(var/i = 1 to fairy_count)
		...
		P.original = target
		P.aim_projectile(target_loc, src, deviation = rand(0, 7))
		P.fire()
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/KeyFire(atom/target)
	...
	var/turf/target_loc = get_turf(target)
	var/turf/start_loc = get_turf(src)
	for(var/turf/T in get_line(start_loc, get_ranged_target_turf_direct(start_loc, target_loc, 24)))
		new /obj/effect/temp_visual/cult/sparks(T)

	SLEEP_CHECK_DEATH(0.5 SECONDS, src)

	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/pillar_start.ogg', 75, FALSE, 12)
...
//
	//SLEEP_CHECK_DEATH(1 SECONDS, src)
	//playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/repulse.ogg', 50, TRUE, 7)
...
//
	...
	//meltdowns = SSlobotomy_corp.InitiateMeltdown(clamp(rand(player_count*0.5, player_count), 1, 10), TRUE, meltdown_type, meltdown_min_time, meltdown_max_time, meltdown_text, 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/meltdown.ogg')
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/OnMeltdownFinish(datum/source, datum/abnormality/abno_datum, worked)
	...
	//	return
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/meltdown_clear.ogg', 50, TRUE, 4)
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/HandledMeltdownEffect()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/meltdown_clear_all.ogg', 50, TRUE, 24)
	meltdown_cooldown = world.time + meltdown_cooldown_time
	...
	//		ChangeResistances(list(BURN = 2, BRAIN = 2, BRUTE = 2, TOX = 2))
	...
	//			check_count += 1
	//		charging = TRUE
	//		ChangeResistances(list(BURN = 0.8, BRAIN = 0.8, BRUTE = 0.8, TOX = 0.8))
	//		SLEEP_CHECK_DEATH(10 SECONDS, src)
	//		charging = FALSE
	//	if(MELTDOWN_PURPLE)
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/StartPillarStorm()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/storm_create.ogg', 50, TRUE, 24)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/storm_start.ogg', 50, FALSE, 48)
...
/mob/living/simple_animal/hostile/megafauna/arbiter/proc/FirePillarStorm()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/storm_charge.ogg', 50, FALSE, 32)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/storm_fire.ogg', 100, FALSE, 48)
...
/obj/effect/arbiter_spike/proc/Activate()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/arbiter/pin.ogg', 75, TRUE)
