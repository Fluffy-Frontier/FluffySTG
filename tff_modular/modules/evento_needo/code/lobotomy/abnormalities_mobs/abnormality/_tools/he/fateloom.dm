...
/obj/structure/toolabnormality/fateloom/attack_hand(mob/living/carbon/human/user)
	...
	if(!S)
		...
		return
	user.playsound_local(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fateloom/garrote_bloody.ogg', 60, TRUE)
...
/datum/status_effect/stacking/red_string
	...
	overlay_file = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi'
	overlay_state = "fateloom"

/datum/status_effect/stacking/red_string/on_apply()
	RegisterSignal(owner, COMSIG_MOB_AFTER_APPLY_DAMAGE, PROC_REF(heal))
	return ..()
...
/datum/status_effect/stacking/red_string/proc/heal()
	...
	if(H.health > H.maxHealth * 0.5)
		return
	if(stacks > 1)
		playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fateloom/garrote.ogg', 80, TRUE, -3)
		...
		for(var/X in H.bodyparts)
			...
			if(BP.body_part && BP.body_part != CHEST)
				BP.dismember()
		playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fateloom/garrote_bloody.ogg', 80, TRUE, -3)
...
/datum/status_effect/stacking/red_string/stack_decay_effect()
	if(stacks < max_stacks)
		...
		playsound(owner, 'sound/items/weapons/cablecuff.ogg', 15, TRUE, -2)
