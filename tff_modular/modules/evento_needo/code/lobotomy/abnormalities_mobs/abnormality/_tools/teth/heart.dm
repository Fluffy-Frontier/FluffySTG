...
/datum/status_effect/display/aspiration/tick()
	...
	if(ferventbeats && rage_safe_time < world.time)
		H.apply_damage(H.maxHealth * (1/100), BRUTE) // Roughly standard regenerator healing
	if(!raging)
		return
	...
	H.apply_damage(H.maxHealth * (2/100), BRUTE) // You are most likely going to die, and very soon.
...
/datum/status_effect/display/aspiration/proc/RageEnable()
	...
	H.playsound_local(get_turf(H), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/heartbeat.ogg', 50, 0, 3)
	ferventbeats = TRUE
...
/datum/status_effect/display/aspiration/proc/SuperRageEnable()
	...
	H.playsound_local(get_turf(H), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/heartbeat2.ogg', 50, 0, 3)
