...
/obj/structure/toolabnormality/you_happy/post_buckle_mob(mob/living/carbon/human/M)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/you_happy/change.ogg', 80, TRUE, -3)
	running = TRUE
	state = "NO"
	bonus = initial(bonus)
...
/obj/structure/toolabnormality/you_happy/proc/Change()
	...
	if(bonus > 49)
		state = "NO"
		icon_state = "YMBH_NO"
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/green_end.ogg', 70, TRUE, -3)
		return
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/you_happy/change.ogg', 40, TRUE, -3)
	switch(state)
		if("YES")
			state = "NO"
			icon_state = "YMBH_NO"
...
/obj/structure/toolabnormality/you_happy/post_unbuckle_mob(mob/living/carbon/human/user)
	...
	switch(state)
		if("YES")
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/you_happy/success.ogg', 130, TRUE, -3)
			var/datum/status_effect/display/YMBH/buff = user.apply_status_effect(/datum/status_effect/display/YMBH)
			buff.bonus = bonus
			buff.apply_buffs()
		if("NO")
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/you_happy/fail.ogg', 80, TRUE, -3)
