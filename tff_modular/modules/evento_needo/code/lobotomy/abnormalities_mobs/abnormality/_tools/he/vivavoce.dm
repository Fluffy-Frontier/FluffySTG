...
/obj/structure/toolabnormality/attribute_giver/vivavoce/attack_hand(mob/living/carbon/human/user)
	...
	if(used_by[user] == 1) // First use
		if(prob(50))
			playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/vivavoce/doorknock.ogg', 100, FALSE, -5)
		return

	user.physiology.brute_mod *= 1.10
...
/datum/status_effect/stacking/rolecall/proc/UserDebuff()
	...
	if(prob(5))
		playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/vivavoce/doorknock.ogg', 100, FALSE, -5)
