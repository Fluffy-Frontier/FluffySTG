...
/obj/structure/toolabnormality/theresia/process()
	...
	if(activated == ACTIVE_DANGEROUS)
		...
		for(var/mob/living/carbon/human/H in view(32, get_turf(src)))
			H.apply_damage(pulse_damage, BRUTE)
		return
...
/obj/structure/toolabnormality/theresia/proc/Activate(mob/user)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/pink_start.ogg', 50, 0, 4)
	current_winds = 0
