...
/datum/status_effect/stacking/slab/on_apply()
	RegisterSignal(owner, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(Punishment))
	return ..()
...
/datum/status_effect/stacking/slab/proc/Punishment(mob/living/carbon/human/owner, damage, damagetype, def_zone)
	...
	attack.dir = H.dir
	playsound(attack, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/white/pale_teleport_out.ogg', 50, TRUE, 3)
...
/datum/status_effect/stacking/slab/proc/PunishDeath(mob/living/M)
	...
	playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/violet/midnight_black_attack2.ogg', 35, TRUE, 3)
	H.gib()
...
/obj/effect/temp_visual/theonite_clone/proc/DoLineAttack(list/line)
	for(var/turf/T in line)
		if(locate(/obj/effect/temp_visual/smash_effect) in T)
			continue
		playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/blade3.ogg', 55, TRUE, 3)
