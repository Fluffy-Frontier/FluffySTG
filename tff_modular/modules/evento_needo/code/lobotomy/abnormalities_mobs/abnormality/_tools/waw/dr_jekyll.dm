...
/obj/structure/toolabnormality/dr_jekyll/attack_hand(mob/living/carbon/human/user)
	...
	if(!H)
		...
		user.apply_damage(999, BRUTE, null, user.run_armor_check(null, BRUTE), spread_damage = TRUE)//DIE!
		return
...
/datum/action/cooldown/dr_jekyll/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if (owner.stat == DEAD)
		return FALSE
