...
/obj/structure/toolabnormality/touch/proc/RoundEndEffect(mob/living/carbon/human/user)
	...
	SSticker.SetRoundEndSound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/donttouch/end.ogg')
	...
	if(prob(50)) // Randomly choose to eighter gib or insane everyone
		for(var/mob/M in GLOB.player_list)
			...
			M.playsound_local(M, pick('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/donttouch/kill.ogg', 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/donttouch/kill2.ogg'), 50, FALSE)
			if(ishuman(M))
		...
		for(var/mob/M in GLOB.player_list)
			if(isnewplayer(M))
				continue
			flash_color(M, flash_color = COLOR_RED, flash_time = 150)
			M.playsound_local(M, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/donttouch/panic.ogg', 50, FALSE)
			if(ishuman(M))
				var/mob/living/carbon/human/H = M
				H.apply_damage(5000, BRUTE, null, null, spread_damage = TRUE) // You cannot escape.
...
/obj/structure/toolabnormality/touch/proc/BreachEffect(mob/living/carbon/human/user)
	...
	for(var/mob/M in GLOB.player_list)
		...
		M.playsound_local(M, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/donttouch/breach.ogg', 50, FALSE)
