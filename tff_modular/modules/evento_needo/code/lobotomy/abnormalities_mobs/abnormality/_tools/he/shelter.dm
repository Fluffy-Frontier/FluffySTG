...
/obj/structure/toolabnormality/shelter/proc/travel(mob/living/carbon/human/user)
	...
	if(user.pulling)
		...
		if(ishuman(AM)) //We don't want players dragging the supplies in and out
			...
			for(var/obj/item/storage/box/pcorp/foodbox in Person.get_all_contents())
				Person.dropItemToGround(foodbox, TRUE)
			for(var/obj/item/food/canned/pcorp/foodcan in Person.get_all_contents())
...
/obj/structure/toolabnormality/shelter/entrance/travel(mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/shelter/open.ogg', 80, TRUE, -3)
	..()
...
/obj/structure/toolabnormality/shelter/exit/travel_check(mob/living/carbon/human/user)
	...
	for(var/obj/item/storage/box/pcorp/foodbox in user.get_all_contents())
		user.dropItemToGround(foodbox, TRUE)
	for(var/obj/item/food/canned/pcorp/foodcan in user.get_all_contents())
...
///obj/effect/shelter_forcefield/Crossed(atom/movable/crossed_atom, oldloc)
//	. = ..()
//	if(!ishostile(crossed_atom)) // kick out any hostile mobs
//		return
//	var/mob/living/simple_animal/hostile/G = crossed_atom
//	if("neutral" in G.faction) //This should prevent an exploit with clerkbots and any other friendly summon teleporting
//		return
//	var/list/potential_locs = shuffle(GLOB.start_landmarks_list)
//	var/turf/T = pick(potential_locs)
//	crossed_atom.forceMove(T) //send them to brazil
