...
/datum/status_effect/display/behavior/proc/UserInsane()
	var/mob/living/carbon/human/H = owner
	H.emote("laugh")
	var/obj/item/organ/eyes/O = H.get_organ_by_type(/obj/item/organ/eyes)
	if(istype(O))
		H.visible_message(span_danger("[H] tears [H.p_their(FALSE)] eyes out!"))
		playsound(get_turf(H), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/behavior/eyes_out.ogg', 35, TRUE)
		O.Remove(H)
