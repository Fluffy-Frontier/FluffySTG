...
/mob/living/simple_animal/hostile/abnormality/quiet_day/examine()
	...
	. +=  "The shadow of an old man seems to be contemplating about something. <br>\
...
/mob/living/simple_animal/hostile/abnormality/quiet_day/try_working(mob/living/carbon/human/user)
	...
	if(!user.combat_mode || !currently_talking)
		...
		visible_message(span_notice("[user] asks [src] to stop telling the story."), \
						span_notice("[user] asks you to stop telling the story."), null, null, user)
		to_chat(user, span_notice("You ask [src] to stop telling the story."))
...
/mob/living/simple_animal/hostile/abnormality/quiet_day/say(message, bubble_type, list/spans, sanitize, datum/language/language, ignore_spam, forced, message_mods, message_range, saymode)
	if(stat == DEAD)
		return
	. = ..()
...
/datum/reagent/abnormality/quiet_day/on_mob_metabolize(mob/living/carbon/human/affected_mob)
	. = ..()
	affected_mob.adjustSanityLoss(-2)
