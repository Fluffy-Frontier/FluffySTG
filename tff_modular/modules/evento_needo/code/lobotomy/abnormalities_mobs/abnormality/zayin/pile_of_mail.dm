...
/mob/living/simple_animal/hostile/abnormality/mailpile/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	...
	if(prob(neutral_chance))
		BadDelivery(user,)
		qliphoth_change(-1)
	else
		Delivery(user)

/mob/living/simple_animal/hostile/abnormality/mailpile/proc/Delivery(mob/living/carbon/human/user)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mailpile/gotmail.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/abnormality/mailpile/proc/BadDelivery(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/mailpile/proc/SendDeathThreat()
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		...
		if(role in list("Manager", "Extraction Officer", "Records Officer", "Sephirah"))
			weaken = TRUE
		var/threat_type = pick_weight(list(
...
/datum/status_effect/brute_armor_buff
	id = "brute_armor_buff"
	status_type = STATUS_EFFECT_UNIQUE
	alert_type = /atom/movable/screen/alert/status_effect/brute_armor_buff
	duration = 30 SECONDS

/datum/status_effect/brute_armor_buff/on_apply()
	. = ..()
	var/mob/living/carbon/human/user = owner
	user.physiology.brute_mod /= 1.5

/datum/status_effect/brute_armor_buff/on_remove()
	. = ..()
	var/mob/living/carbon/human/user = owner
	user.physiology.brute_mod *= 1.5

/atom/movable/screen/alert/status_effect/brute_armor_buff
...
/obj/item/mailpaper/pipebomb/attack_self(mob/living/carbon/human/user)
	...
	sleep(10)
	playsound(get_turf(user), 'sound/items/weapons/flashbang.ogg', 30, TRUE, 8, 0.9)
...
/obj/item/mailpaper/insight/attack_self(mob/living/carbon/human/user)
	...
	if(!HAS_TRAIT(user,TRAIT_COMBATFEAR_IMMUNE))
...
/obj/item/mailpaper/attachment/attack_self(mob/living/carbon/human/user)
	...
	var/datum/status_effect/brute_armor_buff/TMPEFF = user.has_status_effect(/datum/status_effect/brute_armor_buff)
	if (!TMPEFF)
		user.apply_status_effect(/datum/status_effect/brute_armor_buff)
	qdel(src)
...
/obj/item/mailpaper/coupon/attack_self(mob/user)
	...
	playsound(user, 'sound/items/poster/poster_ripped.ogg', 100)
