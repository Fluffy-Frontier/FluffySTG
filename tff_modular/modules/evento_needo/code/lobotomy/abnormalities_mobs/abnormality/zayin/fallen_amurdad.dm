...
/mob/living/simple_animal/hostile/abnormality/fallen_amurdad/examine(mob/user)
	. = ..()
	. += "The sweet stench of rot and decay hit you before you noticed the source was the bleeding person covered in plants. <br>\
...
/mob/living/simple_animal/hostile/abnormality/fallen_amurdad/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
...
/obj/machinery/hydroponics/soil/amurdad/adjust_weedlevel(amt)
	return
...
/obj/structure/amurdad_bomb/proc/ProtectPlants()
	...
	//		AV.atom_integrity += 50
	//		continue
	//	protected_plants += AV
	//	AV.max_integrity = 300
	//	AV.atom_integrity = 300
...
/obj/structure/amurdad_bomb/proc/UpdateStage()
	...
	if(stage == 0)
		return
	var/mutable_appearance/plant_overlay = mutable_appearance('icons/obj/service/hydroponics/growing.dmi', "deathnettle-grow[stage]", layer = OBJ_LAYER + 0.01)
