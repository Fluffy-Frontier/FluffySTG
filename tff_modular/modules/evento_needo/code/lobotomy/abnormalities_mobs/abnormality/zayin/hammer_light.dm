...
/mob/living/simple_animal/hostile/abnormality/hammer_light
	...
	var/list/lock_sounds = list(
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_filterOut1.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_filterOut2.ogg',
	)
	var/list/pickup_sounds = list(
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_usable1.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_usable2.ogg',
	)
	work_types = null
...
/mob/living/simple_animal/hostile/abnormality/hammer_light/proc/Check() // A lot going on here, but basically we assess how bad the situation in the facility is
	...
	if(points >= points_threshold) // If we have enough points, we unseal
		if(sealed)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/chain.ogg', 75, 0, -9)
		sealed = FALSE
	else
...
/mob/living/simple_animal/hostile/abnormality/hammer_light/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	if(!ishuman(user))
		return
	if(prob(50) && healing_cooldown <= world.time)
		...
		for(var/mob/living/carbon/human/H in GLOB.player_list)
			...
			playsound(get_turf(H), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/sorrow_shot.ogg', 25, FALSE, 3)
			new /obj/effect/temp_visual/beam_in(get_turf(H))
	if(!hammer_present)
		to_chat(user, span_warning("The hammer is not there!"))
		return
	if(sealed)
		to_chat(user, span_warning("The hammer is sealed!"))
		return
	PickUpHammer(user)
	return
...
/mob/living/simple_animal/hostile/abnormality/hammer_light/BreachEffect(mob/living/carbon/human/user, breach_type = BREACH_NORMAL)
	...
	sealed = FALSE
	update_icon()
	var/mob/living/simple_animal/hostile/ordeal/pink_midnight/P = locate() in GLOB.simple_animals
	if(!P)
		return FALSE
	var/turf/destination = get_turf(P)
	var/turf/W = get_turf(pick(GLOB.start_landmarks_list)) // spawn hammers at a random department
...
/obj/item/ego_weapon/hammer_light
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_filter.ogg'
	slot_flags = null
	actions_types = list(
		/datum/action/cooldown/spell_action/evening_twilight,
	)
	var/list/spawned_mobs = list()
...
/obj/item/ego_weapon/hammer_light/attack_self(mob/user)
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/snap.ogg', 50, FALSE)
...
/datum/action/cooldown/spell_action/evening_twilight/Activate(mob/living/simple_animal/hostile/target, mob/user)
	if(!istype(user))
		return ..()
	target = null
	var/dist = 100
	...
	if(!target)
		...
		return ..()
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/black_silence/unlock.ogg', 50, FALSE)
	new /obj/effect/temp_visual/beam_in(get_turf(user))
...
/datum/status_effect/evening_twilight/on_apply()
	...
	status_holder.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x32.dmi', "hammer_overlay", -ABOVE_MOB_LAYER))
	status_holder.physiology.burn_mod *= 0.3
	status_holder.physiology.brain_mod *= 0.3
	status_holder.physiology.brute_mod *= 0.3
	status_holder.physiology.tox_mod *= 0.3
	return ..()
...
/mob/living/simple_animal/hostile/lighthammer
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/lighthammer/hammer_filter.ogg'
	health = 1000
	maxHealth = 1000
