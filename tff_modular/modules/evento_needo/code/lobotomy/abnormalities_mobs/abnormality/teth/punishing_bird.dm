...
/mob/living/simple_animal/hostile/abnormality/punishing_bird/proc/OnAbnoWork(datum/source, datum/abnormality/abno_datum, mob/user, work_type)
	...
	if(abno_datum == datum_reference) // They worked on us!
		return FALSE
	if(prob(20 - (abno_datum.current.fear_level * 5))) // So working on WAWs/ALEPHs won't do anything
		qliphoth_change(-1)
	return TRUE
...
/mob/living/simple_animal/hostile/abnormality/punishing_bird/Life()
	if(..())
		...
		for(var/mob/living/carbon/human/H in around)
			if(H.mind && !faction_check_atom(H))
				if(H.sanity_lost)
					priority_mobs += H
...
/mob/living/simple_animal/hostile/abnormality/punishing_bird/proc/kill_bird()
	if(!(HAS_TRAIT(src, TRAIT_GODMODE)) && !isliving(target) && icon_state != "pbird_red")
		QDEL_NULL(src)
	else
		death_timer = addtimer(CALLBACK(src, PROC_REF(kill_bird)), 60 SECONDS, TIMER_STOPPABLE)

// Modified patrolling
/mob/living/simple_animal/hostile/abnormality/punishing_bird/handle_automated_movement()
...
/mob/living/simple_animal/hostile/abnormality/punishing_bird/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
