...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/so_that_no_cry/attack.ogg'
...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry/proc/Apply_Talisman(mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/so_that_no_cry/talisman.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry/proc/ReflectDamage(mob/living/attacker, attack_type = BRUTE, damage)
	if(damage < 1)
		return
	...
	forceMove(jump_turf)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/so_that_no_cry/counter.ogg', min(15 + damage, 100), TRUE, 4)
	...
	attacker.apply_damage(damage, attack_type)
...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry/proc/StartReflecting()
	...
	damage_taken = 0
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/so_that_no_cry/prepare.ogg', 50, TRUE, 7)
	...
	ChangeResistances(list(BURN = 1.5, BRAIN = 1, BRUTE = 1, TOX = 2))
	damage_reflection = FALSE
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/so_that_no_cry/proc/TalismanStun()
	...
	SLEEP_CHECK_DEATH(stun_duration, src)
	icon_state = "so_that_no_cry"
	can_act = TRUE
	talismans = 0
...
/datum/status_effect/stacking/talisman/on_apply()
	...
	status_holder.physiology.tox_mod -= 0.5
	return ..()
...
/datum/status_effect/stacking/talisman/add_stacks(stacks_added)
	if(!ishuman(owner))
		return ..()
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.tox_mod -= 0.5 * stacks_added //max of 25
	return ..()
...
/datum/status_effect/stacking/talisman/on_remove()
	if(!ishuman(owner))
		return ..()
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.tox_mod += 0.5 * stacks
...
/datum/status_effect/stacking/curse_talisman/on_apply()
	if(!ishuman(owner))
		return ..()
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.tox_mod += 0.7 * stacks
	return ..()
...
/datum/status_effect/stacking/curse_talisman/add_stacks(stacks_added)
	if(!ishuman(owner))
		return ..()
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.tox_mod += 0.7 * stacks_added //max of -42
	return ..()
