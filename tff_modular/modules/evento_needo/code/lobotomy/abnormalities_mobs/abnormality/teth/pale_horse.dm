...
/mob/living/simple_animal/hostile/abnormality/pale_horse
	...
	base_pixel_x = -16
	stat_attack = DEAD
	damage_coeff = list(BURN = 1.5, BRAIN = 1.5, BRUTE = 1.5, TOX = 0)
	can_breach = TRUE
	ego_list = list(
...
//work stuff
/mob/living/simple_animal/hostile/abnormality/pale_horse/SuccessEffect(mob/living/carbon/human/user)
	qliphoth_change(1)
	return
...
/mob/living/simple_animal/hostile/abnormality/pale_horse/PostWorkEffect(mob/living/carbon/human/user)
	. = ..()
	if(user.health < (user.maxHealth * 0.5))
		return
	else
		user.apply_damage(user.maxHealth * 0.5, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/pale_horse/AttackingTarget(atom/attacked_target)
	...
	if(T.health > 0)
		...
		if(!M)
			...
			playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/palehorse/debuff.ogg', 50, 0, -1)
		LoseTarget(T)
	else
...
/mob/living/simple_animal/hostile/abnormality/pale_horse/proc/ToAshes(target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/palehorse/kill.ogg', 50, 0, 8)
	...
	for(var/turf/F in view(pulse_range, src))
		...
		for(var/mob/living/H in F)
			if(faction_check_atom(H))
				continue
			H.apply_damage(ash_damage, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/pale_horse/handle_automated_movement()
...
/datum/status_effect/mortis/on_apply()
	...
	status_holder.physiology.tox_mod *= 2
