/mob/living/simple_animal/hostile/abnormality/ppodae
	...
	damage_coeff = list(BURN = 1.5, BRAIN = 0.8, BRUTE = 1, TOX = 2)
	can_breach = TRUE
	vision_range = 14
...
/datum/action/cooldown/ppodae_transform/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/ppodae/proc/UpdateForm()
	if(buff_form)
		ChangeResistances(list(BURN = buff_resist_red, BRAIN = buff_resist_white, BRUTE = buff_resist_black, TOX = buff_resist_pale))
		move_to_delay = buff_speed
		icon_state = "ppodae_active"
		can_slam = TRUE
	else
		ChangeResistances(list(BURN = cute_resist_red, BRAIN = cute_resist_white, BRUTE = cute_resist_black, TOX = cute_resist_pale))
	...
	//smoke.start()
	//qdel(smoke)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scaredycat/cateleport.ogg', 50, 0, 5)
...
/mob/living/simple_animal/hostile/abnormality/ppodae/proc/LimbSteal(mob/living/carbon/L)
	...
	if(length(parts))
		...
		bp.dismember()
		bp.forceMove(get_turf(pick(GLOB.generic_event_spawns))) // Teleports limb to containment
		QDEL_NULL(src)
...
/mob/living/simple_animal/hostile/abnormality/ppodae/proc/Smash(target)
	...
	switch(dir_to_target)
		if(EAST)
			middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, EAST, smash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, smash_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, smash_width)))
					if (U.density)
						break
					...
					area_of_effect += U
		if(WEST)
			middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, WEST, smash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, smash_width)))
				...
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, smash_width)))
					...
					area_of_effect += U
		if(SOUTH)
			middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, SOUTH, smash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, smash_width)))
					if (Y.density)
						break
					if (Y in area_of_effect)
						continue
					area_of_effect += Y
				for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, smash_width)))
					if (U.density)
						break
					if (U in area_of_effect)
						continue
					area_of_effect += U
		if(NORTH)
			middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, NORTH, smash_length))
			for(var/turf/T in middle_line)
				if(T.density)
					break
				for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, smash_width)))
	...
	for(var/turf/T in area_of_effect)
		...
		HurtInTurf(T, list(), smash_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE, hurt_structure = TRUE)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ppodae/bark.wav', 100, 0, 5)
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ppodae/attack.wav', 50, 0, 5)
	SLEEP_CHECK_DEATH(0.5 SECONDS, src)
	can_act = TRUE

/mob/living/simple_animal/hostile/abnormality/ppodae/PostWorkEffect(mob/living/carbon/human/user)
	if(user.get_major_clothing_class() != CLOTHING_ENGINEERING && prob(50))
		qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/ppodae/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
