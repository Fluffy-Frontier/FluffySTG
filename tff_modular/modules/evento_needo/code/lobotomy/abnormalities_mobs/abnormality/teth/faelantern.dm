...
/mob/living/simple_animal/hostile/abnormality/faelantern
	...
	damage_coeff = list(BURN = 1, BRAIN = 1.3, BRUTE = 0.5, TOX = 1.5, BRUTE = 2)
	wander = FALSE
	can_breach = TRUE
	del_on_death = FALSE
...
/mob/living/simple_animal/hostile/abnormality/faelantern/PostWorkEffect(mob/living/carbon/human/user, work_type, pe, work_time, canceled)
	if(user.get_major_clothing_class() == CLOTHING_ARMORED && get_modified_attribute_level(user, TEMPERANCE_ATTRIBUTE) >= 40)
		qliphoth_change(-1)
		return
	if(user.get_major_clothing_class() != CLOTHING_ARMORED && get_modified_attribute_level(user, TEMPERANCE_ATTRIBUTE) < 40)
		qliphoth_change(-1)

/mob/living/simple_animal/hostile/abnormality/faelantern/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/faelantern/proc/BreachDig(broken = FALSE)
	var/turf/teletarget = get_turf(pick(GLOB.generic_event_spawns))
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/faelantern/faelantern_breach.ogg', 200, 1)//proper sfx goes here
...
/mob/living/simple_animal/hostile/abnormality/faelantern/proc/Uproot()//TODO: give it a varaible and condition for its broken state
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/faelantern/faelantern_uproot.ogg', 200, 1)
	var/obj/effect/particle_effect/smoke/S = new(get_turf(src))
	S.alpha = 50
	S.lifetime = 1
	SLEEP_CHECK_DEATH(4, src)
...
/mob/living/simple_animal/hostile/abnormality/faelantern/proc/FairyLure(target)//lure AOE
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/faelantern/faelantern_giggle.ogg', 100, 0)
	for(var/mob/living/carbon/human/victim in view(8, src))
		victim.apply_damage(lure_damage, BRUTE)
		...
		victim.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "fairy_lure", -HALO_LAYER))
...
/mob/living/simple_animal/hostile/abnormality/faelantern/proc/RootWave()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/faelantern/faelantern_breach.ogg', 200, 1)
	SLEEP_CHECK_DEATH(2 SECONDS, src)
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/faelantern/proc/EndEnchant(mob/living/carbon/human/victim, stunned = FALSE)//cannibalized apocalypse bird handling
	if(victim in lured_list)
		lured_list -= victim
		victim.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "fairy_lure", -HALO_LAYER))
...
/obj/effect/root/faelantern/proc/explode() //repurposed code from artillary bees, a delayed attack
	...
	for(var/turf/T in view(0, target_turf))
		...
		for(var/mob/living/L in T)
			L.apply_damage(root_damage, BRUTE)
	qdel(src)
...
/datum/status_effect/fairy_lure/on_apply()
	...
	status_holder.physiology.burn_mod *= 5
