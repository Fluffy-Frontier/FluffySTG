...
/mob/living/simple_animal/hostile/abnormality/shy_look/proc/ChangeOverlay(next_mood)
	...
	switch(next_mood)
		...
		if(5)
			expression = "angry"
	icon_overlay = mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects10x10.dmi', expression, -MUTATIONS_LAYER)
	icon_overlay.pixel_x = -2
	icon_overlay.pixel_y = 30
	add_overlay(icon_overlay)

/mob/living/simple_animal/hostile/abnormality/shy_look/PostWorkEffect(mob/living/carbon/human/user)
	if(previous_mood == 1) // heals 20% hp&sp
