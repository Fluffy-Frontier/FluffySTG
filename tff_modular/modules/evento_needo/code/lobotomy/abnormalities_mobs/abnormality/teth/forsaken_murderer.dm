...
/mob/living/simple_animal/hostile/abnormality/forsaken_murderer/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	...
	if(!client && work_emote_cooldown <= world.time && prob(35))
		...
		//Switch only allows one of the ifs within it to proc, this is helpful with sorting number effects.
			...
			if(6)
				 ...
				 */
				playsound(get_turf(src), 'sound/mobs/humanoids/human/scream/malescream_6.ogg', 35, 3, 3)
