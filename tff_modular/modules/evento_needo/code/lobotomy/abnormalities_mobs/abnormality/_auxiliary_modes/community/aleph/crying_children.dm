...
/mob/living/simple_animal/hostile/abnormality/crying_children
	...
	maxHealth = 2000
	obj_damage = 600
	damage_coeff = list(BURN = 0.6, BRAIN = 1, BRUTE = 0.6, TOX = 1.5)
	...
	ranged = TRUE
	can_breach = TRUE
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/attack_salvador.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/death.ogg'
...
/datum/action/cooldown/tcc_sorrow
	...
	button_icon = 'icons/obj/weapons/guns/projectiles_muzzle.dmi'
	button_icon_state = "muzzle_beam_heavy"
	check_flags = AB_CHECK_CONSCIOUS
	transparent_when_unavailable = TRUE
	cooldown_time = TCC_SORROW_COOLDOWN

/datum/action/cooldown/tcc_sorrow/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	...
	for(var/mob/living/L in view(8, TCC))
		if(TCC.faction_check_atom(L) || L.stat == DEAD)
			continue
...
/datum/action/cooldown/tcc_combust
	...
	//button_icon = 'icons/mob/actions/actions_ability.dmi'
	//button_icon_state = "fire0"
	check_flags = AB_CHECK_CONSCIOUS
	transparent_when_unavailable = TRUE
	cooldown_time = TCC_COMBUST_COOLDOWN

/datum/action/cooldown/tcc_combust/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if(!istype(owner, /mob/living/simple_animal/hostile/abnormality/crying_children))
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/Scorching_Desperation()
	...
	for(var/mob/living/L in GLOB.mob_living_list)
		...
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/attack_aoe.ogg', 50, TRUE)
		L.apply_damage(175, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/crying_children/AttackingTarget(atom/attacked_target)
	...
	if(isliving(attacked_target))
		...
		L.apply_damage(5*burn_mod, FIRE)
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/Bygone_Illusion(mob/living/target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/attack_yuna.ogg', 50, FALSE)
	...
	for(var/turf/open/T in hitline)
		...
		var/list/new_hits = HurtInTurf(T, been_hit, 60, BRUTE, null, TRUE, FALSE, TRUE, TRUE) - been_hit
		been_hit += new_hits
		for(var/mob/living/L in new_hits)
			to_chat(L, span_userdanger("[src] stabs you!"))
			L.apply_damage(4*burn_mod, FIRE)
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/Wounds_Of_Sorrow(mob/living/target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/sorrow_charge.ogg', 50, FALSE)
	SLEEP_CHECK_DEATH(10, src)
	for(var/targets in targets_to_hit)
		var/obj/projectile/beam/sorrow_beam/P = new(get_turf(src))
		P.firer = src
		P.fired_from = src
		P.set_angle(get_angle(src, targets))
		P.fire()
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/sorrow_shot.ogg', 50, FALSE)
	icon_state = "[icon_phase]_idle"
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/Combusting_Courage()
	...
	can_act = FALSE
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/white/pale_teleport_out.ogg', 50, FALSE)
	icon_state = "[icon_phase]_sorrow"
	var/list/been_hit = list()
	...
	icon_state = "[icon_phase]_idle"
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/attack_aoe.ogg', 50, FALSE)
	for(var/turf/T in view(4, src))
		new /obj/effect/temp_visual/fire/fast(T)
		var/list/new_hits = HurtInTurf(T, been_hit, 250, BRUTE, null, TRUE, FALSE, TRUE, TRUE) - been_hit
		been_hit += new_hits
		for(var/mob/living/L in new_hits)
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/SpawnChildren()
	...
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/seperate.ogg'))
...
/mob/living/simple_animal/hostile/abnormality/crying_children/proc/FinalPhase()
	...
	desperate = TRUE
	maxHealth = 4000
	ChangeResistances(list(BURN = 0.4, BRAIN = 0.8, BRUTE = 0.4, TOX = 1))
	revive(HEAL_ALL)
	move_to_delay = 4
	burn_mod = 2
	can_act = TRUE
	...
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/final_phase.ogg'))
...
/mob/living/simple_animal/hostile/child
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crying_children/attack_child.ogg'
	health = 1500
	maxHealth = 1500
	damage_coeff = list(BURN = 0.8, BRAIN = 0.8, BRUTE = 0.8, TOX = 1)
	melee_damage_type = BRUTE
...
/mob/living/simple_animal/hostile/child/unhearing
	...
	damage_coeff = list(BURN = 0.8, BRAIN = 0.8, BRUTE = 1.5, TOX = 0.8)
	var/list/deafened = list()

/mob/living/simple_animal/hostile/child/unhearing/Initialize()
	. = ..()
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(faction_check_atom(H, FALSE) || H.z != z || H.stat == DEAD || HAS_TRAIT(H, TRAIT_DEAF))
			continue
...
/mob/living/simple_animal/hostile/child/unspeaking
	...
	icon_state = "unspeaking_child"
	icon_living = "unspeaking_child"
	damage_coeff = list(BURN = 0.8, BRAIN = 1.5, BRUTE = 0.8, TOX = 0.8)
	var/list/muted = list()

/mob/living/simple_animal/hostile/child/unspeaking/Initialize()
	. = ..()
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(faction_check_atom(H, FALSE) || H.z != z || H.stat == DEAD || HAS_TRAIT(H, TRAIT_MUTE))
			continue
...
/obj/projectile/beam/sorrow_beam/on_hit(atom/target, blocked = FALSE, pierce_hit)
	. = ..()
	if(isliving(target))
