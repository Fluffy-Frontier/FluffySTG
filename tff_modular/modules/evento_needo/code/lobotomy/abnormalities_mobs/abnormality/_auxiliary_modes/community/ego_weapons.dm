...
/obj/item/ego_weapon/support/dragon_staff
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/_auxiliary_modes/community/!icons/ego_weapons.dmi'
	lefthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/_auxiliary_modes/community/!icons/ego_lefthand.dmi'
	righthand_file = 'tff_modular/modules/evento_needo/icons/Teguicons/_auxiliary_modes/community/!icons/ego_righthand.dmi'
	force = 16
	damtype = BRUTE
...
/obj/item/ego_weapon/support/dragon_staff/Pulse(mob/living/carbon/human/user)
	for(var/mob/living/carbon/human/L in view(8, user))
...
/obj/item/ego_weapon/sunspit/attack(mob/living/target, mob/living/user)
	...
	if(charge_amount == charge_cost || charge_amount == charge_cap)//audio cue for when you have enough charge
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/seasons/old_fall_attack.ogg', 25, TRUE)
	can_spin = FALSE
...
/obj/item/ego_weapon/sunspit/ChargeAttack(mob/living/target, mob/living/user)
	...
	if(do_after(user, 8, src))
		...
		addtimer(CALLBACK(src, PROC_REF(spin_reset)), 12)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/seasons/summer_attack.ogg', 75, FALSE, 4)
...
/obj/item/ego_weapon/sunspit/proc/DoLineAttack(list/line, atom/target, mob/living/carbon/human/user)
	...
	for(var/turf/T in line)
		if(locate(/obj/effect/temp_visual/smash_effect) in T)
			continue
		playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/fire3.ogg', 30, TRUE, 3)
...
/obj/item/ego_weapon/shield/waxen
	...
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/burn_sword.ogg'
	...
	block_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/ego/burn_guard.ogg'
...
/obj/item/ego_weapon/shield/waxen/AnnounceBlock(mob/living/carbon/human/source, damage, damagetype, def_zone)
	source.apply_damage(2, FIRE)
	for(var/turf/T in view(1, source))
		new /obj/effect/temp_visual/fire/fast(T)
		for(var/mob/living/L in T)
			if(L == source)
				continue
			if(special_checks_faction && source.faction_check_atom(L))
				continue
			L.apply_damage(20, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
			L.apply_damage(2, FIRE)
	..()
...
/obj/item/ego_weapon/shield/waxen/attack(mob/living/target, mob/living/carbon/human/user)
	...
	Check_Burn(user)
	..()
	user.apply_damage(burn_self, FIRE)
	if(user != target)
		target.apply_damage(burn_enemy, FIRE)
...
/obj/item/ego_weapon/shield/waxen/afterattack(atom/A, mob/living/user, proximity_flag, params)
	...
	for(var/turf/T in get_line(user, get_ranged_target_turf_direct(user, A, 6)))
		if(T.density)
			break
	...
	playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/firebird/Firebird_Hit.ogg', 50, TRUE)
...
/obj/item/ego_weapon/wield/ochre/afterattack(atom/target, mob/living/user, proximity_flag, clickparams)
	...
	playsound(user, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge.ogg', 100, TRUE)
	G.firer = user
...
/obj/projectile/ego_bullet/ochre
	...
	damage = 80
	damage_type = BRUTE
	hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/nail2.ogg'
...
/obj/item/ego_weapon/lance/miasma/proc/VenomBlast(mob/living/carbon/human/user, mob/target)
	...
	for(var/turf/T in view(1, target))
		...
		FX.color = "#96BB00"
		user.HurtInTurf(T, list(), damage_dealt, BRUTE, check_faction = TRUE)
	return
	...
	for(var/i = 1 to 4)
		...
		if(target in view(reach,user))
			switch(i)
				...
				if(4)
					hitsound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/noon_bite.ogg'
					force *= 1.5
