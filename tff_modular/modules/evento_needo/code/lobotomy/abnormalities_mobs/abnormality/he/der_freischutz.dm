/mob/living/simple_animal/hostile/abnormality/der_freischutz
	...
	retreat_distance = 2
	move_to_delay = 6
	damage_coeff = list(BURN = 1, BRAIN = 2, BRUTE = 0.7, TOX = 0.5, FIRE = 0.5)
...
/datum/action/cooldown/switch_portals/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/datum/action/cooldown/remove_portal/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if (!istype(owner, /mob/living/simple_animal/hostile/abnormality/der_freischutz))
		return
...
/mob/living/simple_animal/hostile/abnormality/der_freischutz/proc/PrepareFireBullet(atom/target)
	...
	var/list/turfs_to_check = get_line(beam_start, target_turf)
	playsound(beam_start, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/prepare.ogg', 35, 0, 20)
	face_atom(target)
	...
	SLEEP_CHECK_DEATH(bullet_fire_delay, src)
...
/mob/living/simple_animal/hostile/abnormality/der_freischutz/proc/FireBullet(atom/target, turf/start_turf, turf/end_turf)
	playsound(start_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/shoot.ogg', 35, 0, 20)
	...
	B.original = end_turf
	B.set_angle(get_angle(start_turf, end_turf))
...
/mob/living/simple_animal/hostile/abnormality/der_freischutz/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/der_freischutz/ZeroQliphoth(mob/living/carbon/human/user)
	...
	for(var/mob/M in GLOB.mob_living_list)
		if(istype(M,/mob/living/simple_animal/bot) || (src.z != M.z) || (M.stat == DEAD) || (HAS_TRAIT(M, TRAIT_GODMODE)))
			continue
		targets += M
	...
	for(var/turf/T in GLOB.generic_event_spawns)
		if(istype(get_area(T),/area/station/hallway))
			centralturf = T
...
/mob/living/simple_animal/hostile/abnormality/der_freischutz/proc/fire_magic_bullet(target = get_turf(pick(GLOB.generic_event_spawns)), freidir = pick(EAST,WEST))
	IconChange(firing = TRUE)
	var/offset = -12
	var/list/portals = list()
	var/turf/T = src.loc
	...
	if (freidir == EAST)
		...
		T = locate(tpos.x, tpos.y - 5, tpos.z)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/prepare.ogg', 100, 0, 20)
	playsound(src.loc, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/prepare.ogg', 100, 0)
	for(var/i=1, i<5, i++)
		...
		if(i != 4)
			...
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/shoot.ogg', 100, 0, 20)
			B.dir = freidir
...
/mob/living/simple_animal/hostile/der_freis_portal/Initialize()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/portal.ogg', 100, 0, 10)
