...
/mob/living/simple_animal/hostile/abnormality/snow_queen
	...
	blood_volume = 0
	move_to_delay = 5
	damage_coeff = list(BURN = 1, BRAIN = 1.1, BRUTE = 0.8, TOX = 0.8, BRUTE = 0.8, FIRE = 2) //ASK SOMEONE GOOD AT BALANCING ABOUT THIS -IP
	base_pixel_x = -16
	pixel_x = -16
	can_breach = TRUE
	del_on_death = FALSE
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/SuccessEffect(mob/living/carbon/human/user)
	if(CheckArmor(user))
		FreezeEmployee(user)
		return
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/FailureEffect(mob/living/carbon/human/user)
	if(CheckArmor(user))
		FreezeEmployee(user)
		return
	if(prob(90))
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/try_working(mob/living/carbon/human/user)
	var/work_type = tgui_input_list(user, "Should I try rescuing?", "Choice", list("Rescue", "Ignore"))
	. = ..()
	if(!.)
		balloon_alert(user, "not yet")
		return
	if(!work_type)
		return
	if(work_type == "Rescue")
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/AttackingTarget(atom/attacked_target)
	...
	if(slash_cooldown < world.time && (prob(50) || arena_attacks))
		if(arena_attacks)
			can_act = FALSE
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/unsheathed_blade.ogg', 55, 0, 5)
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/Slash(target, wide = TRUE)
	...
	var/smash_width = 2
	if(wide)
		playsound(get_turf(src), 'sound/items/weapons/bladeslice.ogg', 50, 0, 5)
	else
		playsound(get_turf(src), 'sound/items/weapons/bladeslice.ogg', 50, 0, 5)
		smash_length = 4
		smash_width = 1
	...
	for(var/turf/T in area_of_effect)
		...
		for(var/mob/living/L in HurtInTurf(T, list(), 20, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
			playsound(get_turf(src), 'sound/effects/magic/teleport_app.ogg', 30, 1)
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/RegisterCleaveZones()
	...
	for(var/turf/T in circle_range_turfs(arena_center, 7))
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/ReturnNoOverlap(list/area_one, list/area_two)
	...
	return unique_list(unique_list)
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/BringToArena(mob/living/simple_animal/hostile/abnormality/snow_queen/queen, mob/living/carbon/hero, obj/structure/snowqueened_employee/prison)
	...
	for(var/obj/item/E in hero.get_all_contents())
		if(!is_ego_weapon(E))
			continue
		hero.dropItemToGround(E, TRUE, TRUE)
	hero.forceMove(arena_landmarks[ICE_ARENA_HERO_SPAWN])
	storybook_hero = hero
	RegisterSignal(hero, COMSIG_QDELETING, PROC_REF(WinterContinues))
	...
	notify_ghosts("[hero] is challenging the Snow Queen to a duel!", source = src, header="Something Interesting!")
	return TRUE
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/ChillingCleave(snow_health)
	...
	for(var/i = 0 to 3)
		...
		//	playsound(get_turf(src), 'sound/effects/magic/magic_missile.ogg', 50, 0, 4)
		//	break
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/ProjectileHell(mob/living/L)
	for(var/turf/shootems in teleport_locations)
		...
		playsound(shootems, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/despairknight/attack.ogg', 50, 0, 4)
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/AoeTurfEffect(turf/T, duration, telegraph = FALSE)
	...
	if(telegraph)
		return
	return HurtInTurf(T, list(), 35, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/snow_queen/proc/BladeDash(dash_target)
	...
	//			hit_mob = HurtInTurf(T, hit_mob, 20, BRUTE, null, TRUE, FALSE, TRUE, hurt_structure = FALSE)
	can_act = TRUE
	icon_state = "snowqueen"
	update_icon()
...
/datum/reusable_visual_pool/proc/NewSnowQueenEffect(turf/location, duration = 10, telegraph = FALSE)
	...
	if(telegraph)
		...
		RV.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi'
		RV.icon_state = "cracks_dark"
	else
		RV.name = "ice spike"
		RV.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/32x48.dmi'
...
/obj/structure/snowqueened_employee
	...
	//icon = 'icons/obj/fluff/flora/icedecor.dmi'
	max_integrity = 600
	anchored = TRUE
	density = FALSE
...
/obj/structure/snowqueened_employee/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/ego_weapon/shield/ice_sword))
		...
		//playsound(get_turf(src), 'sound/effects/foostep/glass_step.ogg', 50, TRUE)
		return
	return ..()
...
/obj/structure/chair/snowqueen
	...
	desc = "An icey throne."
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/160x160.dmi'
...
/obj/structure/frozensword/attack_hand(mob/user) //Add code to open the gate when its drawn
	...
	if(!connected_abno)
		...
		if(connected_abno)
			RegisterSignal(connected_abno, COMSIG_QDELETING, PROC_REF(Refresh))
	if(dueling_sword)
		user.put_in_hands(dueling_sword)
		RegisterSignal(dueling_sword, COMSIG_QDELETING, PROC_REF(Refresh))
