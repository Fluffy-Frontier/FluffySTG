...
/mob/living/simple_animal/hostile/abnormality/red_shoes
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Attack.ogg'
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/death()
	...
	if(possessee)//hopefully this refers to the specific individual
		...
		REMOVE_TRAIT(possessee, TRAIT_GODMODE, ADMIN_TRAIT)
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/proc/Apply_Desire(mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Activate.ogg', 100, 1)
	return

/mob/living/simple_animal/hostile/abnormality/red_shoes/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/red_shoes/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/ZeroQliphoth(mob/living/carbon/human/user)//silent girl with extra steps
	...
	if(possessee)//If the first check fails
		return
	addtimer(CALLBACK(src, PROC_REF(after_qli)), 30)

/mob/living/simple_animal/hostile/abnormality/red_shoes/proc/after_qli()
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/proc/Assimilate(mob/living/carbon/user)
	...
	if(ishuman(H) && (H.sanity_lost))
		user.forceMove(src)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Activate.ogg', 50, 1)
		name = user.name
		...
		add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/ego_righthand.dmi', "sanguine", -ABOVE_MOB_LAYER))
		cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x64_effects.dmi', "panicked", -ABOVE_MOB_LAYER))
		BreachEffect(user)
		return TRUE
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/BreachEffect(mob/living/carbon/human/user, breach_type)
	...
	if(!possessee)
		...
		ChangeResistances(list(BURN = 0.5, BRAIN = 1.5, BRUTE = 1, TOX = 1.5))
		//sleep(10)
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/Found(atom/A)//The solo breach generally sticks together
	if(istype(A, /mob/living/simple_animal/hostile/red_shoe))
		...
		if(S.stat != DEAD && !S.target && !S.client && faction_check_atom(S))//cannibalized from steel ordeals
...
/mob/living/simple_animal/hostile/abnormality/red_shoes/proc/ChopFeet(mob/living/carbon/human/H)
	...
	if(!l_foot && !r_foot)//stops spamming the sound effect after being applied once
		return
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Kill.ogg', 100, 1)
	l_foot?.dismember()
	r_foot?.dismember()
...
/datum/status_effect/red_possess/on_apply()
	...
	var/usertemp = (status_holder.get_clothing_class_level(CLOTHING_SERVICE))
	...
	status_holder.apply_damage(desire_damage, BRUTE) //DIE!
	status_holder.physiology.brain_mod += 0.5//By using bonuses, this lowers your maximum prudence
...
/datum/status_effect/red_possess/on_remove()
	if(!ishuman(owner))
		return
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.brain_mod -= 0.5//Return prudence back to normal
	if(status_holder.sanity_lost)
...
/datum/ai_controller/insane/red_possess/SelectBehaviors(delta_time)//Selects red shoes as the target
	...
	if (!shoes)//No runtimes after suppression, woohoo!
		return
	if(HAS_TRAIT(shoes, TRAIT_GODMODE))//don't get possessed if it's already breaching
...
/datum/ai_behavior/desire_move/perform(delta_time, datum/ai_controller/controller)//Paths the pancicked to red shoes, causes runtimes if it's dead
	...
	if(!ishuman(living_pawn))
		return
	walkspeed -= (max(0.95,((living_pawn.get_clothing_class_level(CLOTHING_ARMORED)) * 0.1)))//one-hundreth of a second for every point of justice, capped at 95
...
/datum/ai_behavior/desire_move/proc/Movement(datum/ai_controller/controller)//Ditto
	...
	if(!target)
		if(living_pawn)//there's a runtime if you panic right next to it
			living_pawn.SanityLossEffect()//switch to a murder panic
		return
	if(!LAZYLEN(current_path))
		living_pawn.SanityLossEffect()//ditto
		return
...
/mob/living/simple_animal/hostile/red_shoe
	...
	gender = NEUTER
	damage_coeff = list(BURN = 0.5, BRAIN = 1, BRUTE = 1.5, TOX = 1.5)
	del_on_death = TRUE
	death_message = "crumples into a pile of bones."
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redshoes/RedShoes_Attack.ogg'
	melee_damage_lower = 15
