...
/mob/living/simple_animal/hostile/abnormality/basilisoup
	...
	ranged = TRUE
	fear_level = HE_LEVEL
	damage_coeff = list(BURN = 1, BRAIN = 0.8, BRUTE = 1.2, TOX = 0.8, BRUTE = 1.5, FIRE = 0.6)
	...
	update_qliphoth = 0
/mob/living/simple_animal/hostile/abnormality/basilisoup/apply_damage(damage, damagetype, def_zone, blocked, forced, spread_damage, wound_bonus, exposed_wound_bonus, sharpness, attack_direction, attacking_item, wound_clothing)
	. = ..()
...
/mob/living/simple_animal/hostile/abnormality/basilisoup/try_working(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/basilisoup/PostWorkEffect(mob/living/carbon/human/user)
	. = ..()
...
/mob/living/simple_animal/hostile/abnormality/basilisoup/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
...
/mob/living/simple_animal/hostile/abnormality/basilisoup/proc/Spit(atom/target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/basilisoup/spit1.ogg', 120, 1, 3)
	icon_state = "basilisoup_prepare"
	SLEEP_CHECK_DEATH(20, src)
	icon_state = "basilisoup"
	spit_cooldown = world.time + spit_cooldown_time
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/basilisoup/spit2.ogg', 75, 1, 3)
	for(var/k = 1 to 3)
...
/mob/living/simple_animal/hostile/abnormality/basilisoup/proc/charge(move_dir, times_ran, target)
	...
	for(var/turf/U in range(1, T))
		var/list/new_hits = HurtInTurf(U, been_hit, 0, BRUTE, hurt_mechs = TRUE) - been_hit
		been_hit += new_hits
...
/obj/structure/basilisoup_pot/attack_hand(mob/living/carbon/human/user)
	. = ..()
	if(.)
		return
	user.changeNext_move(CLICK_CD_MELEE)
	if(soup_level >= 20 && !user.combat_mode)
		dump_soup(null, user)
		return
...
/obj/structure/basilisoup_pot/attackby(obj/item/object, mob/user, params)
	if(istype(object, /obj/item/reagent_containers/cup/bowl))
		if(soup_level >= 20)
...
/obj/structure/basilisoup_pot/post_buckle_mob(mob/living/carbon/human/soup_sacrifice)
	...
	qdel(soup_sacrifice)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bloodbath/Bloodbath_EyeOn.ogg', 80, FALSE, -3)
