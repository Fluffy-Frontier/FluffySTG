...
/mob/living/simple_animal/hostile/abnormality/golden_apple
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Attack2.ogg'
	maxHealth = 1200
	health = 1200
	light_color = "D4FAF37"
	light_range = 5
	...
	attack_sound = "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Attack.ogg"
	stat_attack = HARD_CRIT
	melee_damage_lower = 10
	melee_damage_upper = 15
	melee_damage_type = BRUTE
	damage_coeff = list(BURN = 1, BRAIN = 1.5, BRUTE = 1, TOX = 0.5, BRUTE = 2)
...
/datum/action/cooldown/gapple_pulse
	name = "Golden sheen"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/status_sprites.dmi'
...
/datum/action/cooldown/gapple_pulse/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	...
	if(apple.IsContained()) // No more using cooldowns while contained
		return FALSE
	if(apple.is_maggot == TRUE)//False apple shouldn't have this ability, and it should not be usable outside RCA
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/Life()
	...
	if(!.)
		return
	if((pulse_cooldown < world.time) && !(HAS_TRAIT(src, TRAIT_GODMODE)) && !is_maggot)//First form's regular heal pulse
		HealPulse()
		return
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/say(message as text, language, ignore_spam, forced, sanitize, spans, message_mods, message_range, saymode)//UNDER CONSIDERATION: give it access to comms for R corp assault
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/HealPulse(manual = FALSE)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Sparkle.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/Apply_Sheen(mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Sparkle.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/SuccessEffect(mob/living/carbon/human/user)
	Apply_Sheen(user)
	return
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/golden_apple/PostWorkEffect(mob/living/carbon/human/user)
	..()
	var/datum/status_effect/stacking/golden_sheen/G = user.has_status_effect(/datum/status_effect/stacking/golden_sheen)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/death()
	if(health > 0)
		return
	if(!is_maggot)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Attack.ogg', 100, 1)
		addtimer(CALLBACK(src, PROC_REF(EatEmployees)), 15 SECONDS)
		return ..()
	density = FALSE
	for(var/atom/movable/AM in src) //morph code
		AM.forceMove(loc)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/False_Dead.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/BecomeRotten()//phase 2
	...
	HandleAbiltyButtons()
	revive(HEAL_ALL)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/Gold_Falsify.ogg', 50, 1)//it's very loud
	...
	light_range = 0
	light_power = 0
	attack_sound = "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/False_Attack3.ogg"
	melee_damage_lower = 30
	melee_damage_upper = 45
	attack_verb_continuous = "pummels"
	attack_verb_simple = "pummel"
	ChangeResistances(list(BURN = 1.5, BRAIN = 1.5, BRUTE = 1, TOX = 0.5))
	melee_damage_type = BRUTE
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/EatEmployees()
	...
	var/target_hit
	can_act = FALSE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/False_Attack2.ogg', 100, 0, 5)
	for(var/turf/T in view(1, src))
		new /obj/effect/temp_visual/smash_effect(T)
		for(var/mob/living/carbon/L in HurtInTurf(T, list(), 200, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
			if(L.stat >= SOFT_CRIT)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/DigestPerson(mob/living/carbon/human/H)//berserk mode
	...
	BecomeRotten()
	move_to_delay = 0.5
	ChangeResistances(list(BURN = 1, BRAIN = 1, BRUTE = 0.8, TOX = 0.3))
	if(H)
...
/mob/living/simple_animal/hostile/abnormality/golden_apple/proc/Smash(target, wide = TRUE)
	...
	var/smash_width = 2
	if(wide)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/False_Attack.ogg', 50, 0, 5)
	else
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/goldenapple/False_Attack2.ogg', 50, 0, 5)
		smash_length = 4
		smash_width = 1
	middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, dir_to_target, smash_length))
	...
	for(var/turf/T in middle_line)
		if(T.density)
			break
		for(var/turf/Y in get_line(T, get_ranged_target_turf(T, upline, smash_width)))
			if (Y.density)
				break
			if (Y in area_of_effect)
				continue
			area_of_effect += Y
		for(var/turf/U in get_line(T, get_ranged_target_turf(T, downline, smash_width)))
			if (U.density)
				break
	...
	for(var/turf/T in area_of_effect)
		new /obj/effect/temp_visual/smash_effect(T)
		for(var/mob/living/L in HurtInTurf(T, list(), smash_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
...
/datum/status_effect/stacking/maggots/tick()//change this to golden apple's life tick for less lag
	var/mob/living/carbon/human/status_holder = owner
	status_holder.apply_damage(stacks, BRUTE)
