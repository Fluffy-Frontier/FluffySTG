/mob/living/simple_animal/hostile/abnormality/woodsman
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_attack.ogg'
	melee_damage_lower = 15
	melee_damage_upper = 30
	damage_coeff = list(BURN = 1.5, BRAIN = 0.8, BRUTE = 0.8, TOX = 1)
	vision_range = 14
...
/obj/effect/proc_holder/spell/pointed/axe_throw/cast(list/targets, mob/user)
	...
	P.firer = user
	P.set_angle(get_angle(user, target))
	P.fire()
...
/mob/living/simple_animal/hostile/abnormality/woodsman/proc/pull_target(distance)
	...
	playsound(target_turf, 'sound/items/weapons/chainhit.ogg', 50, TRUE)
...
/mob/living/simple_animal/hostile/abnormality/woodsman/proc/Heal(mob/living/carbon/human/body)
	...
	for(var/obj/item/organ/O in body.get_organ_slot(BODY_ZONE_CHEST, TRUE))
...
/mob/living/simple_animal/hostile/abnormality/woodsman/proc/Woodsman_Flurry(target)
	...
	SLEEP_CHECK_DEATH(flurry_delay, src)
	for (var/i = 0; i < flurry_count; i++)
		icon_state = icon_living
		var/list/been_hit = list()
		for(var/turf/T in area_of_effect)
			new /obj/effect/temp_visual/smash_effect(T)
			been_hit = HurtInTurf(T, been_hit, i > 6 ? flurry_big : flurry_small, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
		if (i > 6)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_strong.ogg', 100, 0, 8) // BAM
		else
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/woodsman/woodsman_attack.ogg', 75, 0, 5)
		SLEEP_CHECK_DEATH(flurry_pause, src)
		icon_state = "woodsman_prepare"
	icon_state = icon_living
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/woodsman/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/woodsman/OnQliphothChange()
	. = ..()
...
/mob/living/simple_animal/hostile/abnormality/woodsman/try_working(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/woodsman/user_buckle_mob(mob/living/M, mob/user, check_loc)
	...
	var/obj/item/organ/heart/O = M.get_organ_by_type(/obj/item/organ/heart)
	if(istype(O))
		O.Remove(M)
		QDEL_NULL(O)
