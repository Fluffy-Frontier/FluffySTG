...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/piscinemermaid/waterjump.ogg'
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/piscinemermaid/splashattack.ogg'
	del_on_death = FALSE
	maxHealth = 1500
	health = 1500
	pixel_x = -12
	base_pixel_x = -12
	damage_coeff = list(BURN = 1.5, BRAIN = 1.5, BRUTE = 0.5, TOX = 2) //not that bad without a lover
	rapid_melee = 2
...
/datum/action/innate/change_icon_merm/Activate()
	. = ..()
	owner.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/48x32.dmi'
...
/datum/action/innate/change_icon_merm/Deactivate()
	. = ..()
	owner.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/64x64.dmi'
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/SuccessEffect(mob/living/carbon/human/user)
	if(!crown)
		GiveGift(user)
		return
	if(crown?.loved == user)
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/PostWorkEffect(mob/living/carbon/human/user)
	if(HAS_TRAIT(src, TRAIT_GODMODE))
		icon_living = "pmermaid_standing"
		icon_state = "pmermaid_standing"
		workingflag = FALSE
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/BreachEffect()
	...
	if(!isnull(crown?.loved))
		ChangeResistances(list(BURN = 0.3, BRAIN = 0.3, BRUTE = 0.3, TOX = 0.3)) //others can still help but it's going to take a lot of damage
		...
		if(T)
			...
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/piscinemermaid/waterjump.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/Life()
	...
	if(love_target.stat == DEAD)
		...
		ChangeResistances(list(BURN = 1.5, BRAIN = 1.5, BRUTE = 0.5, TOX = 2)) //back to being a pushover
		love_target = null
		return
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/OnQliphothChange(amount)
	..()
	if(crown?.loved)
...
/mob/living/simple_animal/hostile/abnormality/pisc_mermaid/proc/GiveGift(mob/living/carbon/human/user)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/piscinemermaid/bigsplash.ogg', 50, 1)
	UC.mermaid = src
...
/obj/effect/mermaid_water/unbuckle_mob(mob/living/carbon/human/buckled_mob, force, can_fall)
