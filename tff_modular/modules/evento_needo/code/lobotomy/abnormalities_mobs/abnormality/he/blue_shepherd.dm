...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.6, BRUTE = 1, TOX = 0.8, BRUTE = 1.5)
...
/datum/action/cooldown/evade/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	. = ..()
	if (sidesteping)
		MoveVFX()
...
/datum/action/cooldown/parry/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if (istype(owner, /mob/living/simple_animal/hostile/abnormality/blue_shepherd))
		var/mob/living/simple_animal/hostile/abnormality/blue_shepherd/H = owner
		if(H.no_counter)
			...
			H.ChangeResistances(list(BURN = 0, BRAIN = 0, BRUTE = 0, TOX = 0))
			H.countering = TRUE
			H.slashing = TRUE
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/proc/counter()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/finisher2.ogg', 75, TRUE, 2)
	for(var/turf/T in range(2, src))
		new /obj/effect/temp_visual/smash_effect(T)
		been_hit = HurtInTurf(T, been_hit, counter_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE, mech_damage = 15)
...
/datum/action/cooldown/parry/proc/endcounter()
	if (istype(owner, /mob/living/simple_animal/hostile/abnormality/blue_shepherd))
		...
		H.slashing = FALSE
		H.color = null
		H.ChangeResistances(list(BURN = 0.6, BRAIN = 1, BRUTE = 0.8, TOX = 1.5))
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/try_working(mob/living/carbon/human/user)
	var/mob/living/simple_animal/hostile/abnormality/red_buddy/buddy_abno = buddy?.current
	if(buddy_abno)
		neutral_chance = clamp(buddy_abno.suffering * 0.5, 1, 100) //the more red buddy suffers, the higher your work chance on shepherd is
	else
		neutral_chance = 40
	return ..()


/mob/living/simple_animal/hostile/abnormality/blue_shepherd/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/PostWorkEffect(mob/living/carbon/human/user)
	...
	if(user.get_major_clothing_class() == CLOTHING_ARMORED)
		qliphoth_change(1)
	else if(user.get_major_clothing_class() == CLOTHING_SERVICE)
		hired = TRUE
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/proc/slash()
	...
	for(var/i = 0 to range)
		playsound(src, 'sound/items/weapons/slice.ogg', 75, FALSE, 4)
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/proc/SlashHit(turf/T, list/all_turfs, slash_count, buddy_hit)
	if(stat == DEAD)
		return
	new /obj/effect/temp_visual/smash_effect(T)
	for(var/mob/living/L in HurtInTurf(T, list(), slash_damage, BRUTE, check_faction = combat_map, hurt_mechs = TRUE, hurt_structure = TRUE, break_not_destroy = TRUE))
		...
		L.apply_damage(slash_damage, BRUTE)
		all_turfs -= T
...
/mob/living/simple_animal/hostile/abnormality/blue_shepherd/proc/Lying(mob/living/simple_animal/hostile/abnormality/red_buddy/buddy_abno, mob/living/carbon/human/user)
	...
	if(prob(50) && LAZYLEN(abno_list)) //decide which subject to pick
		...
		if((!(HAS_TRAIT(abno, TRAIT_GODMODE)) && !lie) || ((HAS_TRAIT(abno, TRAIT_GODMODE)) && lie))
