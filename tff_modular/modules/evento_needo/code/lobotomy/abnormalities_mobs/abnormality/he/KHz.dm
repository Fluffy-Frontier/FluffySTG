...
/mob/living/simple_animal/hostile/abnormality/khz/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	var/work_type = tgui_input_list(user, "Choose work type", "Choice", list("Input One", "Input Zero"))
	if(!work_type)
		return
...
/mob/living/simple_animal/hostile/abnormality/khz/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/khz/proc/SetInput()
	...
	switch(output)
		if(1)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip1.ogg', 300, 8)
			input = 8	//01000
		if(2)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip2.ogg', 300, 8)
			input = 25	//11001
		if(3)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip3.ogg', 300, 8)
			input = 24	//11000
		if(4)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip4.ogg', 300, 8)
			input = 13	//01101
		if(5)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip5.ogg', 300, 8)
			input = 26	//11010
		if(6)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip6.ogg', 300, 8)
			input = 16	//10000
		if(7)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/khz/Clip7.ogg', 300, 8)
			input = 28	//11100
...
/obj/item/paper/fluff/radio_call
	name = "Radio Call"
	default_raw_text = {"Input and send back. <br>
