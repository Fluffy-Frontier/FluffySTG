...
/mob/living/simple_animal/hostile/abnormality/pinocchio
	...
	damage_coeff = list(BURN = 1.2, BRAIN = 0.5, BRUTE = 0.7, TOX = 0.9, FIRE = 1.5)
	ego_list = list(
...
//Work/Misc
/mob/living/simple_animal/hostile/abnormality/pinocchio/try_working(mob/living/carbon/human/user)
	if(realboy)
	...
	if(!.)
		return
	if(lying)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pinocchio/success.ogg', 40, 0, 1)
		caught_lie = TRUE
		lying = FALSE
		qliphoth_change(1)
		PostWorkEffect()
...
/mob/living/simple_animal/hostile/abnormality/pinocchio/PostWorkEffect(mob/living/carbon/human/user)
	if(lying)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pinocchio/lie.ogg', 40, 0, 1)
		qliphoth_change(-1)
...
/mob/living/simple_animal/hostile/abnormality/pinocchio/BreachEffect(mob/living/carbon/human/user)
	...
	realboy.pixel_z = 0
	realboy.physiology.burn_mod *= 1.2 //Matches wearing its own E.G.O.
	realboy.physiology.brain_mod *= 0.5
	realboy.physiology.brute_mod *= 0.7
	realboy.physiology.tox_mod *= 0.9
...
/datum/ai_controller/insane/murder/puppet/CanTarget(atom/movable/thing)
	...
	if(. && isliving(thing) && living_pawn.faction_check_atom(thing))
		return FALSE
...
/mob/living/carbon/human/species/pinocchio/UnarmedAttack(atom/attack_target, proximity_flag, list/modifiers)
	if((istype(attack_target, /obj/structure/toolabnormality/touch)) || (istype(attack_target, /obj/structure/bough)))
...
/datum/species/puppet
	...
	attack_verb = "slash"
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pinocchio/attack.ogg'
	miss_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pinocchio/attack.ogg'
