...
/mob/living/simple_animal/hostile/abnormality/porccubus
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.5, BRUTE = 1, TOX = 1.5)
	ranged = TRUE
	...
	wander = FALSE //it can't move anyway but why not
	stat_attack = HARD_CRIT
	melee_damage_type = BRUTE
	can_breach = TRUE
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_death.ogg'
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_attack.ogg'
...
//Work Code
/mob/living/simple_animal/hostile/abnormality/porccubus/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	agent_ckey = user //just in case the agent goes insane midwork

/mob/living/simple_animal/hostile/abnormality/porccubus/PostWorkEffect(mob/living/carbon/human/user)
	if(user.sanity_lost) //if the person is driven insane mid work
		DrugOverdose(user, agent_ckey)
	agent_ckey = null

/mob/living/simple_animal/hostile/abnormality/porccubus/SuccessEffect(mob/living/carbon/human/user)
	qliphoth_change(1)
	var/datum/status_effect/porccubus_addiction/PA = user.has_status_effect(STATUS_EFFECT_ADDICTION)

	if(PA)
		PA.IncreaseTolerance()
	else if(user.get_clothing_class_level(CLOTHING_SERVICE) < 3)
...
/mob/living/simple_animal/hostile/abnormality/porccubus/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/porccubus/Life()
	...
	if(teleport_cooldown < world.time) //if porccubus hasn't taken damage for 5 minutes we make him move so he doesn't stay stuck in whatever cell he got thrown in.
		...
		if(damage_taken)
			damage_taken = FALSE
			return
		var/turf/T = pick(GLOB.generic_event_spawns - get_turf(src))
		forceMove(T)
		damage_taken = FALSE
...
/mob/living/simple_animal/hostile/abnormality/porccubus/proc/PorcDash(atom/target)//additionally, it can dash to its target every 15 seconds if it's out of range
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_giggle.ogg', 10, FALSE, 4) // This thing is absurdly loud
	ranged_cooldown = world.time + ranged_cooldown_time
	leap_charges -= 1
	if(!timer_added)
...
/mob/living/simple_animal/hostile/abnormality/porccubus/AttackingTarget(atom/attacked_target)
	...
	if(.)
		...
		if(!H.sanity_lost)
			return
		var/nirvana = FALSE
		if(H.get_clothing_class_level(CLOTHING_SERVICE) < 3) //if they have under 60 temp they actually get all the stats from overdose, otherwise they just get fucked.
			nirvana = TRUE
...
/datum/status_effect/porccubus_addiction/on_apply()
	...
	playsound(addict, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/porccu_giggle.ogg', 50, FALSE, 4)
	...
	addict.physiology.burn_mod -= attribute_gain
	addict.physiology.brain_mod -= attribute_gain
	addict.physiology.brute_mod -= attribute_gain
	addict.physiology.tox_mod -= attribute_gain
...
/datum/status_effect/porccubus_addiction/tick()
	if(withdrawal_cooldown < world.time)
		...
		addict.physiology.burn_mod += 1
		addict.physiology.brain_mod += 1
		addict.physiology.brute_mod += 1
		addict.physiology.tox_mod += 1
		sanity_gain--
		withdrawal_cooldown = withdrawal_cooldown_time + world.time
...
/datum/status_effect/porccubus_addiction/on_remove()
	...
	if(QDELETED(head))
		return
	playsound(addict, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/head_explode_laugh.ogg', 50, FALSE, 4)
	...
	playsound(addict, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/porccubus/head_explode.ogg', 50, FALSE, 4)
