/mob/living/simple_animal/hostile/abnormality/funeral
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/64x96.dmi' //HOW DO I TURN A PNG INTO THE DMI SPRITES AAAAAAAAAAAAAAA
	icon_state = "funeral"
	icon_living = "funeral"
	...
	move_to_delay = 4
	damage_coeff = list(BURN = 1, BRAIN = 0.5, BRUTE = 1.5, TOX = 1.0, BRUTE = 2)
...
/mob/living/simple_animal/hostile/abnormality/funeral/proc/SpiritGun(atom/target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/funeral/spiritgun.ogg', 75, 1, 3)
	cooler_target.remove_status_effect(/datum/status_effect/spirit_gun_target)
	can_act = TRUE
	gun_cooldown = world.time + gun_cooldown_time
	icon_state = icon_living
	var/line_of_sight = get_line(get_turf(src), get_turf(target)) //better simulates a projectile attack
	for(var/turf/T in line_of_sight)
		if(DensityCheck(T))
			return
	cooler_target.apply_damage(gun_damage, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/funeral/proc/ButterflySwarm(target)
	...
	middle_line = get_line(source_turf, get_ranged_target_turf(source_turf, dir_to_target, swarm_length))
	...
	SLEEP_CHECK_DEATH(1.75 SECONDS, src)
	icon_state = "funeral_coffin"
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/funeral/coffin.ogg', 40, extrarange = 10, ignore_walls = TRUE) // bwiiiiiiinng >flapping
	var/i = 0
	...
	switch(direction)
		if(EAST)
			if(!T.density) //lets middle line to go through airlocks
				hit_turfs |= T
			for(var/turf/Y in get_line(T, get_ranged_target_turf(T, NORTH, swarm_width)))
				if(DensityCheck(Y)) //prevents swarm width from going through airlocks
					break
				hit_turfs |= Y
			for(var/turf/U in get_line(T, get_ranged_target_turf(T, SOUTH, swarm_width)))
				if(DensityCheck(U))
					break
				hit_turfs |= U
		if(WEST)
		...
		if(SOUTH)
			if(!T.density)
				hit_turfs |= T
			for(var/turf/Y in get_line(T, get_ranged_target_turf(T, EAST, swarm_width)))
				if(DensityCheck(Y))
					break
				hit_turfs |= Y
			for(var/turf/U in get_line(T, get_ranged_target_turf(T, WEST, swarm_width)))
				if(DensityCheck(U))
					break
				hit_turfs |= U
		if(NORTH)
...
/mob/living/simple_animal/hostile/abnormality/funeral/proc/SwarmTurfLinger(turf/T)
	for(var/i = 1 to 40) //40 times
		for(var/mob/living/carbon/human/H in HurtInTurf(T, list(), swarm_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE, hurt_structure = TRUE))
			if(H.stat == DEAD)
				continue
			if(H.sanity_lost)
				H.death()
				KillAnimation(H)
		SLEEP_CHECK_DEATH(0.25 SECONDS, src) //10 seconds
...
/mob/living/simple_animal/hostile/abnormality/funeral/proc/KillAnimation(mob/living/carbon/human/killed)
	...
	funeral_overlay.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/32x32.dmi'
...
/mob/living/simple_animal/hostile/abnormality/funeral/FailureEffect(mob/living/carbon/human/user)
	if(prob(80))
		qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/funeral/PostWorkEffect(mob/living/carbon/human/user)
	if(user.get_clothing_class_level(CLOTHING_ENGINEERING) >= 4)
		qliphoth_change(-1)
...
/datum/status_effect/spirit_gun_target/on_apply()
	. = ..()
	owner.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x32.dmi', "funeral_swarm", -MUTATIONS_LAYER))

/datum/status_effect/spirit_gun_target/on_remove()
	. = ..()
	owner.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x32.dmi', "funeral_swarm", -MUTATIONS_LAYER))
