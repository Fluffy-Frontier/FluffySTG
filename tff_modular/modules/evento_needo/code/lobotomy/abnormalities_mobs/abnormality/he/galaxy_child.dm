...
/mob/living/simple_animal/hostile/abnormality/galaxy_child/OnQliphothChange(amount)
	if(datum_reference.qliphoth_meter == 1) //qliphoth increased from 0
		icon_state = "galaxy"
		TurfTransform(/turf/open/floor/iron/dark)
...
/mob/living/simple_animal/hostile/abnormality/galaxy_child/proc/give_pebble(mob/living/carbon/human/new_friend)
	...
	RegisterSignal(new_friend, COMSIG_QDELETING, PROC_REF(on_friend_deletion))

/mob/living/simple_animal/hostile/abnormality/galaxy_child/proc/break_gifts()
	for(var/mob/living/carbon/human/friend as anything in galaxy_friends)
		friend.apply_damage(damage_amount, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/galaxy_child/proc/on_friend_death(mob/living/dead_friend, gibbed)
	...
	UnregisterSignal(dead_friend, COMSIG_QDELETING)
...
/mob/living/simple_animal/hostile/abnormality/galaxy_child/proc/on_friend_deletion(mob/deleted_friend)
	...
	UnregisterSignal(deleted_friend, COMSIG_QDELETING)
...
/datum/action/cooldown/friend_gift/Trigger(trigger_flags, atom/target)
	. = ..()
	if(!.)
		return FALSE
...
/datum/action/cooldown/galaxygiftbreak/Trigger(trigger_flags, atom/target)
	. = ..()
	if(!.)
		return FALSE
