/mob/living/simple_animal/hostile/abnormality/headless_ichthys
	...
	damage_coeff = list(BURN = 1, BRAIN = 1.2, BRUTE = 1.5, TOX = 0.5, BRUTE = 2)
	...
	var/jump_damage = 50
	var/jump_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hammer2.ogg'
	var/jump_aoe = 2
...
/datum/action/innate/abnormality_attack/BloodCannon
	name = "Blood Cannon"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/toolabnormalities.dmi'
	...
	for(var/h = 1 to beam_maximum_ticks) //from this point on it's basically the same as queenie's but with balance adjustments
		...
		for(var/turf/TF in hit_line)
			...
			for(var/mob/living/L in turfs_to_check)
				...
				if(faction_check_atom(L))
					continue
				if(dir != get_dir(src, L)) //so it doesn't hit people behind the mob
					continue
				already_hit += L
			...
			for(var/obj/vehicle/sealed/mecha/V in turfs_to_check)
				if(V in already_hit)
					continue
				V.take_damage(beam_damage, BRUTE, attack_dir = get_dir(V, src))
				already_hit += V
...
/mob/living/simple_animal/hostile/abnormality/headless_ichthys/proc/Enrage() //gains 25% more damage dealt and shorter cooldowns
	...
	beam_damage = 31
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hardslap.ogg'
	jump_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/ichthys/hammer3.ogg'
	jump_aoe = 3
	return
...
/mob/living/simple_animal/hostile/abnormality/headless_ichthys/FailureEffect(mob/living/carbon/human/user)
	if(prob(80))
		qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/headless_ichthys/death(gibbed)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg', 60, 1)
