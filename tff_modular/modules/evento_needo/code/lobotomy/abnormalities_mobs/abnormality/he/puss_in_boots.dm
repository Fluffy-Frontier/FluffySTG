...
//Blessing
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/try_working(mob/living/carbon/human/user)
	...
	if(user.stat != DEAD && !blessed_human && istype(user))
		...
		blessed_human = user
		Blessing(user)
		playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pussinboots/gatoblessing.ogg', 50, 0, 2)
		friendly = TRUE
...
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/PostWorkEffect(mob/living/carbon/human/user)
	ignored = FALSE
	return ..()
...
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/Life()
	...
	if(blessed_human.get_clothing_class_level(blessed_human.get_major_clothing_class()) >= 2) //no buffing to get ahead
		BlessedDeath(blessed_human)
		return
...
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/proc/GoToFriend()
	...
	for(var/turf/T in all_turfs)
		if(T == origin)
			continue
		var/available_turf
		var/list/blessed_human_line = get_line(T, blessed_human)
		...
		if(!available_turf)
			continue
		playsound(src, 'sound/items/weapons/fwoosh.ogg', 250, FALSE, 4)
		forceMove(T)
		if(friendly)
...
//	icon_state = "cat_prepare" maybe someday we'll have nice things
	...
	finishing = TRUE
	face_atom(target)
	T.add_overlay(icon('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "zorowarning"))
...
//	icon_state = "cat_dash" //ditto
	if(target in view(10, src))
		...
		T.add_overlay(icon('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "zoro"))
		addtimer(CALLBACK(T, TYPE_PROC_REF(/atom, cut_overlay), \
								icon('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "zoro")), 14)
		playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pussinboots/slash.ogg', 50, 0, 2)
		forceMove(jump_turf)
		if(ishuman(target))
		...
		playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/pussinboots/counterslash.ogg', 50, 0, 2)
		SLEEP_CHECK_DEATH(6, src)
		playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/crumbling/attack.ogg', 50, 1)
		Finisher(target)
	can_act = TRUE
	finishing = FALSE
	if(friendly)
...
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/proc/Finisher(mob/living/target) //This is super easy to avoid
	target.apply_damage(50, BRUTE, null, target.run_armor_check(null, BRUTE), spread_damage = TRUE) //50% of your health in red damage
	to_chat(target, span_danger("[src] is trying to cut you in half!"))
	if(!ishuman(target))
		target.apply_damage(100, BRUTE) //bit more than usual DPS in pale damage
		return
	if(target.health > 0)
		return
	var/mob/living/carbon/human/H = target
...
/mob/living/simple_animal/hostile/abnormality/puss_in_boots/death(gibbed)
	if(health <= 0)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/limbus_death.ogg', 50, 0, 2)
	density = FALSE
...
/datum/status_effect/chosen/on_apply()
	...
	user.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "inheritance", -MUTATIONS_LAYER))
	user.physiology.burn_mod *= 0.8
	user.physiology.brain_mod *= 0.8
	user.physiology.brute_mod *= 0.8
	user.physiology.tox_mod *= 0.8
...
/datum/status_effect/chosen/on_remove()
	if(!ishuman(owner))
		return
	var/mob/living/carbon/human/user = owner
	user.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "inheritance", -MUTATIONS_LAYER))
	user.physiology.burn_mod /= 0.8
	user.physiology.brain_mod /= 0.8
	user.physiology.brute_mod /= 0.8
	user.physiology.tox_mod /= 0.8
