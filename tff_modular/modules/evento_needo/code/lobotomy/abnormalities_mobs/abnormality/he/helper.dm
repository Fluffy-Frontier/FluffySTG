/mob/living/simple_animal/hostile/abnormality/helper
	...
	damage_coeff = list(BURN = 0.5, BRAIN = 1, BRUTE = 2, TOX = 2)
...
/mob/living/simple_animal/hostile/abnormality/helper/proc/helper_dash(target)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/rise.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/abnormality/helper/proc/do_dash(times_ran)
	...
	if(stop_charge)
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/disable.ogg', 75, 1)
		//var/area/A = get_area(get_turf(src))
		SLEEP_CHECK_DEATH(stuntime, src)
		charging = FALSE
		return
	forceMove(T)
	var/para = TRUE
	...
	playsound(src, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/move0[pick(1,2,3)].ogg", rand(dash_move_min_volune, dash_move_max_volune), 1)
	...
	for(var/mob/living/L in hit_turfs)
		if(!faction_check_atom(L))
			...
			L.apply_damage(dash_damage, melee_damage_type)
			L.blood_volume -= L.blood_volume * 0.05
			if(!ishuman(L))
				dash_damage = dash_damage / 2
			if(L.stat >= HARD_CRIT)
...
/mob/living/simple_animal/hostile/abnormality/helper/FailureEffect(mob/living/carbon/human/user)
	if(prob(80))
		qliphoth_change(-1)
	return
