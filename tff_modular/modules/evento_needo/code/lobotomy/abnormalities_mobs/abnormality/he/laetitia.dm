...
/datum/action/cooldown/laetitia_summon
	...
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi'
...
/datum/action/cooldown/laetitia_summon/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	...
	var/list/candidates = SSpolling.poll_ghost_candidates("Laetitia is calling for help! Are you willing to protect her?", poll_time=10 SECONDS)
...
/datum/action/cooldown/laetitia_gift/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if(!istype(owner, /mob/living/simple_animal/hostile/abnormality/laetitia))
		return FALSE
...
/obj/item/laetitia_gift/attack_self(mob/user)
	...
	if(do_after(user, 5 SECONDS, src))
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/laetitia/spider_born.ogg', 50, 1)
		if (istype(user, /mob/living))
			var/mob/living/L = user
			L.apply_damage((basepower*strength), BRUTE, null, L.run_armor_check(null, BRUTE), FALSE, TRUE)
		for(var/turf/T in range(2, user))
			new /obj/effect/temp_visual/smash_effect(T)
			user.HurtInTurf(T, list(), (basepower*strength), BRUTE, check_faction = FALSE, hurt_mechs = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/laetitia/SuccessEffect(mob/living/carbon/human/user)
	var/datum/status_effect/pranked/P = user.has_status_effect(STATUS_EFFECT_PRANKED)
	if(P && prob(30)) //30% to remove prank
		user.remove_status_effect(STATUS_EFFECT_PRANKED)
	return

/mob/living/simple_animal/hostile/abnormality/laetitia/FailureEffect(mob/living/carbon/human/user)
	var/datum/status_effect/pranked/P = user.has_status_effect(STATUS_EFFECT_PRANKED)
...
/mob/living/simple_animal/hostile/gift
	...
	health = 800
	pixel_x = -16
	damage_coeff = list(BURN = 0.8, BRAIN = 0.8, BRUTE = 1.2, TOX = 1)
	melee_damage_type = BRUTE
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/laetitia/spider_dead.ogg'
...
/datum/status_effect/pranked/on_apply()
	...
	if(user.get_clothing_class_level(CLOTHING_SERVICE) >= 4)
...
/datum/status_effect/pranked/tick()
	...
	if(prank_overlay && (status_holder.get_clothing_class_level(CLOTHING_SERVICE) < 3))
		return
	if(!ishuman(owner))
		return
	//i swear this is all necessary
	prank_overlay = new
	prank_overlay.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi'
...
/datum/status_effect/pranked/on_remove()
	...
	status_holder.apply_damage(200, BRUTE)//Usually a kill, you can block it if you're good
