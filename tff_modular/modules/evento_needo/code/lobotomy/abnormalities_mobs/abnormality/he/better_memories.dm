...
/mob/living/simple_animal/hostile/abnormality/better_memories/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/better_memories/proc/SpawnMinion(turf/spawn_turf)
	...
	RegisterSignal(spawningmonster, COMSIG_QDELETING, PROC_REF(MinionSlain))
	minions++
...
/mob/living/simple_animal/hostile/better_memories_minion
	...
	maxHealth = 1000
	melee_damage_type = BRUTE
	damage_coeff = list(BURN = 1.5, BRAIN = 1.5, BRUTE = 0.5, TOX = 0.5)
...
/mob/living/simple_animal/hostile/better_memories_minion/CanPassThrough(atom/blocker, turf/target, blocker_opinion)
	if(isliving(blocker))
		...
		if(M.mob_size <= MOB_SIZE_HUMAN ||  M.type == type)
			return TRUE
	return ..()
...
/mob/living/simple_animal/hostile/better_memories_minion/handle_automated_movement()
...
/mob/living/simple_animal/hostile/better_memories_minion/bullet_act(obj/projectile/P)
	...
	if(stat == CONSCIOUS && AIStatus != AI_OFF && !client)
		...
		if(!P.nodamage && second_on_hit_state != BULLET_ACT_BLOCK)
			...
			apply_effects(P.stun, P.knockdown, P.unconscious, P.slur, P.stutter, P.eyeblur, P.drowsy, secondarmor, P.stamina, P.jitter, P.paralyze, P.immobilize)
...
/mob/living/simple_animal/hostile/better_memories_minion/proc/FleeNow(turf/target_dest)
	...
	addtimer(CALLBACK(src, PROC_REF(set_varspeed), speed + 1), 1.5 SECONDS)
	fleeing_now = TRUE
	target = null
	if(Goto(FleeDest()))
...
/mob/living/simple_animal/hostile/better_memories_minion/proc/CameraFlash(mob/living/user)
	...
	playsound(loc, 'sound/items/weapons/armbomb.ogg', 75, TRUE, -3)
...
/datum/status_effect/display/better_memories_curse/on_apply()
	...
	L.physiology.brain_mod += 0.3
	L.physiology.brute_mod += 0.3
