/mob/living/simple_animal/hostile/abnormality/road_home
	...
	damage_coeff = list(BURN = 0.3, BRAIN = 2, BRUTE = 2, TOX = 2) //Endure red because catt mentions physical attacks can't hurt her at all.
	...
	fear_level = HE_LEVEL
	wander = FALSE
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_NormalAtk.ogg'
	ego_list = list(
...
/mob/living/simple_animal/hostile/abnormality/road_home/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/road_home/proc/CounterAttack(mob/living/attacker)
	...
	for(var/obj/effect/golden_road/GR in user_turf.contents)
		retaliation = 3
	attacker.apply_damage(retaliation, BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/road_home/Life()
	..()
	if(!HAS_TRAIT(src, TRAIT_SPINNING))
		spin(10 SECONDS, 2)
...
/mob/living/simple_animal/hostile/abnormality/road_home/proc/FlipAttack()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_NormalAtk.ogg', 100, FALSE, 8)
...
/mob/living/simple_animal/hostile/abnormality/road_home/proc/NewHouse()
	...
	var/road_home_turf = pick(GLOB.generic_event_spawns - get_turf(src))
...
/obj/road_house/proc/FallDown()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/Cartoon_Falling_Sound_Effect.ogg', 100, FALSE, 8, falloff_exponent = 1.25)
	sleep(fall_speed)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/roadhome/House_HouseBoom.ogg', 100, FALSE, 8)
	...
	for(var/mob/living/L in view(6, src))
		if(!road_home_mob.faction_check_atom(L))
			var/distance_decrease = get_dist(src, L) * 75
			L.apply_damage((600 - distance_decrease), BRUTE) //white damage so they can join the road home..
			if(!ishuman(L))
				continue
