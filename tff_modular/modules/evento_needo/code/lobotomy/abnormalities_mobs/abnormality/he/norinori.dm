...
/datum/action/cooldown/norisplit
	name = "Split"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/32x32.dmi'
	button_icon_state = "norinori"
	check_flags = AB_CHECK_CONSCIOUS
	transparent_when_unavailable = TRUE
	cooldown_time = 8 SECONDS

/datum/action/cooldown/norisplit/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/norinori/proc/KillCheck(mob/living/carbon/human/user)
	...
	playsound(src, 'sound/effects/blob/blobattack.ogg', 150, FALSE, 4)
	playsound(src, 'sound/items/weapons/chainsawhit.ogg', 200, FALSE, 4)
	...
	SLEEP_CHECK_DEATH(0.2 SECONDS, src)
	if(QDELETED(user))
		attack_sound = initial(attack_sound)
		icon_state = IsContained() ? initial(icon_state) : icon_aggro
		return
...
/mob/living/simple_animal/hostile/abnormality/norinori/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/norinori/proc/Transform()
	...
	SLEEP_CHECK_DEATH(10, src)
	playsound(src, 'sound/effects/blob/blobattack.ogg', 150, FALSE, 4)
	playsound(src, 'sound/items/weapons/chainsawhit.ogg', 250, FALSE, 4)
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/helper/attack.ogg'
	ChangeResistances(list(BURN = 0.1, BRAIN = 1.1, BRUTE = 0.6, TOX = 1.6))
	can_act = TRUE
	rapid_melee = 3
	icon_state = "norinori_transformed"
...
/mob/living/simple_animal/hostile/abnormality/norinori/proc/UnTransform()
	...
	attack_sound = 'sound/items/weapons/slashmiss.ogg'
	SLEEP_CHECK_DEATH(10, src)
	ChangeResistances(list(BURN = 0.5, BRAIN = 1.5, BRUTE = 1, TOX = 2))
	can_act = TRUE
	rapid_melee = 1
