/mob/living/simple_animal/hostile/megafauna/apocalypse_bird
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/dead.ogg'
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/Initialize()
	...
	var/list/potential_locs = shuffle(GLOB.generic_event_spawns)
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Slam()
	...
	addtimer(CALLBACK(src, PROC_REF(Slam_second)), 1.5 SECONDS)

/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Slam_second()
	...
	addtimer(CALLBACK(src, PROC_REF(Slam_third)), 4 SECONDS)

/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Slam_third()
	...
	addtimer(CALLBACK(src, PROC_REF(Slam_end)), 2 SECONDS)

/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Slam_end()
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Teleport()
	...
	if(LAZYLEN(teleport_priority))
		...
		T = get_turf(pick(GLOB.generic_event_spawns))
	if(!istype(T))
		return
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/teleport.ogg', 75, FALSE)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/teleport.ogg', 75, FALSE)
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Bite()
	...
	flick("apocalypse_slam", src)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/beak.ogg', 100, FALSE, 12)
	...
	for(var/turf/TF in area_of_effect)
		...
		var/list/new_hits = HurtInTurf(TF, been_hit, bite_damage, BURN, check_faction = TRUE, hurt_mechs = TRUE) - been_hit
		been_hit += new_hits
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Judge()
	...
	for(var/mob/living/L in GLOB.mob_living_list)
		...
		L.apply_damage(judge_damage, TOX, null, L.run_armor_check(null, TOX), spread_damage = TRUE)
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/Enchant()
	if(attacking || !big_possible)
		return
	attacking = TRUE
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/enchant.ogg'))
	...
	for(var/mob/living/carbon/human/H in enchant_targets)
		...
		H.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "enchanted", -HALO_LAYER))
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/EndEnchant(mob/living/carbon/human/victim)
	if(victim in enchanted_list)
		...
		victim.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "enchanted", -HALO_LAYER))
...
/mob/living/simple_animal/hostile/megafauna/apocalypse_bird/proc/LightFire()
	if(attacking || !big_possible)
		return
	attacking = TRUE
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/prepare.ogg', 80, FALSE, 7)
	...
	for(var/i = 1 to light_amount)
		...
		P.xo = PT.x - T.x
		P.original = PT
		P.set_angle(get_angle(T, PT))
		addtimer(CALLBACK (P, TYPE_PROC_REF(/obj/projectile, fire)), 6.5 SECONDS)

	//SLEEP_CHECK_DEATH(6.5 SECONDS, src)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/fire.ogg', 75, FALSE, 12)
	//SLEEP_CHECK_DEATH(1 SECONDS, src)
	icon_state = icon_living
	//SLEEP_CHECK_DEATH(2 SECONDS, src)
	special_cooldown = world.time + special_cooldown_time
...
/mob/living/simple_animal/apocalypse_egg/beak
	...
	damage_coeff = list(BURN = -2, BRAIN = 0.5, BRUTE = 1, TOX = 0.5)
...
/mob/living/simple_animal/apocalypse_egg/arm
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.5, BRUTE = 0.5, TOX = -2)
...
/mob/living/simple_animal/apocalypse_egg/eyes
	...
	damage_coeff = list(BURN = 0.5, BRAIN = 1, BRUTE = -2, TOX = 0.5)
...
/mob/living/simple_animal/forest_portal
	...
	maxHealth = 30000
	health = 30000
	damage_coeff = list(BURN = 0.3, BRAIN = 0.3, BRUTE = 0.3, TOX = 0.3)
	move_resist = MOVE_FORCE_STRONG
	pull_force = MOVE_FORCE_STRONG
	mob_size = MOB_SIZE_HUGE
...
/mob/living/simple_animal/forest_portal/proc/ConsumeBird(mob/living/simple_animal/hostile/abnormality/bird)
	...
	if(!(bird.type in bird_types))
		return FALSE
	bird.forceMove(src)
	ADD_TRAIT(bird, TRAIT_GODMODE, ADMIN_TRAIT)
...
/mob/living/simple_animal/forest_portal/proc/SpeakBird()
	...
	if(length(stored_birds["spoken"]) >= 3)
		...
		sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/appear.ogg'))
		for(var/mob/M in GLOB.player_list)
			if(M.z == z && M.client)
...
/datum/ai_controller/insane/enchanted/recalculate_idle(datum/exited)
	var/mob/living/living_pawn = pawn
	if(prob(30) && (living_pawn.mobility_flags & MOBILITY_MOVE) && isturf(living_pawn.loc) && !living_pawn.pulledby)
