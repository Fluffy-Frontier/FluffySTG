...
/mob/living/simple_animal/hostile/abnormality/red_hood
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/attack_1.ogg'
	del_on_death = TRUE
	pixel_x = -8
	base_pixel_x = -8
	...
	damage_coeff = list(BURN = 0.6, BRAIN = 1.2, BRUTE = 0.6, TOX = 1.5) // Fuck you, blue shepherd.
	ranged = TRUE
...
/datum/action/innate/abnormality_attack/find_target // AI-controlled Red technically doesn't use this one.
	name = "Locate target"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/teguicons.dmi'
...
/mob/living/simple_animal/hostile/abnormality/red_hood/apply_damage(damage = 0,damagetype = BRUTE, def_zone = null, blocked = FALSE, forced = FALSE, spread_damage = FALSE, wound_bonus = 0, bare_wound_bonus = 0, sharpness = NONE, attack_direction, attacking_item, exposed_wound_bonus)
	if(evading_attack)
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/RageUpdate(rage_change) // Always call this after changing red_rage manually, or call it to change red_rage
	...
	if(red_rage > 0) //Normal rage
		...
		update_light()
		if(red_rage == 3)
			ChangeResistances(list(BURN = 0.3, BRAIN = 0.6, BRUTE = 0.3, TOX = 1.5)) // She takes... very little damage.
		return

	ChangeResistances(list(BURN = 0.6, BRAIN = 1.2, BRUTE = 0.6, TOX = 1.5)) // She takes normal damage now
	speed = initial(speed)
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/HunterBullet(atom/target, shots_remaining = 0)
	...
	P.fire()
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/fire.ogg', 50, FALSE, 4)
	if(shots_remaining)
...
/datum/action/innate/abnormality_attack/strike_without_hesitation
	name = "Blade Throw"
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/projectiles.dmi'
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/GetThrown(atom/target)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/redhood/throw.ogg', 50, FALSE, 4)
	var/turf/startloc = get_turf(src)
	var/angle_to_target = get_angle(src, target)
...
/mob/living/simple_animal/hostile/abnormality/red_hood/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	RequestTarget(user)
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/RequestTarget(mob/living/carbon/human/user) // Code initially jacked from the Possess! verb
	...
	var/mob/living/simple_animal/hunted = input(user, "What's the target?", "Select hunt target", null) as null|anything in sort_names(huntable)
	if(!hunted)
		if(client)
	...
	if(istype(hunted, /mob/living/simple_animal/hostile/abnormality)) //If its an abnormality we add 100 PE for every risk level
		...
		current_cost = tiered_request_costs[abno.fear_level]
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/OnAbnoBreach(datum/source, mob/living/simple_animal/hostile/abnormality/abno)
	...
	qliphoth_change(-1) // This is literally the only way her counter goes down.
...
/mob/living/simple_animal/hostile/abnormality/red_hood/attacked_by(obj/item/I, mob/living/user)
	if(out_on_request ) // Basically, it will retaliate while patrolling without stopping or chasing after them
		say(pick(blue_evade_lines))
...
/mob/living/simple_animal/hostile/abnormality/red_hood/handle_automated_movement()
	if(out_on_request || priority_target)
		if(!priority_target) //For some reason we never got one
			return ..()
		Goto(get_turf(priority_target), 20 DECISECONDS, 1)
		return
	return ..()
...
/mob/living/simple_animal/hostile/abnormality/red_hood/proc/ContractComplete()
	...
	if(target_priority == 4)
		...
		if(!kill_confirmed) //Somehow we completed the contract but the wolf was too far away. Likely died off-screen
			...
			fear_level = WAW_LEVEL
			return
	//______qdel_list_wrapperif(target_priority < 4)
	//______qdel_list_wrapper	say("Work's done. Time to go home.")
	else
