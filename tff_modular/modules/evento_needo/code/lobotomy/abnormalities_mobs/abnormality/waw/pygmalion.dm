...
/mob/living/simple_animal/hostile/abnormality/pygmalion
	...
	maxHealth = 2000
	health = 2000
	damage_coeff = list(BURN = 0.8, BRAIN = 0.8, BRUTE = 0.8, TOX = 1.5)
...
/mob/living/simple_animal/hostile/abnormality/pygmalion/PostWorkEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/pygmalion/proc/GoToSculptor()
	...
	for(var/turf/T in all_turfs)
		if(T == origin)
			continue
		var/available_turf
		var/list/sculptor_line = get_line(T, sculptor)
...
/mob/living/simple_animal/hostile/abnormality/pygmalion/SuccessEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/pygmalion/Life()
	...
	if (IsContained() && sculptor && (sculptor.health/sculptor.maxHealth < 0.5 || sculptor.sanityhealth/sculptor.maxSanity < 0.5) )
		...
		var/user_attribute_level = max(1, sculptor.get_clothing_class_level(CLOTHING_SCIENCE))
		if (user_attribute_level > 3)
			missing_prudence = user_attribute_level - 3
			src.sculptor.physiology.brain_mod += (missing_prudence) / 10
	...
	if (IsContained() && fear_level == TETH_LEVEL)
		fear_level = WAW_LEVEL
...
/mob/living/simple_animal/hostile/abnormality/pygmalion/proc/restorePrudence()
	sculptor.physiology.brain_mod -= (missing_prudence) / 10
