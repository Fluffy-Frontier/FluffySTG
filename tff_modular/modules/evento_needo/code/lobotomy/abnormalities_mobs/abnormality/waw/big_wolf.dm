...
/mob/living/simple_animal/hostile/abnormality/big_wolf
	...
	health = 2500
	del_on_death = FALSE
	damage_coeff = list(BURN = 1, BRAIN = 1, BRUTE = 0.7, TOX = 0.7, BRUTE = 1)
	see_in_dark = 10
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/Initialize(mapload)
	. = ..()
	addtimer(CALLBACK(src, PROC_REF(PreHowl)), howl_cooldown)
...
/datum/action/cooldown/wolf_howl/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/SuccessEffect(mob/living/carbon/human/user)
	if(user.stat != DEAD && locate(/mob/living) in contents)
		flick("wolf_sad", src)
		SpewStomach()

/mob/living/simple_animal/hostile/abnormality/big_wolf/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	EatWorker(user)
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/FleeNow()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/big_wolf/Wolf_FogChange.ogg', 75, 1)
	ADD_TRAIT(src, TRAIT_MOVE_PHASING, "fleeing")
	AIStatus = AI_OFF
	target = null
	walk_to(src, 0)
	TemporarySpeedChange()
	fleeing_now = TRUE
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/TemporarySpeedChange()
	add_movespeed_modifier(/datum/movespeed_modifier/big_wolf_flee)
	addtimer(CALLBACK(src, PROC_REF(remove_movespeed_modifier), /datum/movespeed_modifier/big_wolf_flee), 3 SECONDS)

/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/StopFleeing()
	playsound(get_turf(src), 'sound/effects/magic/ethereal_exit.ogg', 75, 1)
	fleeing_now = FALSE
	color = initial(color)
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/EatWorker(mob/living/L)
	if(!L)
		return FALSE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/crimson/noon_bite.ogg', 75, 1)
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/SpewStomach()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/big_wolf/Wolf_EatOut.ogg', 75, 1)
	...
	if(do_after(src, 1 SECONDS, target = src))
		...
		for(var/i = 0 to 7)
			...
			for(var/turf/T in orange(get_turf(src), 1))
				...
				hit_mob = HurtInTurf(T, hit_mob, 50, BRUTE, null, TRUE, FALSE, TRUE, hurt_structure = TRUE)
				//for(var/mob/living/simple_animal/hostile/abnormality/red_hood/mercenary in hit_mob)
				//	mercenary.apply_damage(100, BRUTE) //triple damge to red
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/PreHowl()
	if(!client && can_act && fleeing_now != TRUE)
		Howl()
	addtimer(CALLBACK(src, PROC_REF(PreHowl)), howl_cooldown)
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/proc/Howl()
	...
	var/mutable_appearance/visual_overlay = mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/effects.dmi', "blip")
	...
	if(do_after(src, 2 SECONDS, target = src))
		...
		for(var/mob/living/L in turfs_to_check)
			...
			//	mercenary.apply_damage(150, BRUTE) //She takes triple damage from the wolf, becauser her resistances are high
		...
		for(var/obj/vehicle/V in turfs_to_check)
			V.take_damage(50, BRUTE)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/big_wolf/Wolf_Howl.ogg', 30, 0, 4)
...
/mob/living/simple_animal/hostile/abnormality/big_wolf/AttackingTarget(atom/attacked_target)
	...
	//	mercenary.apply_damage(bonus_damage_dealt, BRUTE)
	return ..()

/datum/movespeed_modifier/big_wolf_flee
