...
/mob/living/simple_animal/hostile/abnormality/hatred_queen
	...
	maxHealth = 2000
	health = 2000
	damage_coeff = list(BURN = 0.7, BRAIN = 1.2, BRUTE = 0.3, TOX = 1.5)
	stat_attack = HARD_CRIT
	ranged_cooldown_time = 12
	projectiletype = /obj/projectile/hatred
	del_on_death = FALSE
	projectilesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/attack.ogg'
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/dead.ogg'
...
/datum/action/cooldown/toggle_hysteria/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/Initialize()
	...
	var/icon/I = icon('tff_modular/modules/evento_needo/icons/Teguicons/96x64.dmi',icon_living) //create inverted colors icon
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/proc/BeatsTurf(turf/T)
	...
	for(var/turf/TT in range(1, T))
		...
		beats_hit = HurtInTurf(TT, beats_hit, beats_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/proc/BeamAttack(target)
	...
	for(var/i = 1 to 3)
		...
		spawned_effects += S
		playsound(src, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/beam[clamp(i, 1, 2)].ogg", 50, FALSE, 4*i)
	...
	for(var/turf/TF in hit_line) //checks if that line has anything in the way, resets TT as the new beam end location
		if(TF.density)
			TT = TF
			break
	hit_line = get_line(my_turf, TT) //old hit_line is discarded with hit_line which respects walls
	if(friendly)
		...
		for(var/turf/TF in hit_line)
			...
			for(var/mob/living/L in view(beam_stage-1, TF))
				...
				if(faction_check_atom(L))
					...
					continue
				var/damage_before = L.getBruteLoss(BRUTE)
				...
				var/damage_dealt = abs(L.getBruteLoss(BRUTE)-damage_before)
				if(!friendly)
					if(ishuman(L))
	...
	spawned_effects.Cut()
	beamloop.stop()
	SLEEP_CHECK_DEATH(4 SECONDS, src) //Rest after laser beam
	can_act = TRUE
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/OnQliphothChange()
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/SuccessEffect(mob/living/carbon/human/user)
	qliphoth_change(1)
	return
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/hatred_queen/proc/EventStart()
	...
	ChangeResistances(list(BURN = 0.7, BRAIN = 1.2, BRUTE = 0.3, TOX = 1.5))
