/mob/living/simple_animal/hostile/abnormality/little_prince
	...
	damage_coeff = list(BURN = 1.2, BRAIN = 1.3, BRUTE = 0.8, TOX = 2)
...
/mob/living/simple_animal/hostile/abnormality/little_prince/Life()
	...
	for(var/mob/living/carbon/human/user in hypnotized)
		if ((user.stat == DEAD) || !(user.sanity_lost))
			...
			user.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects32x48.dmi', "spore_hypno", -HALO_LAYER))
			hypnotized -= user
...
/mob/living/simple_animal/hostile/abnormality/little_prince/proc/Hypno(mob/living/carbon/human/user)
	if (!(user.sanity_lost))
		playsound(get_turf(user), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/littleprince/Prince_Active.ogg', 50, 0, 2)
		user.apply_damage(user.maxSanity, BRUTE)
		if (!(user.sanity_lost))
	...
	user.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects32x48.dmi', "spore_hypno", -HALO_LAYER))
	QDEL_NULL(user.ai_controller)
...
/mob/living/simple_animal/hostile/abnormality/little_prince/proc/Infect(mob/living/carbon/human/user)
	for (var/i=0, i<5, i++)
		user.apply_damage(rand(10, 20), BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/little_prince/proc/PrinceDeath(datum/source, gibbed)
	SIGNAL_HANDLER
	playsound(get_turf(source), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bee/spores.ogg', 50, 1, 5)
...
/mob/living/simple_animal/hostile/abnormality/little_prince/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
...
/mob/living/simple_animal/hostile/abnormality/little_prince/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/little_prince_1
	...
	death_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/littleprince/Prince_Death.ogg'
	attack_verb_continuous = "smashes"
	attack_verb_simple = "smash"
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/littleprince/Prince_Attack.ogg'
...
///datum/ai_controller/insane/hypno/handle_automated_movement()
