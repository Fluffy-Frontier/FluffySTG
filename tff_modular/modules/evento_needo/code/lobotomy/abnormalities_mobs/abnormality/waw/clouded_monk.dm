...
/mob/living/simple_animal/hostile/abnormality/clouded_monk
	...
	rapid_melee = 2
	ranged = TRUE
	damage_coeff = list(BURN = 1.0, BRAIN = 1.2, BRUTE = 0.8, TOX = 0.8, BRUTE = 1.5)
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_attack.ogg'
...
/mob/living/simple_animal/hostile/abnormality/clouded_monk/proc/OnMobDeath(datum/source, mob/living/died, gibbed) //stolen from big bird
	...
	deathcount++
	if(deathcount >= 2)
		qliphoth_change(-1) // Two deaths reduces it compared to base 10
		deathcount = 0
	return TRUE
...
//work code
/mob/living/simple_animal/hostile/abnormality/clouded_monk/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
...
/mob/living/simple_animal/hostile/abnormality/clouded_monk/BreachEffect(mob/living/carbon/human/user)
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/transform.ogg', 50, 1)
...
/mob/living/simple_animal/hostile/abnormality/clouded_monk/proc/Charge(atom/chargeat = target, delay = 1 SECONDS, chargepast = 3)
	...
	if(!eaten) //different sfx before and after eating someone
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_cast.ogg', 100, 1)
	else
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_groggy.ogg', 150, 1)
...
/mob/living/simple_animal/hostile/abnormality/clouded_monk/proc/EndCharge(bump = FALSE)
	...
	for(var/mob/living/L in range(2, T))//damage applied to targets in range
		...
		playsound(L, 'tff_modular/modules/evento_needo/sounds/Tegusounds/lc13_creatures/lovetown/slam.ogg', 75, 1)
	...
	if(charging)
		if(isliving(A))
			...
			if(!faction_check_atom(L))
				...
				playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/monk_bite.ogg', 75, 1)
				shake_camera(L, 4, 3)
				...
				if(ishuman(L))
					...
					if(H.health < 0)
						...
						if(!eaten)
							...
							playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/clouded_monk/eat_groggy.ogg', 75, 1)
				else
					L.adjustBruteLoss(charge_damage/10)
				EndCharge(TRUE)
				ResetCharge()
		else if(isvehicle(A))
			var/obj/vehicle/V = A
