...
/mob/living/simple_animal/hostile/abnormality/my_form_empties
	...
	maxHealth = 2000
	health = 2000
	damage_coeff = list(BURN = 1, BRAIN = 0.8, BRUTE = 0.8, TOX = 1.4, BRUTE = 1)
...
/mob/living/simple_animal/hostile/abnormality/my_form_empties/proc/StaffAttack(target)
	...
	playsound(get_turf(staff), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/myformempties/MFEattack.ogg', 30, 0, 4)
	SLEEP_CHECK_DEATH(10, src)
	for(var/turf/T in view(staff_range, get_turf(staff)))
		new /obj/effect/temp_visual/smash_effect(T)
		been_hit = HurtInTurf(T, been_hit, staff_damage, BRUTE, check_faction = FALSE, hurt_mechs = TRUE, mech_damage = staff_damage)
...
/mob/living/simple_animal/hostile/abnormality/my_form_empties/proc/KarmaDing()
	...
	for(var/mob/living/L in view(48, src))
		...
		if(faction_check_atom(L))
			continue
		L.apply_damage(pulse_damage, BRUTE)//apply karma here
		ApplyKarma(L, 5)
		if(!ishuman(L))
			continue
...
/mob/living/simple_animal/hostile/abnormality/my_form_empties/SuccessEffect(mob/living/carbon/human/user)
	if(anatman_state)
		qliphoth_change(-1)
	else
		Pray(user)
...
/mob/living/simple_animal/hostile/abnormality/my_form_empties/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/my_form_empties/OnQliphothChange()
	. = ..()
...
/datum/status_effect/stacking/karma/on_apply()
	...
	if(ishuman(H))
		H.physiology.burn_mod += physiology_mod
		H.physiology.brain_mod += physiology_mod
		H.physiology.brute_mod += physiology_mod
		H.physiology.tox_mod += physiology_mod
...
/datum/status_effect/stacking/karma/on_remove()
	...
	var/mob/living/carbon/human/H = owner
	if(ishuman(H))
		H.physiology.burn_mod -= physiology_mod//a runtime occurs here if you were gibbed by karma's effect
		H.physiology.brain_mod -= physiology_mod
		H.physiology.brute_mod -= physiology_mod
		H.physiology.tox_mod -= physiology_mod
		return
...
/obj/effect/myform_staff//the staff
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/32x96.dmi'
