...
#define NAKED_NESTED get_organ_by_type(/obj/item/organ/naked_nest)
...
/mob/living/simple_animal/hostile/abnormality/naked_nest
	...
	wander = FALSE
	damage_coeff = list(BURN = 0.6, BRAIN = 0.8, BRUTE = 1.2, TOX = 1.5) //same stats as original armor
...
/mob/living/simple_animal/hostile/abnormality/naked_nest/FailureEffect(mob/living/carbon/human/user)
...
/mob/living/simple_animal/hostile/abnormality/naked_nest/OpenFire()
	if(bodytemperature <= INHOSPITABLE_FOR_NESTING || ranged_cooldown > world.time || serpentsnested <= 0 || HAS_TRAIT(src, TRAIT_GODMODE)) //Do we have serpents? Is it too cold to leave?
		return FALSE
...
/mob/living/simple_animal/hostile/naked_nest_serpent
	...
	move_to_delay = 3
	damage_coeff = list(BURN = 0.6, BRAIN = 0.8, BRUTE = 1.2, TOX = 1.5)
	stat_attack = HARD_CRIT
...
/obj/item/organ/naked_nest/Insert(mob/living/carbon/M, special = FALSE, movement_flags)
	..()
...
/obj/item/organ/naked_nest/proc/growProcess()
	...
	if((H.bodytemperature <= INHOSPITABLE_FOR_NESTING) && H.stat != DEAD) //increases duration of infection.
...
/obj/item/organ/naked_nest/proc/HatchNest(mob/living/carbon/human/host)
	...
	if(host.get_item_by_slot(ITEM_SLOT_OCLOTHING))
		...
		N.UpdateArmor(list(BURN = 0.6, BRAIN = 0.8, BRUTE = 1.2, TOX = 1.5))
