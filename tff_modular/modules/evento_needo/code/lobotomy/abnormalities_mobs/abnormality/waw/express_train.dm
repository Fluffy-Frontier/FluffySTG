...
/mob/living/simple_animal/hostile/abnormality/express_train/Initialize()
	...
	addtimer(CALLBACK(src, PROC_REF(check_meltdown)), meltdown_tick)
	return ..()

/mob/living/simple_animal/hostile/abnormality/express_train/proc/check_meltdown()
...
/mob/living/simple_animal/hostile/abnormality/express_train/try_working(mob/living/carbon/human/user)
	...
	switch(datum_reference.qliphoth_meter)
		if(0)
			...
			tickets |= user
		if(1)
			for(var/mob/living/carbon/human/H in view(30))
				H.adjustSanityLoss(-50)
				H.adjustBruteLoss(-50)
			tickets |= user
		if(2)
...
/mob/living/simple_animal/hostile/abnormality/express_train/PostWorkEffect(mob/living/carbon/human/user)
	qliphoth_change(4)
	meltdown_timer = world.time + meltdown_tick
	lightscount = 0
	update_icon_state()
	...
	for(var/i = 0, i < maxSegments * 2, i++)
		...
		if(i < 2)
			...
			if(i % 2)
				persX -= xIncrement/4
			if(i == 0)
				notify_ghosts("[seg] is preparing to depart!", source = seg, notify_flags = NOTIFY_CATEGORY_NOFLASH, header="Something Interesting!") // bless this mess
		else
			...
			if(round(i / 2) % 2) // True when the current car is odd-numbered
				...
				persX -= xIncrement/4
		segments += seg
	addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(sound_to_playing_players), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/expresstrain/express_summoned.ogg', 50), 1)
		...
		for(var/turf/T in coveredTurfs)
			for(var/mob/living/M in T.contents)
				...
				if(!seg.noise && seg.icon_state == "expressengine_1") // choo choo
					if(rand())
						playsound(get_turf(seg), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/expresstrain/express_horn.ogg', 100, 0, 40)
					else
						playsound(get_turf(seg), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/expresstrain/express_whistle.ogg', 100, 0, 40)
					seg.noise = 1
				M.apply_damage(400, BRUTE)
				...
				if(iscarbon(M))
					...
					for(var/obj/item/bodypart/part in C.bodyparts)
						if(prob(20) && part.body_part != HEAD && part.body_part != CHEST && C.stat == DEAD)
							part.dismember()
