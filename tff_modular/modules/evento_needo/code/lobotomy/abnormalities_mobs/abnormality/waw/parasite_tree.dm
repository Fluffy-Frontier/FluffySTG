...
/mob/living/simple_animal/hostile/abnormality/parasite_tree/PostWorkEffect(mob/living/carbon/human/user, work_type, pe, work_time, canceled)
	if(user.get_major_clothing_class() != CLOTHING_ARMORED && user.stat != DEAD)
		if(user.has_status_effect(THE_LIARS_BLESSING))
			var/mob/living/carbon/human/witness = locate(/mob/living/carbon/human) in view(1, src) //included mostly for lore since you encourage people to use it.
...
/mob/living/simple_animal/hostile/abnormality/parasite_tree/OnQliphothChange()
	cut_overlays()
...
/mob/living/simple_animal/hostile/abnormality/parasite_tree/proc/dropLeaf()
	...
	if(origin_cooldown <= world.time && datum_reference.qliphoth_meter > 0) //cool it on the leaf dropping if your already breaching
		...
		for(var/mob/living/carbon/human/L in GLOB.player_list)
			if(!faction_check_atom(L) && L.stat != DEAD && L.z == z)
		...
		if(potentialFollowers.len)
			var/mob/living/carbon/human/chosen_agent = pick_weight(potentialFollowers)
...
/mob/living/simple_animal/hostile/parasite_tree_sapling
	...
	wander = FALSE
	wander = 0
	damage_coeff = list(BURN = 1.2, BRAIN = 0.6, BRUTE = 0.8, TOX = 0.8)
	ranged = TRUE
...
/datum/status_effect/display/parasite_tree_blessing/on_apply()
	...
	connected_abno.qliphoth_change(-1)
...
/datum/status_effect/display/parasite_tree_curse/tick()
	...
	status_holder.apply_damage(tree_toxin, BRUTE)
