...
/mob/living/simple_animal/hostile/abnormality/wrath_servant
	...
	maxHealth = 1800
	health = 1800
	damage_coeff = list(BURN = 0.3, BRAIN = 1.2, BRUTE = 0.7, TOX = 1.5)
	...
	rapid_melee = 2
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/small_smash1.ogg'
...
/datum/action/cooldown/wrath_smash/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/datum/action/cooldown/wrath_dash/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
	if(!istype(owner, /mob/living/simple_animal/hostile/abnormality/wrath_servant))
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/IsContained()
	if((HAS_TRAIT(src, TRAIT_GODMODE)) && !stunned)
		return TRUE
	return FALSE
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/AttackingTarget(atom/attacked_target)
	...
	. = ..()
	attack_sound = pick('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/small_smash1.ogg','tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/small_smash2.ogg')
	if(!isliving(attacked_target) || (get_dist(attacked_target, src) > 1))
		return
	var/mob/living/L = attacked_target
	L.apply_damage(rand(10, 15), BRUTE)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/PostWorkEffect(mob/living/carbon/human/user)
	. = ..()
	if(prob(instability))
		qliphoth_change(-1)
		if(LAZYLEN(friend_ship))
			var/mob/living/L = pick(friend_ship)
			say("I... I shouldn't be treating them this way... [L.get_examine_name()]...")
		else
			say("No... I must keep my mind in check. I'm sorry, [user.get_examine_name()].")
	switch(user.get_major_clothing_class())
		...
		if(CLOTHING_ARMORED)
			...
			return
	if(user in friend_ship)
		say("It was good to see you again, [user.get_examine_name()].")
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/try_working(mob/living/carbon/human/user)
	. = ..()
	if(!.)
		return
	if(!(user in friend_ship))
		say("All are to be treated equally, even those you keep in cages. You are no exception, [user.get_examine_name()].")
		return FALSE
	if(!IsContained())
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/BreachEffect(mob/living/carbon/human/user, breach_type)
	...
	src.icon = 'tff_modular/modules/evento_needo/icons/Teguicons/96x64.dmi'
	icon_state = "wrath"
	pixel_x = -32
	base_pixel_x = -32
	fear_level = WAW_LEVEL
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/enrage.ogg', 100, FALSE, 40, falloff_distance = 20)
	toggle_ai(AI_ON)
	REMOVE_TRAIT(src, TRAIT_GODMODE, ADMIN_TRAIT)
	...
	for(var/turf/dep in GLOB.generic_event_spawns)
		if(!target_turf)
			target_turf = dep
			continue
	...
	playsound(target_turf, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_magic.ogg', 60, FALSE, 10)
	Goto(get_closest_atom(/turf, GLOB.generic_event_spawns, src))
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/Dash()
	...
	add_movespeed_modifier(/datum/movespeed_modifier/wrath_servant)
	addtimer(CALLBACK(src, PROC_REF(remove_movespeed_modifier), /datum/movespeed_modifier/wrath_servant), duration)
	COOLDOWN_START(src, dash, dash_cooldown)

/datum/movespeed_modifier/wrath_servant
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/Smash()
	...
	if(!friendly)
		...
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/enrage.ogg', 75, FALSE, 20, falloff_distance = 10)
	...
	for(var/x = 1 to 3)
		...
		playsound(src, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash[x].ogg", 75, FALSE, 20, falloff_distance = 10) // heard from a distance
		for(var/i = 1 to 5)
			if(i < 4)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/ReturnHome()
	...
	//SLEEP_CHECK_DEATH(1 SECONDS, src)
	Teleport(get_turf(pick(GLOB.generic_event_spawns)))
	can_act = FALSE
	//SLEEP_CHECK_DEATH(1 SECONDS, src)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/PerformEnding(mob/living/simple_animal/hostile/azure_hermit/target = null)
	...
	//SLEEP_CHECK_DEATH(4.5 SECONDS, src)
	Teleport(get_turf(pick(GLOB.generic_event_spawns)))
	can_act = FALSE
	animate(src, alpha = 0, time = 5)
	new /obj/effect/temp_visual/guardian/phase(get_turf(src))
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/Downed()
	...
	icon_state = "wrath_stun"
	//SLEEP_CHECK_DEATH(15 SECONDS, src)
	REMOVE_TRAIT(src, TRAIT_GODMODE, ADMIN_TRAIT)
	icon_state = icon_living
	adjustBruteLoss(-maxHealth, forced = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/wrath_servant/proc/EventStart()
	...
	ChangeResistances(list(BURN = 0.3, BRAIN = 1.5, BRUTE = 0.7, TOX = 1.5))
...
/mob/living/simple_animal/hostile/azure_hermit
	...
	maxHealth = 1500
	health = 1500
	damage_coeff = list(BURN = 0.5, BRAIN = 1.5, BRUTE = 0.8, TOX = 1.2)
	...
	rapid_melee = 2
	melee_damage_type = BRUTE
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_attack.ogg'
...
/mob/living/simple_animal/hostile/azure_hermit/Life()
	. = ..()
	if(!can_act || (HAS_TRAIT(src, TRAIT_GODMODE)))
		return
...
/mob/living/simple_animal/hostile/azure_hermit/AttackingTarget(atom/attacked_target)
	...
	if(istype(attacked_target, /mob/living/simple_animal/hostile/abnormality/wrath_servant))
		...
		if(SW.stunned)
			return
		if(SW.health > 400)
			playsound(SW, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_attack_hard.ogg', 75, FALSE, 15, falloff_distance = 5)
			SW.apply_damage(100, BRUTE) // We win these
			var/list/show_area = list()
			...
			for(var/i = 1 to 3)
				for(var/turf/T in (view(i, SW)-view(i-1,SW)))
					been_hit = HurtInTurf(T, been_hit, 10, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
					new /obj/effect/temp_visual/small_smoke/halfsecond(T)
				//SLEEP_CHECK_DEATH(3, src)
		else
			playsound(SW, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/enrage.ogg', 100, FALSE, 40, falloff_distance = 20)
			...
			ADD_TRAIT(SW, TRAIT_GODMODE, ADMIN_TRAIT)
	...
	if(ishuman(attacked_target))
		...
		if(H.get_clothing_class_level(H.get_major_clothing_class()) < 3)
			say("Pardon me.")
...
/mob/living/simple_animal/hostile/azure_hermit/proc/Befuddle()
	...
	for(var/turf/open/sT in view(4, src))
		new /obj/effect/temp_visual/cult/sparks(sT)
	//SLEEP_CHECK_DEATH(1.25 SECONDS, src)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_magic.ogg', 75, FALSE, 10)
	for(var/turf/open/sT in view(4, src))
...
/mob/living/simple_animal/hostile/azure_hermit/proc/Downed()
	...
	density = FALSE
	ADD_TRAIT(src, TRAIT_GODMODE, ADMIN_TRAIT)
	//SLEEP_CHECK_DEATH(20 SECONDS, src)
	REMOVE_TRAIT(src, TRAIT_GODMODE, ADMIN_TRAIT)
	icon_state = icon_living
	adjustBruteLoss(-maxHealth, forced = TRUE)
	density = TRUE
...
/mob/living/simple_animal/hostile/azure_stave
	...
	move_resist = MOVE_FORCE_STRONG
	wander = TRUE // The dudes roam! That sucks!

	faction = list("hostile", "azure")
	damage_coeff = list(BURN = 0.5, BRAIN = 1.5, BRUTE = 1, TOX = 2)
...
/datum/status_effect/wrath_burning/tick()
	...
	status_holder.apply_damage(5, BRUTE)
