...
/mob/living/simple_animal/hostile/abnormality/greed_king
	...
	damage_coeff = list(BURN = 0, BRAIN = 0.5, BRUTE = 1.2, TOX = 1.5)
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kog/GreedHit1.ogg'
	can_breach = TRUE
...
/mob/living/simple_animal/hostile/abnormality/greed_king/proc/startTeleport()
	if(nihil_present)
		return
	if(!can_act || teleport_cooldown > world.time || (HAS_TRAIT(src, TRAIT_GODMODE)))
		return
		...
		for(var/mob/living/L in new_hits)
			if(!nihil_present)
				...
				for(var/obj/vehicle/V in new_hits)
					V.take_damage(80, BRUTE, attack_sound)
					V.visible_message(span_boldwarning("[src] crunches [V]!"))
					playsound(V, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kog/GreedHit1.ogg', 40, 1)
					playsound(V, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kog/GreedHit2.ogg', 30, 1)
				continue
...
/mob/living/simple_animal/hostile/abnormality/greed_king/FailureEffect(mob/living/carbon/human/user)
	if(prob(80))
		qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/greed_king/proc/EventStart()
	...
	SLEEP_CHECK_DEATH(6 SECONDS, src)
	say("For happiness!")
	ChangeResistances(list(BURN = 0, BRAIN = 0.5, BRUTE = 1.2, TOX = 1.5))
