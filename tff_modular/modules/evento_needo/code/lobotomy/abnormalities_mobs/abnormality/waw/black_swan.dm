...
/mob/living/simple_animal/hostile/abnormality/black_swan
	...
	move_to_delay = 4
	damage_coeff = list(BURN = 1, BRAIN = 1, BRUTE = 0.8, TOX = 1.5, BRUTE = 1)
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/blackswan/sis_bash.ogg'
...
/datum/action/cooldown/blackswan_umbrella/Trigger(trigger_flags, atom/target)
	if(!..())
		return FALSE
...
/mob/living/simple_animal/hostile/abnormality/black_swan/Destroy()
	...
	return ..()
/mob/living/simple_animal/hostile/abnormality/black_swan/PostWorkEffect(mob/living/carbon/human/user)
	if(prob(50))
...
/mob/living/simple_animal/hostile/abnormality/black_swan/OnQliphothChange()
...
/mob/living/simple_animal/hostile/abnormality/black_swan/proc/OpenUmbrella()
	if(umbrella_open)
		return
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/blackswan/sis_swoop.ogg', 10, 0, 4)
	umbrella_open = TRUE
		...
		for(var/mob/living/L in turfs_to_check)
			if(isabnormalitymob(L))
				...
				if(maybe_brothers.IsContained())
					maybe_brothers.qliphoth_change(-1)
					continue
			if(faction_check_atom(L, FALSE))
				continue
			if(L.stat == DEAD)
				continue
			L.apply_damage(70, BRUTE)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/blackswan/sis_roar.ogg', 30, 0, 4)
...
/mob/living/simple_animal/hostile/abnormality/black_swan/proc/ReflectProjectile(obj/projectile/P) //reflection code from human_defense.dm
	if(P.starting)
		...
		P.firer = src
		P.set_angle(get_angle(src, locate(clamp(new_x, 1, world.maxx), clamp(new_y, 1, world.maxy), z)))
