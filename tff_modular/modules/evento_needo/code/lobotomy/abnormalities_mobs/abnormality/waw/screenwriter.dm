...
/mob/living/simple_animal/hostile/abnormality/screenwriter
	...
	maxHealth = 4000
	health = 4000
	damage_coeff = list(BURN = 1.3, BRAIN = 0.5, BRUTE = 1.3, TOX = 1.5)
	faction = list("hostile")
	fear_level = WAW_LEVEL
	blood_volume = 0

	var/work_chances = list(
		CLOTHING_ARMORED,
...
//Work stuff
/mob/living/simple_animal/hostile/abnormality/screenwriter/try_working(mob/living/carbon/human/user)
	if(A)
		to_chat(user, span_warning("The abnormality ignores you!"))
		return FALSE
	if(user.get_major_clothing_class() == preferred_work_type || !preferred_work_type)
		happy = TRUE
	else
		happy = FALSE
	return ..()

/mob/living/simple_animal/hostile/abnormality/screenwriter/PostWorkEffect(mob/living/carbon/human/user)
	. = ..()
	preferred_work_type = pick(work_chances)
	SpawnIcon()
...
/mob/living/simple_animal/hostile/abnormality/screenwriter/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/screenwriter/proc/MeltdownEffect()
	...
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/screenwriter/start.ogg', volume = 50))
...
/mob/living/simple_animal/hostile/abnormality/screenwriter/proc/EndScenario()
	sound_to_playing_players(sound('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/screenwriter/finish.ogg', volume = 25))
...
/datum/status_effect/actor/on_remove()
	...
	owner.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x64.dmi', role, -ABOVE_MOB_LAYER))
	if(role == "victim")
		H.physiology.burn_mod += 1
		H.physiology.brute_mod += 1
		H.physiology.brain_mod += 1
		H.physiology.tox_mod +=1
	return..()
...
/datum/status_effect/actor/proc/ChangeToVictim() // So you have chosen death
	...
	status_holder.physiology.burn_mod += 1
	status_holder.physiology.brute_mod += 1
	status_holder.physiology.brain_mod += 1
	status_holder.physiology.tox_mod +=1
	owner.cut_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x64.dmi', role, -ABOVE_MOB_LAYER))
	role = "victim"
	owner.add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x64.dmi', role, -ABOVE_MOB_LAYER))
...
/datum/status_effect/actor/proc/AssignRole()
	...
	switch(role)
		if("coward")
			status_holder.physiology.tox_mod += 0.75
		if("broken")
			status_holder.physiology.burn_mod += 1
		if("failed")
			status_holder.physiology.brain_mod += 1
...
/mob/living/simple_animal/hostile/actor/Initialize()
	. = ..()
	add_overlay(mutable_appearance('tff_modular/modules/evento_needo/icons/Teguicons/32x64.dmi', "abandoned", -ABOVE_MOB_LAYER))

// Patrol Code
/mob/living/simple_animal/hostile/actor/handle_automated_movement()//Hunt down the "victim"s
	...
	if(potential_target)
		...
		S.ChangeToVictim()
		Goto(get_turf(potential_target))
		return
	return ..()
...
/mob/living/simple_animal/hostile/actor/death(gibbed)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/ordeals/white/pale_pistol.ogg', 100, FALSE, 4)
...
//		playsound(get_turf(human_pawn), 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/nail1.ogg', 100, FALSE, 4)
//		human_pawn.adjustBruteLoss(400)
//		var/sanity_damage = human_pawn.get_clothing_class_level(human_pawn.get_major_clothing_class()) * 5
//		for(var/mob/living/carbon/human/H in view(7, human_pawn))
//			if(HAS_TRAIT(H, TRAIT_COMBATFEAR_IMMUNE))
//				continue
//			H.apply_damage(sanity_damage, BRUTE)
