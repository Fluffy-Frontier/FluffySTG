/mob/living/simple_animal/hostile/abnormality/dreaming_current
	...
	maxHealth = 2000
	health = 2000
	damage_coeff = list(BURN = 1.5, BRAIN = 0.5, BRUTE = 1, TOX = 2)
...
/mob/living/simple_animal/hostile/abnormality/dreaming_current/Life()
	. = ..()
	if((HAS_TRAIT(src, TRAIT_GODMODE)) && prob(2)) // Contained
		if(!secret_abnormality)
			icon_state = "current_bubble"
		playsound(src, "sound/effects/bubbles.ogg", 30, TRUE)
		addtimer(CALLBACK(src, PROC_REF(PrepareCharge)), 12)

/mob/living/simple_animal/hostile/abnormality/dreaming_current/proc/PrepareCharge()
...
/mob/living/simple_animal/hostile/abnormality/dreaming_current/proc/ChargeStart(target)
	...
	charging = TRUE
	movement_path = list()
	var/list/initial_turfs = GLOB.generic_event_spawns.Copy() + GLOB.generic_event_spawns.Copy()
	var/list/potential_turfs = list()
	for(var/turf/open/T in initial_turfs)
		if(get_dist(src, T) > 3)
			potential_turfs += T
	for(var/mob/living/L in view(32, src))
		if(prob(50))
			continue
		if(HAS_TRAIT(L, TRAIT_GODMODE) || faction_check_atom(L))
			continue
		if(L.stat == DEAD)
			continue
	...
	for(var/turf/T in movement_path)
		...
		if(!Adjacent(T))
			break
		ChargeAt(T)
		SLEEP_CHECK_DEATH(dash_speed, src)
	charging = FALSE
	icon_state = icon_living
...
/mob/living/simple_animal/hostile/abnormality/dreaming_current/proc/ChargeAt(turf/T)
	...
	forceMove(T)
	if(prob(33))
		playsound(T, "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/dreamingcurrent/move.ogg", 10, TRUE, 3)
	for(var/turf/TF in view(1, T))
		...
		var/list/new_hits = HurtInTurf(TF, been_hit, dash_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE) - been_hit
		been_hit += new_hits
