/mob/living/simple_animal/hostile/abnormality/alriune
	...
	maxHealth = 2000
	health = 2000
	damage_coeff = list(BURN = 1, BRAIN = 1.2, TOX = 0.7, BRUTE = 1.5)
...
/mob/living/simple_animal/hostile/abnormality/alriune/proc/CheckAndPulse()
	if(world.time >= petals_next)
		...
		if(petals_current >= 3) // Attack
			petals_current = 0
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alriune/damage.ogg', 75, TRUE, 12)
			// Attack visual effect, so to speak
			for(var/turf/T in view(7, get_turf(src)))
				animate(T, color = COLOR_PINK, time = 2)
				addtimer(CALLBACK(src, PROC_REF(SetColorOverTime), T, initial(T.color), (2 SECONDS)), 4)
				...
				if(ishuman(L))
					...
					if(H.sanity_lost)
						...
						addtimer(CALLBACK(H, TYPE_PROC_REF(/atom, add_overlay), \
							icon('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "alriune_kill")), 5)
						playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alriune/kill.ogg', 75, TRUE)
						H.death()
			petals_next = world.time + (petals_next_time * 2)
			addtimer(CALLBACK(src, PROC_REF(TeleportAway)), 2 SECONDS)
		else
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alriune/timer.ogg', 50, FALSE, 12)
		update_icon()

/mob/living/simple_animal/hostile/abnormality/alriune/proc/SetColorOverTime(turf/T, new_color, timer)
	animate(T, color = new_color, time = timer)
...
/mob/living/simple_animal/hostile/abnormality/alriune/proc/TeleportAway()
	...
	if(!istype(T))
		return FALSE
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/alriune/curtain_out.ogg', 50, TRUE, 12)
...
/mob/living/simple_animal/hostile/abnormality/alriune/update_overlays()
	. = ..()
	if(petals_current <= 0 || stat == DEAD || HAS_TRAIT(src, TRAIT_GODMODE))
		cut_overlays()
		return
