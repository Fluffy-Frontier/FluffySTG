...
/mob/living/simple_animal/hostile/abnormality/snow_whites_apple
	...
	blood_volume = 0
	obj_damage = 0
	damage_coeff = list(BURN = 0.5, BRAIN = 1.0, BRUTE = 0, TOX = 1.5)
	ranged = TRUE
...
/mob/living/simple_animal/hostile/abnormality/snow_whites_apple/Life()
	...
	for(var/obj/structure/spreading/apple_vine/W in area_of_influence)
		if(W.last_expand <= world.time)
			...
			var/list/did_we_hit = HurtInTurf(get_turf(W), list(), 30, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
			if(did_we_hit.len)
...
/mob/living/simple_animal/hostile/abnormality/snow_whites_apple/proc/TeleportList()
	. = list()
	for(var/T in GLOB.generic_event_spawns)
		. += get_turf(T)
...
/obj/structure/apple_barrier
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/teguobjects.dmi'
...
/obj/structure/apple_barrier/Initialize()
	...
	for(var/obj/structure/apple_barrier/V in get_turf(src))
		if(V != src)
			qdel(V)
			return
	playsound(get_turf(src), 'sound/mobs/non-humanoids/venus_trap/venus_trap_death.ogg', 15)
...
/obj/structure/apple_barrier/play_attack_sound(damage_amount, damage_type = BRUTE, damage_flag = 0)
	if(damage_amount > 10)
		playsound(loc, 'sound/mobs/non-humanoids/venus_trap/venus_trap_hurt.ogg', 60, TRUE)
...
/obj/structure/spreading/apple_vine/proc/VineAttack(hit_thing)
	if(isliving(hit_thing))
		...
		if(ishuman(L))
			...
			if(H.stat == DEAD)
				var/obj/item/organ/eyes/B = H.get_organ_slot(ORGAN_SLOT_BRAIN)
				if(B)
					new /obj/effect/temp_visual/vinespike(get_turf(H))
					H.add_overlay(icon('tff_modular/modules/evento_needo/icons/Teguicons/tegu_effects.dmi', "f0442_victem"))
					B.Remove(H)
	else
...
/obj/effect/proc_holder/spell/pointed/apple_barrier
	...
	button_icon_state = "apple_barrier"
