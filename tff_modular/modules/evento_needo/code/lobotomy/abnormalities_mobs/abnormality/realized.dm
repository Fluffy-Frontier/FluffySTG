/* E.G.O assimilation */
/datum/action/cooldown/ego_assimilation
	name = "E.G.O assimilation"
	desc = "Convert an ALEPH E.G.O into a weapon comptaible with your suit. Can only be used once."
	button_icon = 'tff_modular/modules/evento_needo/icons/Teguicons/ego_weapons.dmi'
	button_icon_state = ""
	base_icon_state = "template"
	var/target_type = null ///obj/item/ego_weapon/mimicry
	//var/obj/structure/toolabnormality/wishwell/linked_structure

/datum/action/cooldown/ego_assimilation/Activate(atom/target)
	..()
	target = FindItems(target)//take the return value of the FindItems() proc here
	if(!target)
		to_chat(target, span_notice("There are no E.G.O weapons nearby."))
		return
	if(!istype(target, /obj/item/ego_weapon))
		to_chat(target, span_notice("That is not an E.G.O weapon."))
		return
	//if(!linked_structure)//Refer to wishing well for a list of all ALEPH E.G.O
	//	linked_structure = GLOB.wishwell
	//	if(!linked_structure)
	//		to_chat(target, span_notice("This ability is currently unavailable."))
	//		return
	//if(target.type in linked_structure.alephitem)//"alephitem" is a list
	//	new target_type(get_turf(target))
	//	qdel(target)
	//	DeleteAbility(target)//Deletes the ability and removes it from the ego suit
	//	return
	to_chat(target, span_notice("Target's risk level is too low."))

/datum/action/cooldown/ego_assimilation/proc/FindItems(target)
	var/list/stufflist = list()
	var/obj/item/ego_weapon/chosen_ego
	for(var/obj/item/ego_weapon/i in view(2, target))
		stufflist += i
	chosen_ego = input(target, "Which E.G.O will you assimilate?") as null|anything in stufflist
	if(!chosen_ego)
		return
	return chosen_ego

/datum/action/cooldown/ego_assimilation/proc/DeleteAbility(mob/living/carbon/human/target)
	var/obj/item/clothing/suit/armor/ego_gear/realization/mysuit = target.get_item_by_slot(ITEM_SLOT_OCLOTHING)
	if(!istype(mysuit))
		return
	//mysuit.realized_ability = null//sets it to a null value
	qdel(src)

/datum/action/cooldown/ego_assimilation/farmwatch
	base_icon_state = "farmwatch"
	button_icon_state = "farmwatch"
	//target_type = /obj/item/ego_weapon/farmwatch

/datum/action/cooldown/ego_assimilation/spicebush
	base_icon_state = "spicebush"
	button_icon_state = "spicebush"
	//target_type = /obj/item/ego_weapon/spicebush

/datum/action/cooldown/ego_assimilation/gasharpoon
	base_icon_state = "gasharpoon"
	button_icon_state = "gasharpoon"
	//target_type = /obj/item/ego_weapon/shield/gasharpoon

/* Fragment of the Universe - One with the Universe */
/datum/action/cooldown/universe_song
	name = "Song of the Universe"
	desc = "An ability that allows its target to damage and slow down the enemies around them."
	button_icon_state = "universe_song0"
...
/datum/action/cooldown/universe_song/Activate(atom/target)
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/fragment/sing.ogg', 50, 0, 4)
	Pulse(target)
	for(var/i = 1 to damage_count - 1)
		addtimer(CALLBACK(src, PROC_REF(Pulse), target), i*3)
	return ..()

/datum/action/cooldown/universe_song/proc/Pulse(mob/target)
	new /obj/effect/temp_visual/fragment_song(get_turf(target))
	for(var/mob/living/L in view(damage_range, target))
		if(target.faction_check_atom(L, FALSE))
			continue
		if(L.stat == DEAD)
			continue
		L.apply_damage(damage_amount, BRAIN, null, L.run_armor_check(null, BRAIN))
...
/datum/action/cooldown/shrimp
	name = "Backup Shrimp"
	desc = "Spawns 6 wellcheers corp liquidation officers for a period of time."
	button_icon_state = "shrimp0"
	base_icon_state = "shrimp"
	cooldown_time = 90 SECONDS



/datum/action/cooldown/shrimp/Activate(atom/target)
	for(var/i = 1 to 6)
		new /mob/living/simple_animal/hostile/shrimp_soldier/friendly/capitalism_shrimp(get_turf(target))
	return ..()

/* Big Bird - Eyes of God */
/datum/action/cooldown/lamp
	name = "Lamp of Salvation"
	desc = "An ability that slows and weakens all enemies around the target."
	button_icon_state = "lamp0"
...
/datum/action/cooldown/lamp/Activate(atom/target)
	StartCooldown(2 SECONDS)
	if(!do_after(target, 1.5 SECONDS))
		to_chat(target, span_warning("You must stand still to see!"))
		return
	var/mob/living/carbon/human/living = target
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bigbird/hypnosis.ogg', 75, 0, 2)
	for(var/mob/living/L in view(debuff_range, target))
		if(living.faction_check_atom(L, FALSE))
			continue
		if(L.stat == DEAD)
			continue
		new /obj/effect/temp_visual/revenant(get_turf(L))
...
/* Nothing There - Shell */
/datum/action/cooldown/goodbye
	name = "Goodbye"
	desc = "An ability that does massive damage in an area and heals you."
	button_icon_state = "goodbye0"
	base_icon_state = "goodbye"
	cooldown_time = 30 SECONDS

	var/damage_amount = 400 // Amount of good bye damage

/datum/action/cooldown/goodbye/Activate(atom/target)
	var/mob/living/carbon/human/H = target
	StartCooldown(1.5 SECONDS)
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/goodbye_cast.ogg', 75, 0, 5)
	if(!do_after(target, 1 SECONDS))
		to_chat(target, span_warning("You must stand still to do the nothing there classic!"))
		return
	for(var/turf/T in view(2, target))
		new /obj/effect/temp_visual/nt_goodbye(T)
		for(var/mob/living/L in T)
			if(H.faction_check_atom(L, FALSE))
				continue
			if(L.stat == DEAD)
				continue
			H.adjustBruteLoss(-10)
			L.apply_damage(damage_amount, BURN, null, L.run_armor_check(null, BURN), spread_damage = TRUE)
			if(L.health < 0)
				L.gib()
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nothingthere/goodbye_attack.ogg', 75, 0, 7)
	return ..()
/* Mosb - Laughter */
/datum/action/cooldown/screach
	name = "Screach"
	desc = "An ability that damages all enemies around the target and increases their weakness to black damage."
...
/datum/action/cooldown/screach/Activate(atom/target)
	StartCooldown(2 SECONDS)
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/bite.ogg', 50, 0)
	if(!do_after(target, 1.5 SECONDS))
		to_chat(target, span_warning("You must stand still to screach!"))
		return
	var/mob/living/carbon/human/H = target
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/scream.ogg', 75, 0, 2)
	...
	for(var/mob/living/L in view(damage_range, target))
		...
		if(L.stat == DEAD)
			continue
		L.apply_damage(ishuman(L) ? damage_amount*0.5 : damage_amount, BRUTE, null, L.run_armor_check(null, BRUTE))
...
/datum/action/cooldown/judgement
	name = "Soul Judgement"
	desc = "An ability that damages all enemies around the target and increases their weakness to pale damage."
	button_icon_state = "judgement0"
...
/datum/action/cooldown/judgement/Activate(atom/target)
	StartCooldown(2 SECONDS)
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/judgementbird/pre_ability.ogg', 50, 0)
	var/obj/effect/temp_visual/judgement/still/J = new (get_turf(target))
	animate(J, pixel_y = 24, time = 1.5 SECONDS)
	if(!do_after(target, 1.5 SECONDS))
		to_chat(target, span_warning("You must stand still to perform judgement!"))
		return
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/judgementbird/ability.ogg', 75, 0, 2)
	for(var/mob/living/L in view(damage_range, target))
		if(L.faction_check_atom(target, FALSE))
			continue
		if(L.stat == DEAD)
			continue
		new /obj/effect/temp_visual/judgement(get_turf(L))
		L.apply_damage(ishuman(L) ? damage_amount*0.5 : damage_amount, TOX, null, L.run_armor_check(null, TOX))
...
/datum/action/cooldown/fire_explosion
	name = "Match flame"
	desc = "An ability that deals high amount of RED damage to EVERYONE around the target after short delay."
...
/datum/action/cooldown/fire_explosion/Activate(atom/target)
	StartCooldown(5 SECONDS)
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scorchedgirl/pre_ability.ogg', 50, 0, 2)
	if(!do_after(target, 1.5 SECONDS))
		to_chat(target, span_warning("You must stand still to ignite the explosion!"))
		return
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scorchedgirl/ability.ogg', 60, 0, 4)
	var/obj/effect/temp_visual/human_fire/F = new(get_turf(target))
	F.alpha = 0
	F.dir = target.dir
	animate(F, alpha = 255, time = (2 SECONDS))
	if(!do_after(target, 2.5 SECONDS))
		to_chat(target, span_warning("You must stand still to finish the ability!"))
		animate(F, alpha = 0, time = 5)
		return
	animate(F, alpha = 0, time = 5)
	INVOKE_ASYNC(src, PROC_REF(FireExplosion), get_turf(target))
	return ..()

/datum/action/cooldown/fire_explosion/proc/FireExplosion(turf/T)
	playsound(T, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/scorchedgirl/explosion.ogg', 125, 0, 8)
	for(var/i = 1 to explosion_range)
		for(var/turf/open/TT in spiral_range_turfs(i, T) - spiral_range_turfs(i-1, T))
			...
			for(var/mob/living/L in TT)
				...
				L.apply_damage(ishuman(L) ? explosion_damage*0.5 : explosion_damage, BURN, null, L.run_armor_check(null, BURN))
		sleep(1)

/* King of Greed - Gold Experience */
/datum/action/cooldown/road_of_gold
...
/datum/action/cooldown/road_of_gold/Activate(atom/target)
	if(!istype(target))
		return ..()
	StartCooldown(2 SECONDS)
	var/mob/living/carbon/human/living = target
	...
	for(var/mob/living/simple_animal/hostile/H in GLOB.alive_mob_list)
		...
		if(HAS_TRAIT(H, TRAIT_GODMODE))
			continue
		if(living.faction_check_atom(H, FALSE))
			continue
		if(H in view(7, target))
			continue
		var/t_dist = get_dist(target, H)
		if(t_dist >= dist)
			continue
		dist = t_dist
		target = H
	if(!target)
		to_chat(target, span_notice("You can't find anything else nearby!"))
		return ..()
	Circle(null, null, target)
	var/pre_circle_dir = target.dir
	to_chat(target, span_warning("You begin along the Road of Gold to your target!"))
	if(!do_after(target, 15, src))
		to_chat(target, span_warning("You abandon your path!"))
		CleanUp()
		return ..()
	animate(target, alpha = 0, time = 5)
	step_towards(target, get_step(target, pre_circle_dir))
	new /obj/effect/temp_visual/guardian/phase(get_turf(src))
	var/turf/open/target_turf = get_step_towards(target, target)
	if(!istype(target_turf))
		target_turf = pick(get_adjacent_open_turfs(target))
	if(!target_turf)
		to_chat(target, span_warning("No road leads to that target!?"))
		CleanUp()
		return ..()
	var/obj/effect/qoh_sygil/kog/KS = Circle(target_turf, get_step(target_turf, pick(GLOB.cardinals)), null)
	sleep(5)
	target.dir = get_dir(target, target)
	animate(target, alpha = 255, time = 5)
	new /obj/effect/temp_visual/guardian/phase/out(get_turf(KS))
	living.forceMove(get_turf(KS))
	CleanUp()
	sleep(2.5)
	step_towards(target, get_step_towards(KS, target))
	if(get_dist(target, target) <= 1)
		var/obj/item/held = living.get_active_held_item()
		if(held)
			held.attack(target, target)
	return ..()

/datum/action/cooldown/road_of_gold/proc/CleanUp()
	for(var/obj/effect/FX in spawned_effects)
		if(istype(FX, /obj/effect/qoh_sygil/kog))
			...
			continue
		FX.Destroy()
	list_clear_nulls(spawned_effects)

/datum/action/cooldown/road_of_gold/proc/Circle(turf/first_target, turf/second_target, mob/target = null)
	var/obj/effect/qoh_sygil/kog/KS
	if(target)
		KS = new(get_turf(target))
	else
		KS = new(first_target)
	spawned_effects += KS
	...
	if(target)
		my_dir = target.dir
		rot_angle = get_angle(target, get_step(target, my_dir))
	else
...
/datum/action/cooldown/justice_and_balance
	...
	button_icon_state = "justicebalance0"
	...
	var/list/spawned_effects = list()
	var/list/SFX = list(
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash3.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash2.ogg',
		'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash1.ogg'
		)
	var/damage = 30
	var/list/targets_hit = list()

/datum/action/cooldown/justice_and_balance/Activate(atom/target)
	INVOKE_ASYNC(src, PROC_REF(Smash), target, charges)
	charges--
	if(charges < 1)
		charges = max_charges
		targets_hit = list()
...
/datum/action/cooldown/justice_and_balance/proc/Smash(mob/living/carbon/human/target, on_use_charges)
	playsound(target, SFX[on_use_charges], 25*(4-on_use_charges))
	var/temp_dam = damage

	var/mob/living/carbon/human/living = owner
	temp_dam *= 1 + living.get_clothing_class_level(CLOTHING_ARMORED)
	if(on_use_charges <= 1)
		temp_dam *= 1.5
	for(var/turf/open/T in range(3, target))
		...
		for(var/mob/living/L in T)
			...
			if(L.stat == DEAD)
				continue
			if(target.faction_check_atom(L))
				continue
			if(L in targets_hit)
				targets_hit[L] += 1
			else
				targets_hit[L] = 1
			L.apply_damage(temp_dam, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
	if(!ishuman(target))
		return
	var/mob/living/carbon/human/H = target
...
/datum/action/cooldown/punishment
	name = "Punishment"
	desc = "Causes massive damage in a small area only when you take a blow."
	button_icon_state = "bird0"
	base_icon_state = "bird"
	cooldown_time = 25 SECONDS

/datum/action/cooldown/punishment/Activate(atom/target)
	var/mob/living/carbon/human/H = target
...
/datum/status_effect/punishment/on_apply()
	. = ..()
	RegisterSignal(owner, COMSIG_MOB_APPLY_DAMAGE, PROC_REF(Rage))
...
/datum/status_effect/punishment/proc/Rage(mob/living/sorce, obj/item/thing, mob/living/attacker)
	...
	playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/apocalypse/beak.ogg', 100, FALSE, 12)
	for(var/turf/T in view(2, H))
		...
		for(var/mob/living/L in T)
			...
			if(L.stat == DEAD)
				continue
			L.apply_damage(500, BURN, null, L.run_armor_check(null, BURN), spread_damage = TRUE)
			if(L.health < 0)
				L.gib()
...
/datum/action/cooldown/petal_blizzard
	name = "Petal Blizzard"
...
/datum/action/cooldown/petal_blizzard/Activate(atom/target)
	var/mob/living/carbon/human/H = target
	to_chat(H, span_userdanger("You feel frailer!"))
	H.apply_status_effect(/datum/status_effect/bloomdebuff)
	//playsound(get_turf(target), 'sound/items/weapons/fixer/generic/sword3.ogg', 75, 0, 7)
	var/mob/living/carbon/human/living = target
	for(var/turf/T in view(healing_range, target))
...
/datum/status_effect/bloomdebuff/on_apply()
	...
	if(ishuman(owner))
		...
		L.physiology.burn_mod *= 1.5
		L.physiology.brain_mod *= 1.5
		L.physiology.brute_mod *= 1.5
		L.physiology.tox_mod *= 1.5
...
/mob/living/simple_animal/hostile/spicebush_plant/death()
	density = FALSE
	//playsound(src, 'sound/items/weapons/ego/farmwatch_tree.ogg', 100, 1)
...
/mob/living/simple_animal/hostile/spicebush_plant/proc/HealPulse()
	...
	//playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/rudolta/throw.ogg', 50, FALSE, 4)//TODO: proper SFX goes here
	for(var/mob/living/carbon/human/L in range(8, src))//view(8, src))
		if(L.stat == DEAD)
			continue
		L.adjustBruteLoss(-2)
		L.adjustSanityLoss(-2)

/datum/action/cooldown/overheat
	name = "Overheat"
	desc = "Burn yourself away in exchange for power."
	button_icon_state = "overheat0"
	base_icon_state = "overheat"
	cooldown_time = 5 MINUTES

/datum/action/cooldown/overheat/Activate(atom/target)
	var/mob/living/carbon/human/H = target
...
/* Yang - Duality */
/datum/action/cooldown/tranquility
	name = "Tranquility"
...
/datum/action/cooldown/tranquility/Activate(atom/target)
	StartCooldown(1.5 SECONDS)
	if(!do_after(target, 1 SECONDS))
		to_chat(target, span_warning("You must stand still to explode!"))
		return
	...
	for(var/i = 0 to explosion_range)
		for(var/turf/T in all_turfs)
			if(get_dist(target, T) > i)
				continue
			new /obj/effect/temp_visual/dir_setting/speedbike_trail(T)
			//target.HurtInTurf(damage_amount, list(), BRAIN)
			for(var/mob/living/carbon/human/L in T)
				if(!L.faction_check_atom(target, FALSE))
					continue
				if(L.stat == DEAD)
...
/datum/action/cooldown/galaxy_gift/Activate(atom/target)
	...
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(H.has_status_effect(/datum/status_effect/galaxy_gift))
			existing_gifted += H
	playsound(get_turf(target), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/despairknight/gift.ogg', 50, 0, 2) //placeholder, uses KoD blessing noise at the moment
	for(var/turf/T in view(range, target))
		new /obj/effect/temp_visual/galaxy_aura(T)
		for(var/mob/living/carbon/human/H in T)
			if(!H.faction_check_atom(target, FALSE))
				continue
			if(H.stat == DEAD)
				continue
			var/datum/status_effect/galaxy_gift/new_gift = H.apply_status_effect(/datum/status_effect/galaxy_gift)
			new_gift.caster = target
			if(H == target)
...
/datum/status_effect/galaxy_gift/proc/Pop()
	...
	for(var/mob/living/carbon/human/H in gifted)
		H.apply_damage(base_dmg_amt*damage_mult, BRUTE, null, H.run_armor_check(null, BRUTE), spread_damage = TRUE)
		H.remove_status_effect(/datum/status_effect/galaxy_gift)
...
/datum/action/cooldown/comatose
	name = "Comatose"
	desc = "Fall asleep to gain 99% resistance to all normal damage."
	button_icon_state = "comatose0"
	base_icon_state = "comatose"
	cooldown_time = 30 SECONDS

/datum/action/cooldown/comatose/Activate(atom/target)
	//if(istype(target.get_item_by_slot(ITEM_SLOT_OCLOTHING), /obj/item/clothing/suit/armor/ego_gear/realization/comatose))
	////	target.Stun(15 SECONDS)
	////	target.Knockdown(1)
	////	target.playsound_local(get_turf(target), "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/happyteddy/teddy_lullaby.ogg", 25, 0)
	////	target.apply_status_effect(/datum/status_effect/dreaming)
	//	return ..()
...
/datum/status_effect/dreaming/on_apply()
	. = ..()
	var/mob/living/carbon/human/H = owner
	H.physiology.burn_mod /= 100
	H.physiology.brain_mod /= 100
	H.physiology.brute_mod /= 100
	H.physiology.tox_mod /= 100
...
/datum/action/cooldown/brokencrown
	name = "Broken Crown"
	...
	button_icon_state = "brokencrown0"
...
/datum/action/cooldown/brokencrown/proc/Absorb(obj/item/I, mob/living/target)
	if(!ego_list || ready)
		to_chat(target, span_notice("You need to use this ability before you can recharge it!"))
		return FALSE
...
/datum/action/cooldown/brokencrown/proc/Reload(obj/item/I, mob/living/target)
	to_chat(target, span_nicegreen("The ability has been recharged."))
	ready = TRUE
	qdel(I)

/datum/action/cooldown/brokencrown/Activate(atom/target)
	if(!ready)
		to_chat(target, span_notice("This ability has been spent and needs to be recharged."))
		return
	//if(istype(target.get_item_by_slot(ITEM_SLOT_OCLOTHING), /obj/item/clothing/suit/armor/ego_gear/realization/brokencrown))
	//	target.playsound_local(get_turf(target), "tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/bloodbath/Bloodbath_EyeOn.ogg", 25, 0)
	//	if(!linked_structure)
	...
	//		var/obj/item/ego_weapon/egoweapon = new ego(get_turf(target))
...
//
	...
	//		var/obj/item/ego_weapon/ranged/egogun = new ego(get_turf(target))
...
/datum/action/cooldown/brokencrown/proc/Reabsorb()
	if(linkeditem && !ready)
...
/datum/action/cooldown/wellcheers
...
/datum/action/cooldown/wellcheers/Activate(atom/target)
	for(var/i = 1 to 3)
		new /mob/living/simple_animal/hostile/shrimp/friendly(get_turf(target))
	return ..()
...
/datum/action/cooldown/prayer
	...
	button_icon_state = "flesh0"
	base_icon_state = "flesh"
	cooldown_time = 180 SECONDS

/datum/action/cooldown/prayer/Activate(atom/target)
	var/mob/living/carbon/human/living = target
	living.apply_status_effect(/datum/status_effect/flesh1)
	StartCooldown(15 SECONDS)
	to_chat(target, span_userdanger("You start praying..."))
	if(!do_after(target, 15 SECONDS))
		living.remove_status_effect(/datum/status_effect/flesh1)
		return
	for(var/mob/living/carbon/human/H in GLOB.player_list)
		if(!living.faction_check_atom(H, FALSE))
			continue
		...
		if(H.z != target.z)
			continue
		playsound(H, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/onesin/bless.ogg', 100, FALSE, 12)
		to_chat(H, span_nicegreen("[target]'s prayer was heard!"))
...
/datum/action/cooldown/nest
	name = "Worm spawn"
	desc = "Spawns 7 worms that will seak out abormalities to infest in making them weaker to red damage."
	button_icon_state = "worm0"
	base_icon_state = "worm"
	cooldown_time = 30 SECONDS



/datum/action/cooldown/nest/Activate(atom/target)
	for(var/i = 1 to 7)
		playsound(get_turf(target), 'sound/misc/moist_impact.ogg', 30, 1)
		var/landing
		landing = locate(target.x + pick(-2,-1,0,1,2), target.y + pick(-2,-1,0,1,2), target.z)
		var/mob/living/simple_animal/hostile/naked_nest_serpent_friend/W = new(get_turf(target))
		W.origin_nest = target
...
/mob/living/simple_animal/hostile/naked_nest_serpent_friend
	...
	move_to_delay = 3
	damage_coeff = list(BRUTE = 1, BURN = 1, BRAIN = 1, BRUTE = 1, TOX = 1)
...
/datum/action/cooldown/rip_space
	name = "Rip Space"
	desc = "Travel at light speed between portals to attack your enemies."
	button_icon_state = "ripper0"
	base_icon_state = "ripper"
	cooldown_time = 1 MINUTES

/datum/action/cooldown/rip_space/Activate(atom/target)
	var/list/targets = list()
	var/mob/living/carbon/human/living = target
	for(var/mob/living/L in view(8, target))
		if(L.stat == DEAD)
			continue
		if(HAS_TRAIT(L, TRAIT_GODMODE))
			continue
		...
		targets += L
	if(!(LAZYLEN(targets)))
		to_chat(target, span_warning("There are no enemies nearby!"))
		return

	StartCooldown(7 SECONDS)
	...
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/ripspace_begin.ogg', 100, 0)
	sleep(1 SECONDS)
	qdel(P)
	target.alpha = 0
	...
	for(var/i = 1 to dash_count)
		...
		dash_attack(L, target)
		if(L.stat == DEAD)
			targets -= L
		if(!LAZYLEN(targets) || living.stat == DEAD)
			break

	target.alpha = 255
	new /obj/effect/temp_visual/rip_space(origin)
	living.forceMove(origin)
	target.density = TRUE
...
/datum/action/cooldown/rip_space/proc/dash_attack(mob/living/target, mob/living/target)
	...
	DE.set_angle(get_angle(X, Y))
	DE.name = target.name
	DE.fire()
	target.orbit(DE, 0, 0, 0, 0, 0)

	sleep(1)
	target.apply_damage(100, BURN, null, target.run_armor_check(null, BURN))
	new /obj/effect/temp_visual/rip_space_slash(get_turf(target))
	new /obj/effect/temp_visual/ripped_space(get_turf(target))
	playsound(target, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wayward_passenger/ripspace_hit.ogg', 75, 0)
	sleep(1)
	qdel(DE)
...
/obj/projectile/ripper_dash_effect/on_hit(atom/target, blocked = FALSE, pierce_hit)
	damage = 0
	return ..()
