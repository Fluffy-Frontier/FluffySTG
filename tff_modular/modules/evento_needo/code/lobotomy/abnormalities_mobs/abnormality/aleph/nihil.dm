...
/mob/living/simple_animal/hostile/abnormality/nihil
	...
	damage_coeff = list(BURN = 1, BRAIN = 0.3, BRUTE = 0.3, TOX = 0.5) //change on phase
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_attack_hard.ogg'
	ranged = TRUE
	casingtype = /obj/item/ammo_casing/caseless/nihil_abnormality
	projectilesound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_magic.ogg'
...
//Work Code
/mob/living/simple_animal/hostile/abnormality/nihil/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-2)
	return

/mob/living/simple_animal/hostile/abnormality/nihil/SuccessEffect(mob/living/carbon/human/user)
	qliphoth_change(1)

/mob/living/simple_animal/hostile/abnormality/nihil/try_working(mob/living/carbon/human/user)
	if(user.get_major_clothing_class() == CLOTHING_ARMORED)
		return ..()
	else
		user.show_aso_blurb("Не хватает силы")
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/AreaAttack() //Replaces normal attack
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_magic.ogg', 60, FALSE, 10)
	for(var/turf/T in range(1, src)) //First hit is just an AOE around nihil
		new /obj/effect/temp_visual/eldritch_smoke(T)
		for(var/mob/living/L in HurtInTurf(T, list(), damage_dealt, BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
			if(GirlCheck(L)) //EXTRA magical girl damage to kill them faster
				L.apply_damage((2 * damage_dealt), BRUTE)
	SLEEP_CHECK_DEATH(8, src)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/hermit_attack_hard.ogg', 25, FALSE, 15, falloff_distance = 5)
	new /obj/effect/temp_visual/voidout(myturf)
	for(var/turf/T in range(1, myturf)) //Second hit is avoidable but deals 3x damage
		for(var/mob/living/L in HurtInTurf(T, list(), (3 * damage_dealt), BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
			L.apply_void(3)
			if(GirlCheck(L)) //EXTRA magical girl damage to kill them faster
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/NukeAttack(forced) //Phase-change attack with a long cooldown
	...
	addtimer(CALLBACK(src, PROC_REF(NukeAttack_second)), 50)

/mob/living/simple_animal/hostile/abnormality/nihil/proc/NukeAttack_second()
	...
	for(var/mob/living/L in view(25, src))
		if(L.z != z)
			continue
		...
		L.apply_damage(clamp((damage_mod * (25 - dist)), 15, nuke_damage), BRUTE) //Between 500 and 15 damage, scaling down heavily past a distance of 7 tiles
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/TryTeleport(forced = FALSE)
	...
	if(!LAZYLEN(teleport_potential)) //2nd priority - anyone alive
		for(var/mob/living/L in GLOB.mob_living_list)
			if(L.stat == DEAD || L.z != z || HAS_TRAIT(L, TRAIT_GODMODE) || faction_check_atom(L))
				continue
			teleport_potential += get_turf(L)
	...
	for(var/mob/living/L in urange(13, src)) //vfx
		if(L.z == z && L.client)
			shake_camera(L, 10, 1)
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/gun.ogg', 65, FALSE, 10)
	...
	SLEEP_CHECK_DEATH(2 SECONDS, src) //2 seconds to teleport
	invisibility = 0
	density = TRUE
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/TeleportOut(turf/teleport_target)
	...
	switch(current_phase)
		if("GREED")
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/weapons/fixer/generic/dodge.ogg', 75, FALSE, 10)
			for(var/turf/T in view(2, src))
				new /obj/effect/temp_visual/small_smoke(T)
				for(var/mob/living/H in HurtInTurf(T, list(), (0.5 * explode_damage), BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
		...
		if("DESPAIR")
			...
			for(var/i = 1 to 10)
				...
				var/list/hit_line = get_line(T, get_turf(target)) //targetting line
				for(var/turf/TF in hit_line)
					if(TF.density)
						break
					new /obj/effect/temp_visual/cult/sparks(TF)
			playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/despairknight/dead.ogg', 50, 0, 2)
		...
		if("WRATH")
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/wrath_servant/big_smash3.ogg', 75, FALSE, 10, falloff_distance = 5)
			for(var/turf/T in view(2, src))
				HurtInTurf(T, list(), explode_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/TeleportIn()
	switch(current_phase)
		if("GREED")
			...
			playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/kog/GreedHit1.ogg', 75, FALSE, 10)
			for(var/turf/T in view(3, src))
				new /obj/effect/temp_visual/small_smoke(T)
				for(var/mob/living/H in HurtInTurf(T, list(), (2 * explode_damage), BRUTE, check_faction = TRUE, hurt_mechs = TRUE))
					visible_message("[src] tosses [H] out of the way!")
					var/rand_dir = pick(NORTH, SOUTH, EAST, WEST)
					var/atom/throw_target = get_edge_target_turf(H, rand_dir)
					if(!H.anchored)
...
/mob/living/simple_animal/hostile/abnormality/nihil/proc/Debuff(attack_count,event_start)
	if(attack_count > 13)
		...
		return
	if(!attack_count)
		sound_to_playing_players(sound("tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nihil/attack.ogg"))
...
/mob/living/simple_animal/hostile/abnormality/nihil/BreachEffect(mob/living/carbon/human/user)
	...
	for(var/turf/T in GLOB.generic_event_spawns)
		for(var/mob/living/simple_animal/forest_portal/FP in T.contents) // Prevents breaching on top of apocalypse bird
			potential_spawns = GLOB.generic_event_spawns.Copy()
			potential_spawns -= T
			continue
		portal = new(T)
		break
...
/mob/living/simple_animal/nihil_portal/Initialize()
	...
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/dead.ogg', 100, FALSE, 40, falloff_distance = 10) //Play a weird sound
...
/obj/effect/magical_girl_portal/proc/SpawnGirl()
	...
	for(var/mob/living/simple_animal/hostile/abnormality/A in GLOB.abnormality_mob_list) //breach the girls
		...
		playsound(girltarget, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/hatredqueen/attack.ogg', 60, TRUE, 10)
...
/datum/status_effect/stacking/void/add_stacks(stacks_added)
	...
	status_holder.physiology.burn_mod -= 10 * stacks_added
	status_holder.physiology.brain_mod -= 10 * stacks_added
	status_holder.physiology.brute_mod -= 10 * stacks_added
	status_holder.physiology.tox_mod -= 10 * stacks_added
...
/datum/status_effect/stacking/void/on_remove()
	...
	if(!ishuman(owner))
		return
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.burn_mod += 10 * stacks
	status_holder.physiology.brain_mod += 10 * stacks
	status_holder.physiology.brute_mod += 10 * stacks
	status_holder.physiology.tox_mod += 10 * stacks
...
/obj/structure/statue/petrified/magicalgirl/Destroy()
	...
	if(petrified_mob)
		REMOVE_TRAIT(petrified_mob, TRAIT_GODMODE, ADMIN_TRAIT)
...
/mob/living/proc/apply_void(stacks)
	...
	if(!V)
		...
		SLEEP_CHECK_DEATH(1, src) //Prevent runtimes
		G.add_stacks(stacks - 1)
	else
		V.add_stacks(stacks)
		V.refresh()
		playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/nihil/filter.ogg', 15, FALSE, -3)
