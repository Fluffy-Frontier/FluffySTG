/mob/living/simple_animal/hostile/abnormality/mountain
	...
	pixel_x = -16
	base_pixel_x = -16
	damage_coeff = list(BURN = 1.2, BRAIN = 0.8, BRUTE = 0.8, TOX = 0.5)
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/bite.ogg'
...
/mob/living/simple_animal/hostile/abnormality/mountain/AttackingTarget(atom/attacked_target)
	...
	if(. && isliving(attacked_target))
		...
		if(isliving(attacked_target) && (L.health < 0 || L.stat == DEAD))
			...
			var/atom/targett = locate(targets_from)
...
/mob/living/simple_animal/hostile/abnormality/mountain/attacked_by(obj/item/I, mob/living/user)
	if(CanAttack(user)) // Basically, it will retaliate while patrolling without stopping or chasing after them
...
/mob/living/simple_animal/hostile/abnormality/mountain/proc/StageChange(increase = TRUE)
	// Increase stage
	if(phase <= 1)
		stop_automated_movement = !(HAS_TRAIT(src, TRAIT_GODMODE))
	else
		stop_automated_movement = (HAS_TRAIT(src, TRAIT_GODMODE))
	if(increase)
		if(belly >= 2)
			if(phase < 3)
				playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/level_up.ogg', 75, 1)
				adjustHealth(-5000)
				maxHealth += 1000
				phase += 1
				belly = 0
			...
			if(phase == 3)
				...
				move_to_delay = 30 SECONDS
			if(phase == 2)
				icon_living = "mosb_breach"
				move_to_delay = 20 SECONDS
			icon_state = icon_living
		return
	// Decrease stage
	if(phase <= 1) // Death
		return FALSE
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/level_down.ogg', 75, 1)
	adjustHealth(-5000)
	maxHealth -= 1000
	phase -= 1
	icon_living = "mosb_breach"
...
/mob/living/simple_animal/hostile/abnormality/mountain/proc/Scream()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/scream.ogg', 75, 1, 5)
	var/list/been_hit = list()
	for(var/turf/T in view(7, src))
		HurtInTurf(T, been_hit, scream_damage, BRUTE, null, TRUE, FALSE, TRUE, hurt_hidden = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/mountain/proc/Slam(range)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/slam.ogg', 75, 1)
	var/list/been_hit = list()
	for(var/turf/open/T in view(2, src))
		new /obj/effect/temp_visual/small_smoke/halfsecond(T)
		HurtInTurf(T, been_hit, slam_damage, BRUTE, null, TRUE, FALSE, TRUE, hurt_hidden = FALSE, hurt_structure = TRUE, break_not_destroy = TRUE)
...
/mob/living/simple_animal/hostile/abnormality/mountain/proc/Spit(atom/target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/mountain/spit.ogg', 75, 1, 3)
	for(var/k = 1 to 3)
...
/mob/living/simple_animal/hostile/abnormality/mountain/handle_automated_movement()
...
/mob/living/simple_animal/hostile/abnormality/mountain/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return

/mob/living/simple_animal/hostile/abnormality/mountain/PostWorkEffect(mob/living/carbon/human/user)
	if(user.health < 0)
		qliphoth_change(-1)
	if(agent_hurt)
		qliphoth_change(-1)
