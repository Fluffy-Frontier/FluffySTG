...
/mob/living/simple_animal/hostile/abnormality/last_shot
	...
	damage_coeff = list(BURN = 1, BRAIN = 1, BRUTE = 1.5, TOX = 2)
...
/mob/living/simple_animal/hostile/abnormality/last_shot/try_working(mob/living/carbon/human/user)
	...
	if(user.get_clothing_class_level(CLOTHING_SERVICE) >= 4)
		neutral_chance = 30
		negative_chance = 50
	if(user.get_clothing_class_level(CLOTHING_ARMORED) <= 4 || user.get_clothing_class_level(CLOTHING_ENGINEERING) <= 4)
		neutral_chance = 25
		negative_chance = 55
	return ..()

/mob/living/simple_animal/hostile/abnormality/last_shot/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/abnormality/last_shot/Life()
	...
	if(!.)
		return FALSE
	if((spawn_cooldown < world.time) && !(HAS_TRAIT(src, TRAIT_GODMODE)))
...
/mob/living/simple_animal/hostile/abnormality/last_shot/proc/MeatSpawn()
	...
	for(var/mob/living/simple_animal/hostile/meatblob/theblob in range(meat_reach, src))
		...
		if(guards_count > 2) //We have enough guards
			theblob.wander = TRUE //Send the rest out
	return
...
/mob/living/simple_animal/hostile/meatblob
	...
	move_to_delay = 4.5
	wander = TRUE //The stronger blobs use commander AI, these will wander if alone.
...
/mob/living/simple_animal/hostile/meatblob/gunner
	...
	wander = FALSE //They're sent out in waves
	var/guntimer //takes time to reload after 20 shots
	var/can_act = TRUE
	var/reload_time = 15
	var/remaining_bullets = 20
	var/maximum_bullets = 20
	var/reload_sound = 'sound/items/weapons/gun/general/bolt_rack.ogg'
