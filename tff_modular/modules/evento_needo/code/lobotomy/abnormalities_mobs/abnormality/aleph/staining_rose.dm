...
/mob/living/simple_animal/hostile/abnormality/staining_rose/proc/meltdown_check()
	sound_to_playing_players('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/rose/meltdown.ogg')	//Church bells ringing, whether it happens or not.
	if(chosen)
...
/datum/weather/petals/weather_act_mob(mob/living/living)
	. = ..()
	if(ishuman(living))
		living.apply_status_effect(STATUS_EFFECT_WILTING)
...
/datum/status_effect/wilting/on_apply()
	...
	status_holder.physiology.burn_mod /= 0.5
	status_holder.physiology.brain_mod /= 0.5
	status_holder.physiology.brute_mod /= 0.5
...
/datum/status_effect/sacrifice/on_apply()
	...
	if(!ishuman(owner))
		return
	var/mob/living/carbon/human/status_holder = owner
	status_holder.physiology.burn_mod /= 0.7
	status_holder.physiology.brain_mod /= 0.7
	status_holder.physiology.brute_mod /= 0.7
	status_holder.physiology.tox_mod /= 0.7
