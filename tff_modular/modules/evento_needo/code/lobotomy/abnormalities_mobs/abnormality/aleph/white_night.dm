...
/mob/living/simple_animal/hostile/abnormality/white_night
	...
	damage_coeff = list(BURN = 0.5, BRAIN = -2, BRUTE = 0.5, TOX = 0.2)
	pixel_x = -16
	base_pixel_x = -16
	pixel_y = -16
	base_pixel_y = -16
...
/mob/living/simple_animal/hostile/abnormality/white_night/Initialize(mapload)
	. = ..()
	addtimer(CALLBACK(src, PROC_REF(holy_revival_check)), holy_revival_cooldown_base)
...
/mob/living/simple_animal/hostile/abnormality/white_night/proc/holy_revival_check()
...
/mob/living/simple_animal/hostile/abnormality/white_night/death(gibbed)
	for(var/mob/living/carbon/human/heretic in heretics)
		...
		heretic.playsound_local(get_turf(heretic), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/apostle_bell.ogg', 50)
...
/mob/living/simple_animal/hostile/abnormality/white_night/proc/revive_humans(range_override = null, faction_check = "apostle")
	...
	been_hit = list()
	playsound(src, 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/apostle_spell.ogg', 75, 1, range_override)
	var/turf/target_c = get_turf(src)
	var/list/turf_list = list()
	for(var/i = 1 to range_override)
		turf_list = (target_c.y - i > 0 			? get_line(locate(max(target_c.x - i, 1), target_c.y - i, target_c.z), locate(min(target_c.x + i - 1, world.maxx), target_c.y - i, target_c.z)) : list()) +\
					(target_c.x + i <= world.maxx ? get_line(locate(target_c.x + i, max(target_c.y - i, 1), target_c.z), locate(target_c.x + i, min(target_c.y + i - 1, world.maxy), target_c.z)) : list()) +\
					(target_c.y + i <= world.maxy ? get_line(locate(min(target_c.x + i, world.maxx), target_c.y + i, target_c.z), locate(max(target_c.x - i + 1, 1), target_c.y + i, target_c.z)) : list()) +\
					(target_c.x - i > 0 			? get_line(locate(target_c.x - i, min(target_c.y + i, world.maxy), target_c.z), locate(target_c.x - i, max(target_c.y - i + 1, 1), target_c.z)) : list())
...
/mob/living/simple_animal/hostile/abnormality/white_night/proc/revive_target(mob/living/L, attack_range = 1, faction_check = "apostle")
	...
	if(!(faction_check in L.faction))
		playsound(L.loc, 'sound/machines/clockcult/steam_whoosh.ogg', 50 - attack_range, TRUE, -1)
...
/* Work effects */
/mob/living/simple_animal/hostile/abnormality/white_night/OnQliphothChange()
	...
	sound_to_playing_players('tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/apostle_bell.ogg', (25 * (3 - datum_reference.qliphoth_meter)))
	return

/mob/living/simple_animal/hostile/abnormality/white_night/SuccessEffect(mob/living/carbon/human/user)
	if(prob(66))
		qliphoth_change(1)
...
/mob/living/simple_animal/hostile/abnormality/white_night/FailureEffect(mob/living/carbon/human/user)
	qliphoth_change(-1)
	return
...
/mob/living/simple_animal/hostile/apostle
	...
	obj_damage = 400
	ranged = TRUE
	damage_coeff = list(BURN = 1.5, BRAIN = 0.5, BRUTE = 0.5, TOX = 0.5)
	move_to_delay = 5
	pixel_x = -8
	base_pixel_x = -8
	see_in_dark = 7
	...
	wander = TRUE // You have legs, use them.
	var/can_act = TRUE
	var/death_counter = 0
...
/mob/living/simple_animal/hostile/apostle/revive(full_heal = FALSE, excess_healing = 0, force_grab_ghost)
	.= ..()
...
/mob/living/simple_animal/hostile/apostle/scythe/proc/ScytheAttack()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/scythe_spell.ogg', 75, FALSE, 5)
...
/mob/living/simple_animal/hostile/apostle/scythe/guardian
	...
	move_to_delay = 7
	melee_damage_type = BRUTE
	damage_coeff = list(BURN = 0.5, BRAIN = 0.5, BRUTE = 0.5, TOX = 1.5)
	vision_range = 12
	aggro_vision_range = 12
	move_to_delay = 10 SECONDS
	scythe_range = 3
	scythe_cooldown_time = 8 SECONDS // More often, since the damage increase was disliked.
	scythe_damage_type = BRUTE
...
/mob/living/simple_animal/hostile/apostle/scythe/guardian/handle_automated_movement()
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/apostle_growl.ogg', 75, FALSE)
	add_movespeed_modifier(/datum/movespeed_modifier/white_night_patrol)
	addtimer(CALLBACK(src, PROC_REF(remove_movespeed_modifier), /datum/movespeed_modifier/white_night_patrol), 5 SECONDS)

/datum/movespeed_modifier/white_night_patrol
...
/mob/living/simple_animal/hostile/apostle/scythe/guardian/ScytheAttack()
	...
	for(var/turf/T in view(scythe_range, src))
		...
		for(var/mob/living/L in T) // Not changing this one because it notable does not gib pre-dead bodies, only living ones.
			...
			if(faction_check_atom(L))
				continue
			L.apply_damage(scythe_damage, scythe_damage_type)
...
/mob/living/simple_animal/hostile/apostle/spear
	...
	attack_sound = 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/spear.ogg'
	icon_state = "apostle_spear"
	icon_living = "apostle_spear"
	melee_damage_type = BRUTE
	damage_coeff = list(BURN = 0.5, BRAIN = 0.5, BRUTE = 1.5, TOX = 0.5)
	var/spear_cooldown
...
/mob/living/simple_animal/hostile/apostle/spear/proc/SpearAttack(target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/spear_charge.ogg', 75, 0, 5)
	SLEEP_CHECK_DEATH(22, src)
	been_hit = list()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/spear_dash.ogg', 100, 0, 20)
...
/mob/living/simple_animal/hostile/apostle/spear/proc/do_dash(move_dir, times_ran)
	...
	for(var/turf/TF in view(1, T))
		...
		var/list/new_hits = HurtInTurf(T, been_hit, spear_damage, BRUTE, check_faction = TRUE, hurt_mechs = TRUE, hurt_structure = TRUE) - been_hit
		been_hit += new_hits
...
/mob/living/simple_animal/hostile/apostle/staff
	...
	melee_damage_type = BRUTE // Okay, look, they aren't really meant to melee anyway
	damage_coeff = list(BURN = 0.5, BRAIN = 1.5, BRUTE = 0.5, TOX = 0.5)
	var/staff_cooldown
...
/mob/living/simple_animal/hostile/apostle/staff/proc/StaffAttack(target)
	...
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/whitenight/staff_prepare.ogg', 75, 0, 7)
	SLEEP_CHECK_DEATH(2.5 SECONDS, src)
	beamloop.start()
	SLEEP_CHECK_DEATH(hit_ticks * 2, src)
	beamloop.stop()
	can_act = TRUE
