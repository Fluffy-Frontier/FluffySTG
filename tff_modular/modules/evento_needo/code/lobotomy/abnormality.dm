...
/obj/effect/magic_bullet/Initialize()
	. = ..()
	playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/freischutz/shoot.ogg', 100, 1)
...
/obj/effect/magic_bullet/proc/moveBullet() // Shamelessly stolen code from immovable rod and paradise lost, GO!
	...
	while(src.x < 245 && src.x > 10 && src.y < 245 && src.y > 10)
		...
		for(var/mob/living/L in T.contents)
			if(L in beenHit)
				continue
			L.apply_damage(140, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
			L.apply_damage(14, FIRE)
			beenHit |= L
		...
		for(var/mob/living/L in nearMiss)
			if(L in beenHit)
				continue
			L.apply_damage(70, BRUTE, null, L.run_armor_check(null, BRUTE), spread_damage = TRUE)
			L.apply_damage(6, FIRE)
			beenHit |= L
...
/obj/effect/frei_trail
	...
	icon = 'tff_modular/modules/evento_needo/icons/Teguicons/projectiles_tracer.dmi'
...
/obj/effect/expresstrain/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	if(icon_state != "expressengine_1")
		return ..()
	if(clickety == 22)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/expresstrain/express_move_loop.ogg', 100, 0, 20)
		clickety = 0
	clickety += 1
	//if(clickety % 2)
...
/obj/effect/cinderella/Moved(atom/old_loc, movement_dir, forced, list/old_locs, momentum_change)
	if(clickety == 22)
		playsound(get_turf(src), 'tff_modular/modules/evento_needo/sounds/Tegusounds/abnormalities/cinderella/carriage_loop.ogg', 100, 0, 20)
		clickety = 0
	clickety += 1
	//if(clickety % 2)
	//	var/obj/effect/particle_effect/smoke/s = new(locate(src.x, src.y - 2, src.z))
