...
/obj/item/clothing/under/syndicate/nova/maid/armored
...
/obj/item/clothing/under/syndicate/nova/maid/armored/Initialize(mapload)
	. = ..()
	RegisterSignal(src, COMSIG_ITEM_POST_EQUIPPED, PROC_REF(check_equip))

/obj/item/clothing/under/syndicate/nova/maid/armored/Destroy()
	. = ..()
	UnregisterSignal(src, COMSIG_ITEM_POST_EQUIPPED)

/obj/item/clothing/under/syndicate/nova/maid/armored/mob_can_equip(mob/living/user, slot, disable_warning, bypass_equip_delay_self, ignore_equipped, indirect_action)
...
/obj/item/clothing/under/syndicate/nova/maid/armored/proc/check_equip(datum/source, mob/user, slot, initial)
	SIGNAL_HANDLER
	if(slot == ITEM_SLOT_ICLOTHING)
...
/obj/item/clothing/under/syndicate/nova/maid/armored/proc/remove_block(datum/source, obj/item, force, newloc, no_move, invdrop, silent)
	SIGNAL_HANDLER
	if(is_worn)
		UnregisterSignal(source, list(COMSIG_HUMAN_EQUIPPING_ITEM, COMSIG_MOB_UNEQUIPPED_ITEM))
		is_worn = FALSE

/obj/item/clothing/under/syndicate/nova/maid/armored/proc/block_equips(datum/source, mob/target, slot)
	SIGNAL_HANDLER
