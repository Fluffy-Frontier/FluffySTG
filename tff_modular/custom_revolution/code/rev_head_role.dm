...
/datum/antagonist/custom_rev/head/admin_add(datum/mind/new_owner, mob/admin)
	...
	if(confirm == "Да")
		var/given_name = tgui_input_text(admin, "Имя для члена данного движения:", "Нейминг")
		if(!given_name)
			return FALSE
		var/given_team_name = tgui_input_text(admin, "Название для командны/объединения:", "Тим Нейминг")
		if(!given_team_name)
			return FALSE
		...
		if(!mindshield_protection)
			return FALSE
		var/agression_factor = tgui_alert(admin, "Это враждебное по потношению к власти, компании и прочему движение?", "Бад-Гайс?", list("Да", "Нет"))
		if(!agression_factor)
			return FALSE
